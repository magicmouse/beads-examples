...
export const log_echo = function(msg) {
    ...
    g_publishers)) for (client of pub.clients) null != client.my_log && (client.my_log += "SERV> " + msg + "\n");
};
...
export function fs_write_str(mypath, mybuf, options = null) {
    var encode_ss, encoding = UTF8, result = T;
...
export function decode_remote_call(func_list, basis, buf) {
    ...
    if (null != func_ptr) {
        ...
        valkind == SOP_TREE_BIN ? bytes_get_tree(buf, parm_val = adr(new a_tree(_M, "$SOP_TREE_BIN"))) : (buf.position -= 1, 
...
ENUMS[9007191226219221] = "dd_disabled", 
FIELDS[F_dd_disabled] = !0;

export const F_dd_groupname = 9007194254913655;

ENUMS[9007194254913655] = "dd_groupname", 
FIELDS[F_dd_groupname] = !0;

export const F_dd_groups = 9007191258918813;

ENUMS[9007191258918813] = "dd_groups", 
FIELDS[F_dd_groups] = !0;

export const F_dd_hint = 9007191122773e3;

ENUMS[9007191122773e3] = "dd_hint", 
FIELDS[F_dd_hint] = !0;

export const F_dd_multiple = 9007192588830161;

ENUMS[9007192588830161] = "dd_multiple", 
FIELDS[F_dd_multiple] = !0;

export const F_dd_name = 9007192799204246;

ENUMS[9007192799204246] = "dd_name", 
FIELDS[F_dd_name] = !0;

export const F_dd_ok = 9007194092033631;

ENUMS[9007194092033631] = "dd_ok", 
FIELDS[F_dd_ok] = !0;

export const F_dd_on_change = 9007190136393669;

ENUMS[9007190136393669] = "dd_on_change", 
FIELDS[F_dd_on_change] = !0;

export const F_dd_options = 9007191166888725;

ENUMS[9007191166888725] = "dd_options", 
FIELDS[F_dd_options] = !0;

export const F_dd_required = 9007192866187472;

ENUMS[9007192866187472] = "dd_required", 
FIELDS[F_dd_required] = !0;

export const F_dd_selected = 9007191385192996;

ENUMS[9007191385192996] = "dd_selected", 
FIELDS[F_dd_selected] = !0;

export const F_dd_str = 9007192253881088;

ENUMS[9007192253881088] = "dd_str", 
FIELDS[F_dd_str] = !0;

export const F_dd_sysobj = 9007192636888243;

ENUMS[9007192636888243] = "dd_sysobj", 
FIELDS[F_dd_sysobj] = !0;

export const F_dd_tip = 9007192179090138;

ENUMS[9007192179090138] = "dd_tip", 
FIELDS[F_dd_tip] = !0;

export const F_dd_value_ptr = 9007191932136795;

ENUMS[9007191932136795] = "dd_value_ptr", 
FIELDS[F_dd_value_ptr] = !0;

export const F_dd_value = 9007192890917530;

ENUMS[9007192890917530] = "dd_value", 
FIELDS[F_dd_value] = !0;
...
ENUMS[9007193617762828] = "dx", 
FIELDS[F_dx] = !0;

export const F_dy = 9007193634540447;

ENUMS[9007193634540447] = "dy", 
FIELDS[F_dy] = !0;
...
function js_shadow_ss(path) {
    for (var index = 1, ss = ""; ;) {
        ...
        is_not_numeric(opacity) == T && (opacity = 1), 
        index > 1 && (ss += ","), 
        ss += `${dx}px ${dy}px ${blur}px ${spread}px ${js_color_to_str(color, opacity)}`, 
        index += 1;
    }
    return ss;
}
...
export function remote_call(connection, func_name, func_hash, parms) {
    ...
    parmval instanceof a_path ? bytes_put_vtree(gg_buf, parmval) : bytes_put_val(gg_buf, parmval, 0), 
...
export function cursor_set(curs) {
    if (setv(_M, 0, runtime, F_cursor_changed, T), 
    curs != g_curr_cursor) {
...
export function numeric_arg(val) {
    return void 0 != val && "number" == typeof val && is_numeric(val) == T ? val : (CHECKS && argument_err("bad numeric parameter"), 
    0);
};
...
export function bytes_put_val(buf, val, flag) {
    ...
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, ZERO", buf.position))) : val == T ? (bytes_put_u8(buf, flag | NOV_Y), 
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, T", buf.position))) : val == F ? (bytes_put_u8(buf, flag | NOV_N), 
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, F", buf.position))) : val == U ? (bytes_put_u8(buf, flag | NOV_U), 
...
export function bit_set(mod, loc, valp, bitx, newval) {
    ...
    oldval == U && (oldval = 0);
    var bits = oldval >> 0;
    newval == T ? bits |= 1 << bitx : bits &= ~(1 << bitx), 
...
function follow_blocks(depth, b, p) {
    ...
    null != b.drawer.tracker && setv(_M, 0, p, F_btracks, T), 
...
export function browser_has_bluetooth(flagptr) {
    if (void 0 == navigator.bluetooth) return c, 
    setv(_M, 0, flagptr, T), 
...
export function browser_bluetooth_getc(service_ss, char_ss, finish_func) {
    ...
    characteristic.readValue())).then(value => {
        ...
        setv(_M, 0, g_bluetooth, 0, T);
        let batteryLevel = value.getUint8(0);
        log(">> Battery Level is " + batteryLevel + "%"), 
        finish_func.code(g_bluetooth);
    }).catch(error => {
        setv(_M, 0, g_bluetooth, 0, F), 
...
export function halt(msg) {
    log("## halt: " + msg), 
    IS_NODE ? process.exit(1) : window.alert(msg);
};

export function quit(err = 0) {
    log(`## quit ${err}`), 
    IS_NODE ? process.exit(err) : (window.open("", "_parent", ""), 
    window.close());
};
...
export function debug_jump(targ_major, is_blackbox) {
    if (is_numeric(targ_major) == T) {
        ...
        is_blackbox == T ? (curr_micros = g_blackbox_micro, 
...
export function solve_point(options) {
    ...
    is_numeric(basis_l) == T && is_numeric(basis_t) == T && is_numeric(basis_w) == T && is_numeric(basis_h) == T || argument_err("bad basis"), 
    (pin - 1) % 3) {
      case 0:
        x = basis_l;
        break;
...
function delta_clock(delta) {
    ...
    TRACE_LOOM && log(str.conv("delta_clock, lx={}, time={n3}, is_rel={bool}", lx, item.time, item.is_relative)), 
...
export function animate_num_bezier(valpath, x1, y1, x2, y2, options) {
    ...
    startval !== endval) {
        ...
        path_setv(_M, WHERE_LOOM, busypath, T), 
...
export function tree_sibling_hi(curr) {
    ...
    parent.children) if (is_numeric(sub = parent.children[ix].sub) == T && sub >= refsub && sub < bestsub && (bestsub = sub) == refsub) break;
...
export function draw_str(container, ss, options) {
    ...
    setv(_M, 0, metrics, F_y, 0)); else if (null == container.can) if ((mydiv = document.createElement("div")).style.position = "absolute", 
    ...
    container.appendChild(mydiv); else {
        switch (0 == maxlines && (mydiv.style.display = "flex"), 
...
export function validate_input(field, val) {
    ...
    if (0 == val.length) is_required ? (result = F, 
    TRACE_INPUT && log("  failed because empty, but required")) : (result = T, 
    TRACE_INPUT && log("  okay because empty, not required")); else if (minlen != U && val.length < minlen) result = F, 
    TRACE_INPUT && log("  failed because too short"); else if (maxlen != U && val.length > maxlen) result = F, 
    TRACE_INPUT && log("  failed because too long"); else {
        var vpatt = get_object(field, F_in_pattern);
        null != vpatt && (vpatt.lastIndex = 0, 
        vpatt.test(val) || (result = F, 
...
function calc_warnss(elem, field) {
    ...
    var result = "";
    return elem.required && 0 == elem.value.trim().length ? elem.my_show_star ? result = FIELD_EMPTY_SYMBOL : !0 : ok_flag == F && (result = FIELD_BAD_SYMBOL), 
    result;
}
...
export function dropdown_start(mod, loc, dropdown) {
    ...
    name == U_STR && (name = (g_next_bid++).toString()), 
    sel_elem.name = name, 
    sel_elem.id = name;
    var tip = gets(dropdown, F_dd_tip);
    tip != U_STR && (sel_elem.title = tip);
    var placeholder = gets(dropdown, F_dd_hint);
    placeholder != U_STR && ((elem = document.createElement("option")).label = placeholder, 
    elem.value = "", sel_elem.appendChild(elem)), 
    sel_elem.my_field = dropdown;
    var ngroups = count(adr(dropdown, F_dd_groups));
    if (TRACE_DROPDOWN && log(`dropdown_start, id=${sel_elem.id}, ngroups=${ngroups}`), 
    0 != ngroups) {
        group = sel_elem;
        for (var groupx = 1; groupx <= ngroups; groupx++) {
            ngroups > 1 && (group = document.createElement("optgroup"), 
            sel_elem.appendChild(group), 
            (ss = gets(dropdown, F_dd_groups, groupx, F_dd_groupname)) == U_STR && (ss = "???"), 
            group.label = ss, (dflag = getn(dropdown, F_dd_groups, groupx, F_dd_disabled)) == T && (group.disabled = !0));
            ...
            valss = gets(dropdown, F_dd_groups, groupx, F_dd_options, itemx, F_dd_value), 
            dflag = getn(dropdown, F_dd_groups, groupx, F_dd_options, itemx, F_dd_disabled), 
            sflag = getn(dropdown, F_dd_groups, groupx, F_dd_options, itemx, F_dd_selected), 
            elem = document.createElement("option"), 
            group.appendChild(elem), 
            elem.text = ss, valss != U_STR && (elem.value = valss), 
            dflag == T && (elem.disabled = !0), 
            sflag == T && (elem.selected = !0);
        }
...
export function dropdown_draw(container, field, options) {
    ...
    if (null != elem) {
        ...
        tip != U_STR && (elem.title = tip), 
        elem.onchange = on_dropdown_change, 
        ...
        elem.style.textIndent = to_px(indent), 
        0 != corner && (elem.style.borderRadius = to_px(corner)), 
        elem.style.border = `${border.toFixed(1)}px solid ${js_color_to_str(bord_color)}`, 
        backcolor != U && (elem.style.backgroundColor = js_color_to_str(backcolor)), 
        elem.style.color = js_color_to_str(color), 
        elem.style.font = gen_font_ss(font, size, U, weight, is_italic), 
        1 != opacity && (elem.style.opacity = opacity.toFixed(3)), 
...
export function input_draw(container, field, options) {
    ...
    if (null != elem) {
        ...
        0 != corner && (elem.style.borderRadius = to_px(corner)), 
        elem.style.border = `${border.toFixed(1)}px solid ${js_color_to_str(bord_color)}`, 
        TRACE_DINPUT && log(`  input border, color=${js_color_to_str(bord_color)}`), 
        backcolor != U && (elem.style.backgroundColor = js_color_to_str(backcolor)), 
...
function svg_define_shadow(shad_ptr, wrapper) {
    ...
    for (var dx, dy, color, opacity, blur, ix = 1; (blur = getn(shad_ptr, ix, F_blur)) != U; ) {
        (dx = getn(shad_ptr, ix, F_dx)) == U && (dx = 0), 
        (dy = getn(shad_ptr, ix, F_dy)) == U && (dy = 0), 
        (color = getn(shad_ptr, ix, F_color)) == U && (color = BLACK), 
        (opacity = getn(shad_ptr, ix, F_opacity)) == U && (opacity = 1);
        let shadf = document.createElementNS(SVG_NS, "feDropShadow");
        filter_elem.appendChild(shadf), 
        shadf.setAttribute("dx", dx.toFixed(1)), 
        shadf.setAttribute("dy", dy.toFixed(1)), 
        shadf.setAttribute("stdDeviation", blur.toFixed(1)), 
        shadf.setAttribute("flood-color", js_color_to_str(color, opacity)), 
        ix += 1;
    }
    return filter_id;
}
...
export function draw_rect(container, options) {
    ...
    for (let property in options) switch (property) {
      ...
      case "bord_pos":
        is_numeric(options.bord_pos) !== T && argument_err("bad pos"), 
...
export function draw_line(container, options) {
    ...
    dx !== U && (x2 = add(x1, dx)), 
    dy !== U && (y2 = add(y1, dy)), 
    is_numeric(x1) !== T || is_numeric(y1) !== T || is_numeric(x2) !== T || is_numeric(y2) !== T) return void (CHECKS && argument_err("bad args"));
    switch (joinx) {
      case JOINT_ROUND:
        joint = "round";
        break;
...
export function draw_polygon(container, polygon, options) {
    ...
    is_numeric(x) === T && is_numeric(y) === T ? first ? (first = !1, 
    ...
    for (;iter.next(); ) {
        ...
        !udb.objectStoreNames.contains(s_name)) {
            ...
            for (;iter2.next(); ) {
                ...
                flags2.unique = ix_uniq === T, 
                flags2.multiEntry = ix_mult === T, 
...
function add_select_list(sel, list) {
    ...
    getn(list, ix, F_opt_selected) === T && (option.selected = !0), 
    getn(list, ix, F_opt_disabled) === T && (option.disabled = !0), 
...
export function browser_draw_pulldown(container, box, items, field_ptr, options) {
    ...
    getn(field_ptr, F_in_required === T) && (sel.required = !0), 
...
export function a_loop(options) {
    ...
    this.levels = [], options) switch (property) {
      ...
      case "by":
        ...
        is_numeric(this.range_delta) != T) return void argument_err("bad 'delta' value");
        if (this.range_delta <= 0) return void argument_err("delta must be positive");
        break;

      case "from":
        if (this.range_from = options.from, 
        is_numeric(this.range_from) != T) return void argument_err("bad from: value");
      ...
      case "to_":
        ...
        is_numeric(this.range_to) != T) return void argument_err("bad 'to:' value");
        break;
...
function q_next_task() {
    ...
    q_loader = new a_loader_x(mytask.folder, mytask.fname, TYPE_BYTES, on_task_complete, T), 
...
function on_task_complete(okay, data) {
    okay === T ? (TRACE_TASKS && log(str.conv("## task {int} okay", q_taskx)), 
...
export function add_family(family, dimensions, nonlinear = F) {
    nonlinear === T && setv(_M, 0, families, family, F_fam_nonlinear, T), 
...
export function mul(a, b) {
    ...
    ERR_MEAS) : (cm = b.clone(), 
    a === T || (cm.mag = a === F || 0 === a ? 0 : a === U ? U : b.mag * a), 
    cm) : a_is_mea && b_is_num ? is_err_enum_b(b) ? (TRAP_ERR && internal_err("bad arg"), 
    ERR_MEAS) : (cm = a.clone(), 
    b === T || (cm.mag = b === F || 0 === b ? 0 : b === U ? U : a.mag * b), 
...
export function insert_tree(mod, loc, src, dest) {
    ...
    if (bx.exists && bx.lev.length > 1 && is_numeric(refsub_g)) {
        ...
        for (var i = 0; i < naffected; i++) {
            var offset = naffected - i;
            (mov_src = dest.clone()).key[mov_src.key.length - 1] += offset - 1, 
            (mov_dst = dest.clone()).key[mov_src.key.length - 1] += offset, 
            move_tree(mod, loc, mov_src, mov_dst);
        }
    }
...
function alloc_loom_elem(options) {
    ...
    is_numeric(interval) === T && interval < .01 && argument_err("cannot use repeat intervals less than 10 msec"), 
    elem.is_relative = !1, 
...
function loom_release_id(cleared_id) {
    ...
    lx = 0; lx < g_loom.length; lx++) (item = g_loom[lx]).prereq === cleared_id && (CHECKS && is_numeric(item.time) !== T && internal_err("loom time not numeric"), 
...
function abort_animate_num(ax, va, finishflag = F) {
    finishflag === T && path_setv(_M, WHERE_LOOM, va.valpath, va.endval), 
    path_setv(_M, WHERE_LOOM, va.busypath, F), 
...
export function animate_num_ctrl(id, options) {
    ...
    pause === T ? va.paused || pause_start(va) : resume === T ? va.paused && pause_end(va) : toggle === T ? va.paused ? pause_end(va) : pause_start(va) : finish === T ? abort_animate_num(ax, va, T) : abort === T && abort_animate_num(ax, va, F)) : CHECKS && argument_err("not found");
};
...
export function loom_process_events() {
    ...
    va_elapsed >= va.duration ? (path_setv(_M, WHERE_LOOM, va.busypath, F), 
    ...
    lx += 1; else {
        switch (elem.evkind) {
          case EV_TIMER:
            ...
            elem.interval_mon != U && getn(runtime, F_monitor_active) == T ? elem.time = currtime + elem.interval_mon : elem.time = currtime + elem.interval, 
            elem.is_relative = !0;
            break;
...
export function rtl_init() {
    if (is_rtl) TRACE_INIT && log("--- ignoring second rtl init"); else {
        ...
        cpu = CPU_ARM, is_full_screen = T) : /android/i.test(navigator.userAgent) ? (OS_KIND = OS_AND, 
        setv(_M, 0, runtime, F_os_kind, OS_AND), 
        cpu = CPU_ARM, is_full_screen = T) : is_MAC ? (OS_KIND = OS_OSX, 
...
export function k_layer(parent, drawf, options) {
    ...
    for (let property in options) switch (property) {
      ...
      case "opacity":
        is_numeric(options.opacity) != T && argument_err("bad arg"), 
        opacity = options.opacity;
        break;

      case "pin":
        is_numeric(options.pin) != T && argument_err("bad arg"), 
        pin = options.pin;
        break;
      ...
      case "tx":
        is_numeric(options.tx) != T && argument_err("bad arg"), 
        left = options.tx;
        break;

      case "ty":
        is_numeric(options.ty) != T && argument_err("bad arg"), 
        top = options.ty;
        break;

      case "skewx":
        is_numeric(options.skewx) != T && argument_err("bad arg"), 
        skewx = options.skewx;
        break;

      case "skewy":
        is_numeric(options.skewy) != T && argument_err("bad arg"), 
        skewy = options.skewy;
        break;
    ...
    if (!CHECKS || is_numeric(left) == T && is_numeric(top) == T && is_numeric(width) == T && is_numeric(height) == T || argument_err("bad bounds"), 
    null != canvas_dest) {
...
function k_inset(b, arg_meas_n, arg_meas_s, arg_meas_e, arg_meas_w, arg_unit, arg_draw_func, arg_track_func = NOTRACK) {
    ...
    inset_n = 0, inset_s = 0, 
    inset_e = 0, inset_w = 0, 
    is_numeric(arg_meas_n) == T && arg_meas_n >= 0 ? inset_n = unit_to_pixels(b, arg_meas_n, arg_unit) : CHECKS && argument_err("invalid inset"), 
    is_numeric(arg_meas_s) == T && arg_meas_s >= 0 ? inset_s = unit_to_pixels(b, arg_meas_s, arg_unit) : CHECKS && argument_err("invalid inset"), 
    is_numeric(arg_meas_e) == T && arg_meas_e >= 0 ? inset_e = unit_to_pixels(b, arg_meas_e, arg_unit) : CHECKS && argument_err("invalid inset"), 
    is_numeric(arg_meas_w) == T && arg_meas_w >= 0 ? inset_w = unit_to_pixels(b, arg_meas_w, arg_unit) : CHECKS && argument_err("invalid inset"), 
...
function k_indent_to_net(block, targh, targv, justh, justv, arg_draw_func, arg_track_func = NOTRACK) {
    ...
    !CHECKS || is_numeric(justh) == T && is_numeric(justv) == T && is_numeric(targh) == T && is_numeric(targv) == T || argument_err("non-numeric inputs"), 
...
function callback_find_any(b, data) {
    ...
    null != b.tbl) ; else if (null != b.drawer) {
        ...
        null != tracker ? (absorbval = null != b.drawer.parms ? tracker.apply(null, [ b, data.event ].concat(b.drawer.parms)) : tracker(b, data.event)) == T ? (data.go = !1, 
...
function find_xy_in_grid(b, event, localx, localy) {
    ...
    (absorbval = unfreeze_track(b, event, b.drawer)) == T && (g_last_majorx > 0 && (setv(_M, 0, runtime, F_major_steps, g_last_majorx, F_hist_cell_cum, cumx), 
...
function do_pointer_event(evkind, evtime, wherex, wherey, touch) {
    ...
    setv(_M, WHERE_LOOM, event, F_is_shift, T)), 
    touch.mod_alt && setv(_M, WHERE_LOOM, event, F_is_alt, T), 
    touch.mod_cmd && setv(_M, WHERE_LOOM, event, F_is_cmd, T), 
    touch.mod_ctl && setv(_M, WHERE_LOOM, event, F_is_ctrl, T), 
    g_last_majorx = -1, 
    evkind != EV_HOVER) {
...
function do_key_event(when, unistr, keycode, is_shift = !1, is_alt = !1, is_cmd = !1, is_ctrl = !1) {
    let b;
    if (g_interface_locked == T) return TRACE_EVENTS && log(str.conv("interface locked / suppressing key event, keycode=", keycode)), 
    !1;
    ...
    is_shift && setv(_M, WHERE_LOOM, event, F_is_shift, T), 
    is_alt && setv(_M, WHERE_LOOM, event, F_is_alt, T), 
    is_cmd && setv(_M, WHERE_LOOM, event, F_is_cmd, T), 
    is_ctrl && setv(_M, WHERE_LOOM, event, F_is_ctrl, T);
...
function try_in_block(b, event) {
    ...
    null != tracker) {
        ...
        null != b.tbl) {
            ...
            let absorbed;
            if ((absorbed = unfreeze_track(b, event, b.drawer)) == T) return TRACE_XY && log("  CONSUMED by block"), 
...
function trickle(seconds) {
    ...
    if (null != g_root_block) {
        ...
        g_last_drawn_seq < g_currseq) {
            ...
            getn(runtime, F_update_blocklist) == T && debug_get_blocks(adr(debug_blocklist)), 
