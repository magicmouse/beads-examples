import * as str from "./node_str.mjs";
...
import fs from "/usr/local/lib/node_modules/graceful-fs/graceful-fs.js";

import https from "https";

var g_publishers = [], navigator = {
    language: "en-US",
    userAgent: "Intel"
}, window = null;

export const IS_NODE = !0;

const TRACE_CALL = !1, TRACE_FS = !1;

console.log("START OF NODE");

export const log_echo = function(msg) {
    var pub, client;
    for (pub of (console.log(msg), 
    g_publishers)) for (client of pub.clients) null != client.my_log && (client.my_log += "@@ " + msg + "\n");
};

function is_ArrayBuffer(value) {
    return value instanceof ArrayBuffer || "[object ArrayBuffer]" === toString.call(value);
}

function is_Uint8Array(value) {
    return value instanceof Uint8Array || "[object Uint8Array]" === toString.call(value);
}

var Audio = function() {
    this.parm = 0, this.load = null;
}, document = null;

function a_publisher(arg_cert, arg_key, arg_port, arg_id, arg_bandwidth, arg_data, arg_funclist) {
    if ("" == arg_key) this.wss = new ws.Server({
        port: arg_port
    }); else {
        const mykey = fs.readFileSync(arg_key);
        null == mykey && console.log(`### cannot read publisher key ${arg_key} `);
        const mycert = fs.readFileSync(arg_cert);
        null == mycert && console.log(`### cannot read publisher cert ${arg_cert}`);
        const my_server = https.createServer({
            key: mykey,
            cert: mycert
        }, (req, res) => {});
        this.wss = new ws.Server({
            server: my_server
        }), my_server.listen(arg_port);
    }
    this.id = arg_id, this.bandwidth = arg_bandwidth, 
    this.data_ptr = arg_data, 
    this.funclist = arg_funclist, 
    this.clients = [];
}

export function subscriber_disconnect(arg_server, arg_slot, arg_delay) {
    console.log(`subscriber_disconnect, slot=${arg_slot}, delay=${arg_delay}`), 
    arg_delay > 0 ? setTimeout(disconnect_now, 1e3 * arg_delay, arg_server, arg_slot) : disconnect_now(arg_sysobj, arg_slot);
};

function disconnect_now(arg_server, arg_slot) {
    null != arg_server.clients ? (console.log(`disconnect_now, server has ${arg_server.clients.length} clients`), 
    null != arg_server.clients[arg_slot] ? arg_server.clients[arg_slot].terminate() : console.log(`disconnect_now bad slot ${arg_slot}`)) : console.log("disconnect_now bad server record");
}

export function publish_start(arg_cert, arg_key, arg_port, arg_id, arg_bandwidth, arg_data, ...arg_funclist) {
    if (arg_data instanceof a_function) ; else if (arg_data instanceof a_path) ; else if (null != arg_data) return void argument_err(`publish_start, data type=${arg_data.constructor.name}`);
    TRACE_CHAN && console.log(`publishing on port ${arg_port}`);
    var myserver = new a_publisher(arg_cert, arg_key, arg_port, arg_id, arg_bandwidth, arg_data, arg_funclist);
    return g_publishers.push(myserver), 
    myserver.wss.on("connection", function(ws, request) {
        var url = request.url;
        if (TRACE_CHAN && console.log(`connection from client ${request.connection.remoteAddress}:${request.connection.remotePort}, url=${url}`), 
        "/BE01/" != url.substr(0, 6)) return console.log("--bad version, killing subscribe"), 
        void ws.terminate();
        var id_ss = url.substr(6, 16);
        if (str.hex_to_num(id_ss) != arg_id) return console.log("--bad secret id, killing subscribe"), 
        void ws.terminate();
        var opcode = url.substr(22, 6);
        if ("/SUBS/" != opcode) return console.log("--bad opcode, killing subscribe"), 
        void ws.terminate();
        var flagss = url.substr(28, 4);
        ws.my_log = null, "1" == flagss.substr(3, 1) && (ws.my_log = "");
        var extra = "";
        "/" == (opcode = url.substr(32, 1)) ? (extra = decodeURIComponent(url.substr(33)), 
        console.log(`  extra is [${extra}]`)) : console.log(`  no extra, opcode=${opcode}`);
        var nclients = myserver.clients.push(ws);
        ws.my_clientx = nclients - 1, 
        ws.my_subset = arg_data instanceof a_function ? arg_data.code(ws.my_clientx, extra) : arg_data, 
        ws.my_seq = 0, ws.my_update_interval = 1, 
        ws.my_funclist = arg_funclist, 
        null == arg_data ? ws.my_next_update = INFINITY : (ws.my_next_update = 0, 
        publish_first_send(ws)), 
        ws.on("message", function data_handler(message) {
            if ("string" == typeof message) TRACE_CHAN && console.trace(`NODE: received string [${message}]`); else if (is_ArrayBuffer(message)) TRACE_CHAN && console.trace("NODE: received is_ArrayBuffer!"); else if (is_Uint8Array(message)) {
                TRACE_CHAN && console.log(`received Uint8Array, len=${message.byteLength}, off=${message.byteOffset}`);
                var mybuf = ArrayBuffer_to_ByteArray(message.buffer.slice(message.byteOffset), LITTLE_ENDIAN);
                process_serv_chunks(request.connection.remoteAddress, arg_funclist, arg_data, mybuf) < 0 && ws.terminate();
            } else TRACE_CHAN && console.trace(`received some other kind of object ${toString.call(message)}`);
        }), ws.on("close", function on_ws_close() {
            TRACE_CHAN && console.log(`--ws.close, clientx ${ws.my_clientx}`), 
            ws.my_clientx >= 0 && (myserver.clients.splice(ws.my_clientx, 1), 
            ws.my_clientx = -1);
        }), ws.on("error", function on_ws_error() {
            TRACE_CHAN && console.log(`--ws.error, clientx ${ws.my_clientx}`), 
            ws.my_clientx >= 0 && (myserver.clients.splice(ws.my_clientx, 1), 
            ws.my_clientx = -1);
        });
    }), myserver;
...
export function publish_stop(myserver) {
    var ix = g_publishers.indexOf(myserver);
    if (TRACE_CHAN && console.log(`NODE: publish_stop, removing publisher at index ${ix}`), 
    ix >= 0) for (client of (g_publishers.splice(ix, 1), 
    myserver.clients)) 1 == client.readyState && client.close();
};

import child_process from "child_process";

export function os_shell(cmd, callback) {
    child_process.exec(cmd, function nested_callback(error, stdout, stderr) {
        var errmsg = "";
        error && (errmsg = error.toString(), 
        null == callback && console.log(errmsg));
        null != callback && callback(errmsg, stdout, stderr);
    });
};

export function fs_read_str(mypath, options) {
    var encode_ss, buf, encoding = UTF8;
    ...
    switch (encoding) {
      ...
      default:
        argument_err("bad encoding");
    }
    try {
        buf = fs.readFileSync(mypath, encode_ss);
    } catch (err) {
        buf = ERR_STR;
    }
    return buf;
...
export function fs_write_str(mypath, mybuf, options = null) {
    var encode_ss, encoding = UTF8, result = Y;
    ...
    try {
        fs.writeFileSync(mypath, mybuf, encode_ss);
    } catch (err) {
        TRACE_FS && console.log(`Cannot write ${mypath}, err=${err.message}`), 
        result = N;
    }
    return result;
};

export function fs_modtime(filepath) {
    try {
        const fd = fs.openSync(filepath, "r");
        return fs.fstatSync(fd).mtimeMs / 1e3;
    } catch (err) {
        return U;
    }
};

const BEAD_SIG = 1684104546;

export function fs_write_tree(filepath, mybuf) {
    let pos_len, pos_eof;
    null == gg_buf ? gg_buf = new_bytes(1e5, LITTLE_ENDIAN) : gg_buf.position = 0, 
    bytes_put_u32(gg_buf, BEAD_SIG), 
    bytes_put_u8(gg_buf, 1), 
    bytes_put_u8(gg_buf, SOP_BATCH_START), 
    bytes_put_u8(gg_buf, 0), 
    pos_len = gg_buf.position, 
    bytes_put_u32(gg_buf, 0), 
    bytes_put_vtree(gg_buf, adr(mybuf)), 
    bytes_put_u8(gg_buf, SOP_BATCH_END), 
    pos_eof = gg_buf.position, 
    gg_buf.position = pos_len, 
    bytes_put_u32(gg_buf, pos_eof - pos_len - 4), 
    gg_buf.position = pos_eof, 
    TRACE_FS && console.log(str.conv("--writing {} bytes to {}", pos_eof, filepath));
    var myview = new DataView(gg_buf.arraybuf, 0, pos_eof);
    try {
        return fs.writeFileSync(filepath, myview, "binary"), 
        Y;
    } catch (err) {
        return console.log(`## Cannot write ${filepath}, err=${err.message}`), 
        ERR;
    }
};
...
function buf2hex(buffer, nbytes) {
    if (0 == byteToHex.length) for (let n = 0; n <= 255; ++n) {
        const hexOctet = n.toString(16).padStart(2, "0");
        byteToHex.push(hexOctet);
    }
    const buff = new Uint8Array(buffer), hexOctets = [];
    for (let i = 0; i < nbytes; ++i) hexOctets.push(byteToHex[buff[i]]);
    return hexOctets.join("");
}

function nodebuf_to_arraybuffer(buf) {
    return buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.length);
}

export function fs_read_tree(mod, loc, filepath, mydest) {
    var nodebuf;
    TRACE_FS && console.log(`fs_read_tree ${filepath}`);
    try {
        nodebuf = fs.readFileSync(filepath);
    } catch (err) {
        return console.log(`## Cannot read ${filepath}, err=${err.message}`), 
        ERR;
    }
    var bytes = ArrayBuffer_to_ByteArray(nodebuf_to_arraybuffer(nodebuf), LITTLE_ENDIAN), sig = bytes_get_u32(bytes);
    if (sig != BEAD_SIG) return console.log(`## fs_read_tree: bad signature: 0x${str.to_str(sig, {
        base: 16
    })}`), ERR;
    TRACE_FS && console.log("- sig ok");
    var ver = bytes_get_u8(bytes);
    return 1 != ver ? (console.log(`## fs_read_tree: bad version: 0x${str.to_str(ver, {
        base: 16
    })}`), ERR) : (TRACE_FS && console.log("- ver ok"), 
    process_client_batches(bytes, mydest), 
    Y);
};

function ws_send(socket, buf, startpos, endpos) {
    if (1 == socket.readyState) {
        var bytes = new Uint8Array(buf.arraybuf, startpos, endpos - startpos);
        socket.send(bytes, {
            binary: !0
        });
    }
...
function process_serv_chunks(client_ip, funclist, basis, mybuf) {
    var opcode;
    if ((opcode = bytes_get_u8(mybuf)) != SOP_BATCH_START) return TRACE_DECODE && console.log(`Expected SOP_BATCH_START, got 0x${opcode.toString(16)}`), 
    -1;
    var compression = bytes_get_u8(mybuf), len = bytes_get_i32(mybuf), chunk_start = mybuf.position;
    TRACE_DECODE && console.log(`process_serv_chunks, comp=${compression}, len=${len}, chunk_start=${chunk_start}`);
    bigloop: for (;;) switch (opcode = bytes_get_u8(mybuf)) {
      case SOP_NOP:
        TRACE_DECODE && console.log("-- opcode SOP_NOP");
        break;

      case SOP_CALL:
        TRACE_DECODE && console.log("-- opcode SOP_CALL"), 
        decode_remote_call(funclist, basis, mybuf);
        break;

      case SOP_BBOX:
        var filename = bytes_get_str(mybuf), mail_addr = bytes_get_str(mybuf), mail_subj = bytes_get_str(mybuf), mail_body = bytes_get_str(mybuf);
        TRACE_BLACKBOX && console.log(`filename=${filename}, email=[${mail_addr}] subj=[${mail_subj}]`);
        var glob = bytes_get_bytes(mybuf), destfolder = `./${filename}`;
        fs.mkdir(destfolder, {
            recursive: !0
        }, fs_callback);
        var date_suffix = str.time_to_str("[year]-[month2]-[day2]@[hour][minute]"), destpath = `${destfolder}/${filename}-${client_ip.replace(/:/g, "-").slice(-16)}-${date_suffix}.bbox`;
        fs.writeFile(destpath, glob.view, fs_callback), 
        "" != mail_addr && ("" == mail_subj && (mail_subj = `blackbox received for ${filename}`), 
        "" == mail_body && (mail_body = `file was store at ${destpath}`), 
        console.log(`sending email to ${mail_addr}`));
        break;
      ...
      case SOP_BATCH_END:
        TRACE_DECODE && console.log("-- opcode SOP_BATCH_END");
        break bigloop;

      default:
        TRACE_DECODE && console.trace(`-- bad opcode: ${opcode}`);
        break bigloop;
    }
    TRACE_DECODE && console.log(`    process_serv_chunks: at end, pos=${mybuf.position}, expect=${chunk_start + len}`);
}

function fs_callback(err) {
    null != err && console.log(`fs_callback, err=${err}`);
}

function publish_first_send(ws) {
    let pos_len, pos_eof;
    TRACE_CHAN && console.log("--publish_first_send"), 
    null == gg_buf ? gg_buf = new_bytes(1e5, LITTLE_ENDIAN) : gg_buf.position = 0, 
    bytes_put_u8(gg_buf, SOP_BATCH_START), 
    bytes_put_u8(gg_buf, 0), 
    pos_len = gg_buf.position, 
    bytes_put_u32(gg_buf, 0), 
    null != ws.my_log && "" != ws.my_log && (bytes_put_u8(gg_buf, SOP_LOG), 
    bytes_put_str(gg_buf, ws.my_log), 
    ws.my_log = ""), bytes_put_vtree(gg_buf, ws.my_subset), 
    bytes_put_u8(gg_buf, SOP_BATCH_END), 
    pos_eof = gg_buf.position, 
    gg_buf.position = pos_len, 
    bytes_put_u32(gg_buf, pos_eof - pos_len - 4), 
    gg_buf.position = pos_eof, 
    TRACE_CHAN && console.log(str.conv("--sending {} bytes to subscriber", pos_eof)), 
    ws_send(ws, gg_buf, 0, pos_eof), 
    ws.my_seq = g_currseq, 
    ws.my_next_update = elapsed_raw + ws.my_update_interval;
}

var g_timer_id, g_node_tick = 0;

function node_tick() {
    clock_tick(), loom_process_events(), 
    update_subscribers(), 
    ++g_node_tick >= 36e3 && (trim_micro(), 
    g_node_tick = 0);
}

function trim_micro() {
    var oldest = g_currseq;
    for (const pub of g_publishers) for (const ws of pub.clients) ws.last_seq < oldest && (oldest = ws.last_seq);
    var oldlen = g_micro.length;
    for (const micro of g_micro) {
        if (micro.seq > oldest) break;
        g_micro.shift();
    }
    TRACE_CHAN && console.log(`trim_micro, len was ${oldlen}, trimmed is ${g_micro.length}`);
}

function update_subscribers() {
    var pub, ws, currtime = elapsed_raw;
    for (pub of g_publishers) for (ws of pub.clients) currtime >= ws.my_next_update && publish_send_update(currtime, ws);
}

function count_matches(key1, key2) {
    for (var len1 = key1.length, len2 = key2.length, ncommon = len1 < len2 ? len1 : len2, i = 0; i < ncommon; i++) if (key1[i] != key2[i]) return i;
    return ncommon;
}

export function publish_send_update(currtime, ws) {
    let pos_len, pos_eof, relkey, keylen, i, need_header = !0, nlevels = ws.my_subset.key.length;
    ...
    TRACE_ENCODE && console.log(str.conv("{n3}: publish_send_update clientx={} my_seq={}, nmicro={}", currtime, ws.my_clientx, ws.my_seq, g_micro.length)), 
    null != ws.my_log && "" != ws.my_log && (emit_header(), 
    bytes_put_u8(gg_buf, SOP_LOG), 
    bytes_put_str(gg_buf, ws.my_log), 
    ws.my_log = "");
    var nmatches, npops, some_nodes = !1, lastkey = [];
    for (const hist of g_micro) if (!(hist.seq <= ws.my_seq) && hist.kind & OPS_THAT_ARE_PUBLISHED && (TRACE_ENCODE && console.log(str.conv("  -- testing hist seq {} with path={}", hist.seq, hist.path)), 
    is_path_in_path(hist.path, ws.my_subset))) {
        for (some_nodes || (need_header && emit_header(), 
        bytes_put_u8(gg_buf, SOP_TREE_BIN), 
        some_nodes = !0), relkey = trim_key_top(hist.path.key, nlevels), 
        TRACE_ENCODE && console.log(str.conv("  -- sending key {key}, relkey={key}, val={}", hist.path.key, relkey, hist.newval)), 
        nmatches = count_matches(lastkey, relkey), 
        npops = lastkey.length - nmatches, 
        i = 0; i < npops; i++) TRACE_ENCODE && console.log("   POP"), 
        bytes_put_u8(gg_buf, SUB_POP);
        for (keylen = relkey.length, 
        i = nmatches; i < keylen; i++) TRACE_ENCODE && console.log(str.conv("   SUB {}", relkey[i])), 
        bytes_put_subscript(gg_buf, relkey[i]);
        bytes_put_val(gg_buf, hist.newval), 
        (lastkey = relkey).length > 0 && lastkey.pop();
    }
    some_nodes && bytes_put_u8(gg_buf, SOP_EOT), 
    need_header || (bytes_put_u8(gg_buf, SOP_BATCH_END), 
    pos_eof = gg_buf.position, 
    gg_buf.position = pos_len, 
    bytes_put_u32(gg_buf, pos_eof - pos_len - 4), 
    TRACE_ENCODE && console.log(str.conv("--sending update of {} bytes to subscriber {}", pos_eof, ws.my_clientx)), 
    ws_send(ws, gg_buf, 0, pos_eof)), 
    ws.my_seq = g_currseq, 
    ws.my_next_update = currtime + ws.my_update_interval;
};

function trim_key_top(key, nlevels) {
    if (nlevels <= 0) return key.slice();
    let result = [], len = key.length;
    for (let i = nlevels; i < len; i++) result.push(key[i]);
    return result;
}

export function decode_remote_call(func_list, basis, buf) {
    var func_hash = bytes_get_u32(buf), func_ptr = find_function_hash(func_hash, func_list);
    if (null != func_ptr) {
        var nparms = bytes_get_u8(buf);
        TRACE_CALL && console.log(`decode_remote_call, func=${func_ptr.name}, nparms=${nparms}`);
        var parm_val, valkind, i, parms_array = [];
        for (i = 0; i < nparms; i++) valkind = bytes_get_u8(buf), 
        TRACE_CALL && console.log(`  at offset ${buf.position - 1}, parm ${i}: valkind=${valkind}`), 
        valkind == NOV_RELPATH ? parm_val = bytes_get_relpath(buf, basis) : (buf.position -= 1, 
        parm_val = bytes_get_val(buf)), 
        TRACE_CALL && console.log(`  parm ${i}: val=${str.to_str(parm_val)}`), 
        parms_array.push(parm_val);
        TRACE_CALL && console.log(`-- before remote_call, history=${g_micro.length}`), 
        func_ptr.apply(null, parms_array), 
        TRACE_CALL && console.log(`-- after remote_call, history=${g_micro.length}`);
    } else console.log(`### function not listed in Publish, hash=${func_hash.toString(16)}`);
};

function find_function_hash(hash, list) {
    var i;
    for (i = 0; i < list.length; i++) if (TRACE_CALL && console.log(`  find_function_hash, targ=${hash.toString(16)}, f=${list[i].name}, hash=${list[i].hash.toString(16)}`), 
    hash == list[i].hash) return list[i].code;
    return null;
}

export function node_go() {
    g_timer_id = setInterval(node_tick, 1e3 / 60);
};
...
ENUMS[FIRST_ENUM + 3] = "px", 
ENUMS[9007194157790523] = "a_find", 
ENUMS[9007191603303728] = "find_start", 
ENUMS[9007192689371859] = "find_len", 
FIELDS[9007191603303728] = !0, 
FIELDS[9007192689371859] = !0, 
ENUMS[9007192534696881] = "a_find_repl", 
ENUMS[9007193956346111] = "find_patt", 
ENUMS[9007194250103617] = "repl_patt", 
FIELDS[9007193956346111] = !0, 
FIELDS[9007193956346111] = !0;
...
ENUMS[9007191792462830] = "F_in_sysobj", 
FIELDS[F_in_sysobj] = !0;
...
FIELDS[F_monitor_active] = !0;

export const F_monitor_target = 9007191836863486;

ENUMS[9007191836863486] = "monitor_target", 
FIELDS[9007191836863486] = !0;

export const F_mouse_inside = 9007191840940968;

ENUMS[9007191840940968] = "mouse_inside", 
FIELDS[F_mouse_inside] = !0;
...
ENUMS[9007192057739057] = "vv_funcx", 
FIELDS[F_vv_funcx] = !0;

export const F_vv_line1 = 9007191730799590;

ENUMS[9007191730799590] = "vv_line1", 
FIELDS[F_vv_line1] = !0;

export const F_vv_line2 = 9007191714021971;

ENUMS[9007191714021971] = "vv_line2", 
FIELDS[F_vv_line2] = !0;
...
function default_lgp(raw_msg) {
    var msg;
    if (msg = g_log_indent > 0 && "" != raw_msg ? str.str_repeat("\t", g_log_indent) + raw_msg : raw_msg, 
    TRACE_LOGG) {
        var elem = document.getElementById("$log");
        if (null != elem) {
            elem.style.marginLeft = "10px";
            var buf = elem.innerHTML;
            buf += str.safe_for_html(msg) + "<br>", 
            elem.innerHTML = buf, 
            elem.scrollBy(0, 1e3);
        }
    }
...
function dump_bytes(buf, len) {
    ...
    log(`-- dump of buffer, len=${len}, maxlen=${maxlen}`), 
...
function js_corner_to_str(list) {
    ...
    if ("number" == typeof list) ncorners = 1, 
    corner0 = list; else if (list instanceof a_tree) if (corners = [], 
    1 == (ncorners = count(list)) || 2 == ncorners || 4 == ncorners || 8 == ncorners) {
        for (var ix = 1; ix <= ncorners; ix++) corners.push(getn(list, ix));
        corner0 = corners[0];
    } else argument_err("bad number of corner values " + String(ncorners)), 
    ncorners = 0;
    switch (ncorners) {
      case 1:
...
function js_grad_to_str(node, opacity = 1) {
    ...
    for (;iter.next(); ) {
        ...
        1 != opacity && (stop_opacity *= opacity), 
        comma && (ss += ", "), 
        comma = !0, ss += `${js_color_to_str(stop_color, stop_opacity)} ${stop_pos.toFixed(2)}%`;
    }
    return ss + ")";
}

function js_shadow_ss(sptr) {
    return "grey 50px 50px";
}

function js_set_screen_size() {
    var pixel_ratio = 1;
    if (IS_NODE) USABLE_H = SCREEN_H = 1024, 
    USABLE_V = SCREEN_V = 768, 
    SCREEN_DPI = 96; else {
        pixel_ratio = window.devicePixelRatio;
        let s = window.screen;
        SCREEN_H = s.width, 
        SCREEN_V = s.height, 
        USABLE_H = window.innerWidth, 
        USABLE_V = window.innerHeight, 
        SCREEN_DPI = pixel_ratio > 1 ? 140 : 96, 
        TRACE_RESIZE && log(`screen=${SCREEN_H},${SCREEN_V}, usable=${USABLE_H},${USABLE_V}, ratio=${pixel_ratio}, DPI=${SCREEN_DPI}`);
        const list = [ {
            hh: 430,
            vv: 932,
            ratio: 3,
        ...
        for (var item of list) if ((SCREEN_H == item.hh && SCREEN_V == item.vv || SCREEN_H == item.vv && SCREEN_V == item.hh) && abs(item.ratio - pixel_ratio) < .01) {
            SCREEN_DPI = item.actual / pixel_ratio, 
            TRACE_RESIZE && log(`overriding DPI to ${SCREEN_DPI}`);
            break;
        }
    }
    setv(_M, 0, runtime, F_screen_horz, SCREEN_H), 
    setv(_M, 0, runtime, F_screen_vert, SCREEN_V), 
    setv(_M, 0, runtime, F_screen_dpi, SCREEN_DPI), 
    setv(_M, 0, runtime, F_pixel_ratio, pixel_ratio);
}
...
export function js_svg_wrapper(container, opacity) {
    ...
    return null != container.div && (content_h = container.div.content_h, 
    content_v = container.div.content_v), 
    wrapper.style.overflow = "visible", 
    "scroll" == container.style.overflowX && (content_h -= SCROLLBAR_WIDTH), 
    "scroll" == container.style.overflowY && (content_v -= SCROLLBAR_WIDTH), 
    wrapper.style.left = "0px", 
    wrapper.style.top = "0px", 
    wrapper.setAttribute("width", content_h.toFixed(0)), 
    wrapper.setAttribute("height", content_v.toFixed(0)), 
    1 != opacity && wrapper.setAttribute("opacity", opacity.toFixed(3)), 
    container.appendChild(wrapper), 
    wrapper;
};

export function input_start(mod, loc, field) {
    var elem, is_multiline = getn(field, F_in_multiline) == Y, kind = getn(field, F_in_kind);
    ...
    if (name == U_STR && (name = id), 
    elem.name = name, TRACE_INPUT && log(`input_start, label=${elem.id}`), 
    !is_multiline) switch (kind) {
      case IN_COLOR:
        elem.type = "color";
        break;
    ...
    null != keyb && (elem.inputMode = keyb), 
    elem.spellcheck = getn(field, F_in_spellcheck) == Y;
    var maxlen = getn(field, F_in_maxlen);
    maxlen == U && (maxlen = 80), 
    elem.maxLength = maxlen;
    var ss = gets(field, F_in_hint);
    null != ss && (elem.placeholder = ss), 
    getn(field, F_in_required) == Y && (elem.required = !0), 
    validate_input(field, ""), 
    setv(mod, loc, field, F_in_sysobj, elem);
};
...
export function input_set_value(mod, loc, field, newval) {
    ...
    null == elem ? CHECKS && internal_err("input was not started") : elem.value = newval, 
    validate_input(field, newval);
};
...
export function input_set_selection(id, options) {
    ...
    if (null != id) {
        ...
        if (null != dom) stop == U && (stop = 9999999), 
        dom.focus(), dom.setSelectionRange(start - 1, stop); else if (CHECKS) debugger;
    }
};
...
export function remote_call(connection, func_name, func_hash, parms) {
    ...
    for (null == gg_buf ? gg_buf = new_bytes(SEND_BUF_SIZE, LITTLE_ENDIAN) : gg_buf.position = 0, 
    bytes_put_u8(gg_buf, SOP_BATCH_START), 
    bytes_put_u8(gg_buf, 0), 
    fix1 = gg_buf.position, 
    bytes_put_u32(gg_buf, 0), 
    bytes_put_u8(gg_buf, SOP_CALL), 
    bytes_put_u32(gg_buf, func_hash), 
    nparms = parms.length, 
    bytes_put_u8(gg_buf, nparms), 
    TRACE_ENCODE && log(str.conv("=> remote call to {}, hash={}, nparms={}", func_name, str.to_str(func_hash, {
        base: 16
    }), nparms)), i = 0; i < nparms; i++) parmval = parms[i], 
    parmpos = gg_buf.position, 
    parmval instanceof a_path ? bytes_put_relpath(gg_buf, parmval, socket.my_client_dest) : bytes_put_val(gg_buf, parmval, 0), 
    TRACE_ENCODE && log(`  at offset ${parmpos}: parm[${i}]=${str.to_str(parmval)}`);
    bytes_put_u8(gg_buf, SOP_BATCH_END), 
    bytes_fixup_len(gg_buf, fix1);
    var ncum = getn(connection, F_client_bytes_out) + gg_buf.position;
    setv(_M, 0, connection, F_client_bytes_out, ncum), 
    TRACE_CHAN && log(str.conv("--sending {} bytes ({} cumulative) of remote call to func {}", gg_buf.position, ncum, func_name)), 
    websocket_send(socket, gg_buf, gg_buf.position);
};
...
export function websocket_send(sock, data, len = U) {
    if (data instanceof ByteArray) {
        ...
        TRACE_CHAN && log(str.conv("--sock.send binary len={}", len)), 
        sock.send(exactbuf);
    } else "string" == typeof data ? (TRACE_CHAN && log(str.conv("--sock.send string={}", data.substr(0, 20))), 
    sock.send(data)) : log(`### unknown type of data being sent: ${typeof data}`);
};

const CURSOR_ID = "$cursor";

export function cursor_set(curs) {
    if (setv(_M, 0, runtime, F_cursor_changed, Y), 
    curs != g_curr_cursor) {
        switch (g_curr_cursor = curs, 
        curs) {
          case CURS_ARROW:
...
export function numeric_arg(val) {
    return void 0 != val && "number" == typeof val && is_numeric(val) == Y ? val : (CHECKS && argument_err("bad numeric parameter"), 
    0);
};
...
function bytes_put_relpath(buf, argpath, basis) {
    ...
    for (TRACE_ENCODE && log(`bytes_put_relpath, offset=${buf.position}, nlevels=${nlevels}`), 
    bytes_put_u8(buf, NOV_RELPATH), 
    bytes_put_u8(buf, nlevels), 
    i = 0; i < nlevels; i++) TRACE_ENCODE && log(`  relpath level=${i}, offset=${buf.position}`), 
...
function bytes_get_relpath(buf, basis) {
    ...
    for (TRACE_DECODE && log(`bytes_get_relpath, nlevels=${nlevels}`), 
    i = 0; i < nlevels; i++) TRACE_DECODE && log(`  relpath level=${i}, offset=${buf.position}`), 
    subscript = bytes_get_val(buf), 
...
export function bytes_put_num(bytes, val) {
    ...
    bytes.position += 8, 
    bytes.position > bytes.length && (bytes.length = bytes.position);
};
...
export function bytes_put_bytes(bytes, src, len) {
    ...
    dst_view.set(src_view, 0), 
    bytes.position += len, 
    TRACE_ENCODE && log(`bytes_put_bytes, len=${len}, new pos=${bytes.position}`), 
    bytes.position > bytes.length && (bytes.length = bytes.position);
};
...
export function bytes_get_bytes(bytes) {
    ...
    return bytes.position += len, 
    ArrayBuffer_to_ByteArray(chunk, LITTLE_ENDIAN);
};

export function bytes_put_uv(buf, len) {
    len <= 120 ? bytes_put_u8(buf, len) : len < 65536 ? (bytes_put_u8(buf, 121), 
    bytes_put_u16(buf, len)) : len < TWO_24 ? (bytes_put_u8(buf, 122), 
    bytes_put_u8(buf, len / TWO_16), 
    bytes_put_u16(buf, len % TWO_16)) : len < TWO_32 ? (bytes_put_u8(buf, 123), 
    bytes_put_u32(buf, len)) : len < TWO_40 ? (bytes_put_u8(buf, 124), 
    bytes_put_u8(buf, len / TWO_32), 
    bytes_put_u32(buf, len % TWO_32)) : len < TWO_48 ? (bytes_put_u8(buf, 125), 
    bytes_put_u16(buf, len / TWO_32), 
    bytes_put_u32(buf, len % TWO_32)) : len < TWO_56 ? (bytes_put_u8(buf, 126), 
    bytes_put_u8(buf, len / TWO_48), 
    bytes_put_u16(buf, len / TWO_32 % TWO_16), 
    bytes_put_u32(buf, len % TWO_32)) : (bytes_put_u8(buf, 127), 
    bytes_put_u32(buf, len / TWO_32), 
    bytes_put_u32(buf, len % TWO_32));
};
...
export function bytes_get_uv(buf) {
    ...
    switch (b1) {
      ...
      case 122:
        return hi = bytes_get_u8(buf), 
        lo = bytes_get_u16(buf), 
        hi * TWO_16 + lo;
      ...
      case 124:
        return hi = bytes_get_u8(buf), 
        lo = bytes_get_u32(buf), 
        hi * TWO_32 + lo;

      case 125:
        return hi = bytes_get_u16(buf), 
        lo = bytes_get_u32(buf), 
        hi * TWO_32 + lo;

      case 126:
        return hi = bytes_get_u8(buf), 
        med = bytes_get_u16(buf), 
        lo = bytes_get_u32(buf), 
        hi * TWO_48 + med * TWO_32 + lo;

      case 127:
        return hi = bytes_get_u32(buf), 
        lo = bytes_get_u32(buf), 
        hi * TWO_32 + lo;
...
export function bytes_put_i24(buf, val) {
    bytes_put_u8(buf, val), 
    bytes_put_u8(buf, val >> 8), 
    bytes_put_u8(buf, val >> 16);
};

export function bytes_get_i24(buf) {
    var result, low = bytes_get_u8(buf), med = bytes_get_u8(buf), hig = bytes_get_u8(buf);
    return result = (hig << 16) + (med << 8) + low, 
    128 & hig && (result |= 4278190080), 
    result;
};
...
export function bytes_put_val(buf, val, flag) {
    if ("number" == typeof val) 0 == val ? (bytes_put_u8(buf, flag | NOV_0), 
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, ZERO", buf.position))) : val == Y ? (bytes_put_u8(buf, flag | NOV_Y), 
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, Y", buf.position))) : val == N ? (bytes_put_u8(buf, flag | NOV_N), 
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, N", buf.position))) : val == U ? (bytes_put_u8(buf, flag | NOV_U), 
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, U", buf.position))) : Number.isNaN(val) ? (bytes_put_u8(buf, flag | NOV_ERR), 
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, ERR", buf.position))) : (bytes_put_u8(buf, flag | NOV_NUM), 
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, num={}", buf.position, val)), 
    bytes_put_num(buf, val)); else if ("string" == typeof val) bytes_put_u8(buf, flag | NOV_STR), 
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, str={}", buf.position, val)), 
    bytes_put_str(buf, val); else if (val instanceof a_path) bytes_put_u8(buf, flag | NOV_PATH), 
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, path", buf.position)), 
    bytes_put_path(buf, val); else if (val instanceof a_function) {
        bytes_put_u8(buf, flag | NOV_FUNC), 
        TRACE_ENCODE && log(str.conv(" put_val, offset={}, func", buf.position)), 
        bytes_put_u32(buf, val.hash);
    } else if (val instanceof a_meas) bytes_put_u8(buf, flag | NOV_MEAS), 
    TRACE_ENCODE && log(str.conv(" put_val, offset={}, meas", buf.position)), 
    bytes_put_num(buf, val.mag), 
    bytes_put_num(buf, val.unit), 
    bytes_put_num(buf, val.family), 
    bytes_put_str(buf, val.unitss); else {
        if (val instanceof a_image) throw bytes_put_u8(buf, flag | NOV_IMAGE), 
        TRACE_ENCODE && log(str.conv(" put_val, offset={}, image", buf.position)), 
        new Error("a_image not yet");
        if (val instanceof a_sound) throw bytes_put_u8(buf, flag | NOV_SOUND), 
        TRACE_ENCODE && log(str.conv(" put_val, offset={}, sound", buf.position)), 
        new Error("a_sound not yet");
        if (!(val instanceof ByteArray)) throw bytes_put_u8(buf, flag | NOV_OBJECT), 
        new Error(`unsupported type ${typeof val}`);
        bytes_put_u8(buf, flag | NOV_BYTES), 
        TRACE_ENCODE && log(str.conv(" put_val, offset={}, bytes", buf.position)), 
        bytes_put_bytes(buf, val, val.position);
    }
};

export function bytes_put_path(buf, mypath) {
    let mykey;
    for (mykey of (mypath.base instanceof a_tree ? (TRACE_ENCODE && log(str.conv(" put_path, offset={}, hard hash={}", buf.position, mypath.base.sub)), 
    bytes_put_u32(buf, mypath.base.sub)) : (TRACE_ENCODE && log(str.conv(" put_path, offset={}, soft hash={}", buf.position, mypath.base)), 
    bytes_put_u32(buf, mypath.base)), 
    bytes_put_u8(buf, mypath.key.length), 
    mypath.key)) bytes_put_val(buf, mykey, 0);
};

export function bytes_get_path(buf) {
    let mypath = new a_path(), hash = bytes_get_u32(buf);
    if (mypath.base = TREES[hash], 
    void 0 == mypath.base) throw new Error("can't resolve path base");
...
function bytes_put_vtree(buf, subtree) {
    subtree instanceof a_path ? (bytes_put_u8(buf, SOP_TREE_BIN), 
    bytes_put_tree(buf, subtree)) : internal_err(`subtree passed to bytes_put_vtree is not a_path: ${subtree.constructor.name}`);
}

function bytes_put_subscript(buf, sub) {
    "string" == typeof sub ? (bytes_put_u8(buf, SUB_STR), 
    TRACE_ENCODE && log(str.conv("  at {} SUB_STR [{}]", buf.position, sub)), 
    bytes_put_str(buf, sub)) : 1 == sub ? (bytes_put_u8(buf, SUB_1), 
    TRACE_ENCODE && log(str.conv("  at {} SUB_1", buf.position))) : sub >= -TWO_7 && sub < TWO_7 ? (bytes_put_u8(buf, SUB_INT8), 
    TRACE_ENCODE && log(str.conv("  at {} SUB_INT8 [{}]", buf.position, sub)), 
    bytes_put_i8(buf, sub)) : sub >= -TWO_15 && sub < TWO_15 ? (bytes_put_u8(buf, SUB_INT16), 
    TRACE_ENCODE && log(str.conv("  at {} SUB_INT16 [{}]", buf.position, sub)), 
    bytes_put_i16(buf, sub)) : sub >= -TWO_23 && sub < TWO_23 ? (bytes_put_u8(buf, SUB_INT24), 
    TRACE_ENCODE && log(str.conv("  at {} SUB_INT24 [{}]", buf.position, sub)), 
    bytes_put_i24(buf, sub)) : sub >= -TWO_31 && sub < TWO_31 ? (bytes_put_u8(buf, SUB_INT32), 
    TRACE_ENCODE && log(str.conv("  at {} SUB_INT32 [{}]", buf.position, sub)), 
    bytes_put_i32(buf, sub)) : is_enum_b(sub) ? (bytes_put_u8(buf, SUB_ENUM), 
    TRACE_ENCODE && log(str.conv("  at {} SUB_ENUM [{}]", buf.position, sub)), 
    bytes_put_u32(buf, sub - FIRST_ENUM)) : (bytes_put_u8(buf, SUB_NUM), 
    TRACE_ENCODE && log(str.conv("  at {} SUB_NUM [{}]", buf.position, sub)), 
    bytes_put_num(buf, sub));
}

export function bytes_put_tree(buf, path) {
    bytes_put_tree2(buf, path_to_node(path)), 
    bytes_put_u8(buf, SOP_EOT);
};

function bytes_put_tree2(buf, node) {
    if (!(node instanceof a_tree)) return void log("warning: bytes_put_tree2 ignored as no data given");
    let flag = 0;
    if (0 == (node.eflags & EF_VAL_IS_U) && (null != node.children && (flag = NOPOP), 
    TRACE_ENCODE && log(str.conv("bytes_put_tree, val={}", node.val)), 
    bytes_put_val(buf, node.val, flag)), 
    null != node.children) {
        var child;
        for (var ix in node.children) bytes_put_subscript(buf, (child = node.children[ix]).sub), 
        bytes_put_tree2(buf, child);
        TRACE_ENCODE && log(str.conv(" end of children at {} opcode=POP", buf.position)), 
        bytes_put_u8(buf, SUB_POP);
    }
}

export function bytes_get_tree(buf, destp) {
    var opcode, sub_num, sub_str, skip_pop, val, dest = destp.clone();
    outer: for (;;) switch (skip_pop = 0 != ((opcode = bytes_get_u8(buf)) & NOPOP), 
    opcode &= OPCODE_MASK, 
    TRACE_DECODE && log(str.conv("get_tree at {} opcode={} skip={}", buf.position, opcode_ss(opcode), skip_pop)), 
    opcode) {
      case SUB_NUM:
        sub_num = bytes_get_num(buf), 
        dest.key.push(sub_num), 
        TRACE_DECODE && log(str.conv("  SUB_NUM [{}]", sub_num));
        break;

      case SUB_STR:
        sub_str = bytes_get_str(buf), 
        dest.key.push(sub_str), 
        TRACE_DECODE && log(str.conv("  SUB_STR [{}]", sub_str));
        break;

      case SUB_1:
        dest.key.push(1), sub_num = 1, 
        TRACE_DECODE && log("  SUB_1");
        break;

      case SUB_INT8:
        sub_num = bytes_get_i8(buf), 
        dest.key.push(sub_num), 
        TRACE_DECODE && log(str.conv("  SUB_INT8 [{}]", sub_num));
        break;

      case SUB_INT16:
        sub_num = bytes_get_i16(buf), 
        dest.key.push(sub_num), 
        TRACE_DECODE && log(str.conv("  SUB_INT16 [{}]", sub_num));
        break;

      case SUB_INT24:
        sub_num = bytes_get_i24(buf), 
        dest.key.push(sub_num), 
        TRACE_DECODE && log(str.conv("  SUB_INT24 [{}]", sub_num));
        break;

      case SUB_INT32:
        sub_num = bytes_get_i32(buf), 
        dest.key.push(sub_num), 
        TRACE_DECODE && log(str.conv("  SUB_INT32 [{}]", sub_num));
        break;

      case SUB_ENUM:
        sub_num = (val = bytes_get_u32(buf)) + FIRST_ENUM, 
        dest.key.push(sub_num), 
        TRACE_DECODE && log(str.conv("  SUB_ENUM [{}]", sub_num));
        break;
      ...
      case SOP_BATCH_END:
        TRACE_DECODE && log("  ERROR in stream, forgot to send SOP_EOT"), 
        buf.position -= 1;
        break outer;

      default:
        val = bytes_get_val2(buf, opcode), 
        TRACE_DECODE && log(str.conv("  storing value {}, dest={}", val, dest)), 
        setv(_M, 0, dest, val), 
        skip_pop || dest.key.pop();
    }
    TRACE_DECODE && log(str.conv("..end get_tree at {}", buf.position));
};

export function bytes_put_micro(buf, micro) {
    TRACE_ENCODE && log(str.conv("  put_micro at {} .kind {}", buf.position, micro.kind)), 
    bytes_put_u32(buf, micro.kind), 
    bytes_put_u32(buf, Math.round(100 * micro.when)), 
    bytes_put_uv(buf, micro.module), 
    bytes_put_uv(buf, micro.where), 
    micro.kind & OPS_THAT_USE_PATH && (TRACE_ENCODE && log(str.conv("  put_micro .path at {}", buf.position)), 
    bytes_put_path(buf, micro.path), 
    bytes_put_val(buf, micro.oldval, 0), 
    bytes_put_val(buf, micro.newval, 0));
};

export function bytes_get_micro(buf) {
    let micro = {};
    return TRACE_DECODE && log(str.conv("  get_micro at {} .kind", buf.position)), 
    micro.kind = bytes_get_u32(buf), 
    TRACE_DECODE && log(str.conv("     .kind={}", micro.kind)), 
    micro.when = bytes_get_u32(buf) / 100, 
    micro.module = bytes_get_uv(buf), 
    micro.where = bytes_get_uv(buf), 
    micro.kind & OPS_THAT_USE_PATH && (TRACE_DECODE && log(str.conv("  path={}", micro.path)), 
    micro.path = bytes_get_path(buf), 
    micro.oldval = bytes_get_val(buf), 
    micro.newval = bytes_get_val(buf)), 
    micro;
};
...
export function bytes_put_str(bytes, str) {
    ...
    for (bytes_put_uv(bytes, len8), 
    i = 0; i < len8; i++) bytes.view.setUint8(bytes.position, utf8[i]), 
    bytes.position += 1, 
    bytes.position > bytes.length && (bytes.length = bytes.position);
};
...
export function bytes_get_str(bytes) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (value < 128) str += String.fromCharCode(value); else if (value > 191 && value < 224) str += String.fromCharCode((31 & value) << 6 | 63 & bytes_get_u8(bytes)), 
        i += 1; else if (value > 223 && value < 240) str += String.fromCharCode((15 & value) << 12 | (63 & bytes_get_u8(bytes)) << 6 | 63 & bytes_get_u8(bytes)), 
        i += 2; else {
            var charCode = ((7 & value) << 18 | (63 & bytes_get_u8(bytes)) << 12 | (63 & bytes_get_u8(bytes)) << 6 | 63 & bytes_get_u8(bytes)) - 65536;
            str += String.fromCharCode(charCode >> 10 | 55296, 1023 & charCode | 56320), 
            i += 3;
        }
    }
    return str;
...
function bytes_get_val2(buf, valkind) {
    ...
    switch (TRACE_DECODE && log(str.conv("get_val at {}", buf.position)), 
    valkind) {
      case NOV_NUM:
      ...
      case NOV_FUNC:
        return val_hash = bytes_get_u32(buf), 
        null == (val_func = FUNCS[val_hash]) && CHECKS && internal_err("missing inverse for func"), 
        val_func;

      case NOV_MEAS:
        return (val_meas = new a_meas()).mag = bytes_get_num(buf), 
        val_meas.unit = bytes_get_num(buf), 
        val_meas.family = bytes_get_num(buf), 
        val_meas.unitss = bytes_get_str(buf), 
        val_meas;
...
export function set_ui_language(lang) {
    is_enum_b(lang) ? (setv(_M, 0, runtime, F_ui_language, lang), 
    refresh_all()) : internal_err("bad language selection");
};

export function bit_test(val, bitx) {
    return val == U ? U : val & 1 << (bitx -= 1) ? Y : N;
};
...
export function bit_set(mod, loc, valp, bitx, newval) {
    ...
    oldval == U && (oldval = 0);
    var bits = oldval >> 0;
    newval == Y ? bits |= 1 << bitx : bits &= ~(1 << bitx), 
    path_setv(mod, loc, valp, bits);
};

export function clock_tick() {
    now_raw = sys_rawtime(), 
    elapsed_raw = now_raw - time_launched_raw, 
    now = sys_dilatedime(), 
...
export function zap_clock() {
    let raw = sys_rawtime();
    time_launched_raw = raw, 
    time_launched = raw, 
    now_raw = raw, now = raw, 
    elapsed = 0, elapsed_raw = 0, 
    g_era_raw = raw, g_era_dilated = raw;
};
...
export function sys_dilatedime() {
    ...
    return TRACE_TIME && log(str.conv(">> now, now_raw={n3}, era_dilated={n3}, era_raw={n3}, scale={n3}, net={n3}", now_raw, g_era_dilated, g_era_raw, g_time_scale, net)), 
    net;
};

export function is_leap_year(year) {
    return is_err_enum_b(year) ? ERR : year == U ? U : year % 4 == 0 && year % 100 != 0 || year % 400 == 0 ? Y : N;
};
...
export function monitored_draw(b, options) {
    ...
    is_dead && (b.bflags |= BFLAG_DEAD), 
    b.bflags &= ~BFLAG_IDE, 
    b.bounds.width == g_old_mon_h && b.bounds.height == g_old_mon_v || (g_old_mon_h = b.bounds.width, 
    g_old_mon_v = b.bounds.height, 
    post_resize_event(b, monitored_draw_F.tracker)), 
    g_mon_root_block = b, 
    aaaa.main_drawer.code(b);
};
...
export const debug_stack_trace = function() {
    var obj = {};
    return Error.captureStackTrace(obj, debug_stack_trace), 
    obj.stack;
};
...
function follow_blocks(depth, b, p) {
    var l, t, w, h, curr;
    if (setv(_M, 0, p, F_bname, b.blabel), 
    setv(_M, 0, p, F_bkind, b.bkind), 
    setv(_M, 0, p, F_bdepth, depth), 
    b.bkind == BKIND_GRID_BASE && (setv(_M, 0, p, F_ncells, F_x, b.div.ncols), 
    setv(_M, 0, p, F_ncells, F_y, b.div.nrows)), 
    null != b.drawer.tracker && setv(_M, 0, p, F_btracks, Y), 
    w = parseFloat(b.style.width), 
    h = parseFloat(b.style.height), 
    b == g_mon_root_block) l = 0, 
    t = 0; else for (l = parseFloat(b.style.left), 
    t = parseFloat(b.style.top), 
    curr = b; (curr = curr.parentNode) != g_mon_root_block && null != curr; ) l += parseFloat(curr.style.left), 
    t += parseFloat(curr.style.top);
    setv(_M, 0, p, F_bbox, F_left, l), 
    setv(_M, 0, p, F_bbox, F_top, t), 
    setv(_M, 0, p, F_bbox, F_width, w), 
    setv(_M, 0, p, F_bbox, F_height, h);
...
export function debug_get_blocks(list) {
    if (null != g_mon_root_block) {
        ...
        new_seq > g_last_blocklist_seq && (trunc_tree(_M, 0, list), 
        follow_blocks(0, g_mon_root_block, list), 
        g_last_blocklist_seq = new_seq);
    }
};
...
export function debug_blackbox_send(url, port, id, filename, options) {
    ...
    subscribe_start(_M, 0, bbox_conn, url, port, id, U, null, {
        handler: function bbox_send_progress(conn) {
            ...
            switch (status) {
              case CSTATUS_CONNECTED:
                ...
                bytes_put_u8(buf, SOP_BATCH_START), 
                bytes_put_u8(buf, 0);
                var fixpos1 = buf.position;
                bytes_put_u32(buf, 0), 
                bytes_put_u8(buf, SOP_BBOX), 
                bytes_put_str(buf, filename);
                void 0 != options.mail_addr && (mail_subj = options.mail_addr);
                var mail_subj = "";
                void 0 != options.mail_subj && (mail_subj = options.mail_subj);
                var mail_body = "";
                void 0 != options.mail_body && (mail_body = options.mail_body), 
                bytes_put_str(buf, ""), 
                bytes_put_str(buf, mail_subj), 
                bytes_put_str(buf, mail_body);
                var fixpos2 = buf.position;
                bytes_put_u32(buf, 0), 
                debug_blackbox_encode(buf, options), 
                bytes_fixup_len(buf, fixpos2), 
                bytes_put_u8(buf, SOP_BATCH_END), 
                bytes_fixup_len(buf, fixpos1);
...
export function debug_blackbox_write(mypath, options) {
    var buf = new_bytes(SEND_BUF_SIZE, LITTLE_ENDIAN);
    debug_blackbox_encode(buf, options), 
    TRACE_BLACKBOX && log(str.conv("-- writing blackbox file to {}", mypath)), 
    os_write_bytes(mypath, buf);
};
...
export function debug_blackbox_encode(buf, options) {
    ...
    for (ss of (bytes_put_u16(buf, 1), 
    bytes_put_str(buf, gets(runtime, F_app_name)), 
    bytes_put_str(buf, gets(runtime, F_app_version)), 
    bytes_put_num(buf, getn(runtime, F_os_kind)), 
    bytes_put_num(buf, getn(runtime, F_os_language)), 
    bytes_put_str(buf, gets(runtime, F_os_version)), 
    bytes_put_u16(buf, getn(runtime, F_screen_dpi)), 
    bytes_put_u32(buf, getn(runtime, F_window_horz)), 
    bytes_put_u32(buf, getn(runtime, F_window_vert)), 
    bytes_put_num(buf, time_launched), 
    bytes_put_str(buf, kind), 
    bytes_put_str(buf, note), 
    bytes_put_u32(buf, getn(runtime, F_major_firstx)), 
    bytes_put_uv(buf, MODULES.length), 
    MODULES)) bytes_put_str(buf, ss);
    bytes_fixup_len(buf, fix1), 
    TRACE_BLACKBOX && log(str.conv("  blackbox had {} major steps", count(adr(runtime, F_major_steps))));
    var fix2 = bytes_put_chunk(buf, "majr");
    bytes_put_vtree(buf, adr(runtime, F_major_steps)), 
    bytes_fixup_len(buf, fix2);
    var fix3 = bytes_put_chunk(buf, "micr");
    for (micro of (bytes_put_uv(buf, g_micro.length), 
    g_micro)) bytes_put_micro(buf, micro);
    bytes_put_u8(buf, SOP_EOT), 
    bytes_fixup_len(buf, fix3);
};

export function debug_blackbox_read(fileobj, callback) {
    g_old_mon_h = 0, os_read_bytes(fileobj, new a_function(_M, "await_bbox_read", function await_bbox_read(buf) {
        ...
        setv(_M, 0, blackbox_runtime, F_app_name, app_name), 
        setv(_M, 0, blackbox_runtime, F_app_version, app_ver), 
        setv(_M, 0, blackbox_runtime, F_os_kind, os_kind), 
        setv(_M, 0, blackbox_runtime, F_os_language, os_lang), 
        setv(_M, 0, blackbox_runtime, F_os_version, os_ver), 
        setv(_M, 0, blackbox_runtime, F_screen_dpi, screen_dpi), 
        setv(_M, 0, blackbox_runtime, F_window_horz, window_h), 
        setv(_M, 0, blackbox_runtime, F_window_vert, window_v), 
        setv(_M, 0, blackbox_runtime, F_debug_kind, kind), 
        setv(_M, 0, blackbox_runtime, F_debug_note, note), 
        setv(_M, 0, blackbox_runtime, F_screen_horz, window_h), 
        setv(_M, 0, blackbox_runtime, F_screen_vert, window_v);
        var i, nmods = bytes_get_uv(buf);
        for (BBOX_MODULES = [], 
        i = 0; i < nmods; i++) BBOX_MODULES.push(bytes_get_str(buf));
        if ("majr" != bytes_get_chunk(buf).name) return void callback.code(-2);
        if (trunc_tree(_M, 0, adr(blackbox_runtime, F_major_steps)), 
        bytes_get_u8(buf) != SOP_TREE_BIN) throw new Error("bad opcode");
        bytes_get_tree(buf, adr(blackbox_runtime, F_major_steps)), 
        TRACE_BLACKBOX && log(str.conv("  read of {} major steps", count(adr(blackbox_runtime, F_major_steps))));
        if (g_micro.length = 0, 
        "micr" != bytes_get_chunk(buf).name) return void callback.code(-2);
        var micro, dead_nmicro = bytes_get_uv(buf);
        for (g_blackbox_micro = [], 
        i = 0; i < dead_nmicro; i++) micro = bytes_get_micro(buf), 
        g_blackbox_micro.push(micro);
        ...
        setv(_M, 0, blackbox_runtime, F_major_stepx, dead_nmajor), 
        setv(_M, 0, blackbox_runtime, F_major_firstx, firstx), 
        timer_clear_all(), time_launched = dead_launched, 
        elapsed = micro.when, 
        g_time_scale = 0, g_era_dilated = now = dead_launched + elapsed, 
        g_era_raw = now_raw, 
        do_micro_fwd(0, dead_nmicro, g_blackbox_micro), 
        callback.code(Y);
    }));
};
...
export function subscribe_start(mod, loc, connection, arg_url, arg_port, arg_id, arg_dest, _options) {
    ...
    "" != extra && (net += "/" + encodeURIComponent(extra)), 
    TRACE_CHAN && log(`>> subscribing to ${net}`);
    var sock = new WebSocket(net);
    sock.binaryType = "arraybuffer", 
    sock.my_client_dest = arg_dest, 
    sock.my_id = arg_id, 
    sock.my_first = !0, 
    ...
    setv(mod, loc, connection, F_client_status, CSTATUS_TRYING), 
    null != handler && handler(connection, CSTATUS_TRYING), 
    sock.onopen = function(evt) {
        TRACE_CHAN && log(str.conv(">> on_open, id={}", arg_id)), 
        setv(mod, loc, connection, F_client_status, CSTATUS_CONNECTED), 
        sock.timerID = setInterval(socket_keepalive, 6e4, sock), 
        null != handler && handler(connection, CSTATUS_CONNECTED);
    }, sock.onclose = function(evt) {
        TRACE_CHAN && log(str.conv(">> on_close, id={}", arg_id)), 
        clearInterval(sock.timerID), 
        getn(connection, F_client_status) != CSTATUS_NO_ANSWER && (setv(mod, loc, connection, F_client_status, CSTATUS_CLOSED), 
        null != handler && handler(connection, CSTATUS_CLOSED));
    }, sock.onerror = function(evt) {
        TRACE_CHAN && log(str.conv(">> on_error, id={}", arg_id)), 
        setv(mod, loc, connection, F_client_status, CSTATUS_NO_ANSWER), 
        null != handler && handler(connection, CSTATUS_NO_ANSWER);
    }, sock.onmessage = function(evt) {
        TRACE_CHAN && log(str.conv(">> on_message, id={}, len={}", arg_id, evt.data.byteLength));
        var ignore = !1;
        (sock.my_first && (sock.my_first = !1, 
        ignore = arraybuf_begins("HTTP/1.1 101 ", evt.data)), 
        ignore) || (process_client_batches(ArrayBuffer_to_ByteArray(evt.data, LITTLE_ENDIAN), arg_dest), 
        null != handler && handler(connection, CSTATUS_DATA));
    };
};
...
function socket_keepalive(socket) {
    ...
    bytes_put_u32(buf, 0), 
    bytes_put_u8(buf, SOP_NOP), 
    bytes_put_u8(buf, SOP_BATCH_END), 
    bytes_fixup_len(buf, fix1), 
    TRACE_CHAN && log(`-- ${now} sending keep alive, ${buf.position} bytes`), 
    websocket_send(socket, buf, buf.position);
}

export function subscribe_stop(connection) {
    var sock = get_object(connection, F_client_channel);
    setv(_M, 0, connection, F_client_status, CSTATUS_DISCONNECTING), 
    TRACE_CHAN && log(str.conv(">> subscribe_stop, id={}", sock.my_id)), 
    sock.close(1e3, "subscribe_stop");
};
...
function process_client_batches(mybuf, dest) {
    ...
    if ((opcode = bytes_get_u8(mybuf)) == SOP_BATCH_START) {
        ...
        if (len > mybuf.arraybuf.byteLength) {
            ...
            bigloop: for (;;) switch (pos = mybuf.position, 
            opcode = bytes_get_u8(mybuf), 
            TRACE_DECODE && log(`at ${pos}: ${opcode_ss(opcode)}`), 
            opcode) {
              case SOP_NOP:
                break;
              ...
              case SOP_CALL:
                bytes_get_u32(mybuf);
                for (var val, nparms = bytes_get_u8(mybuf), parmvals = [], i = 0; i < nparms; i++) val = bytes_get_val(mybuf), 
                parmvals.push(val);
                break;
...
export function http_request(mod, loc, url, callback, _options) {
    ...
    if (null == response && argument_err("response field is required"), 
    setv(mod, loc, response, F_http_status, 0), 
    setv(mod, loc, response, F_http_phase, HTTP_START), 
    "http" != url.substr(0, 4) && (url = "http://" + url), 
    80 != port && (url += ":" + port), 
    nocache) switch (method) {
      case "GET":
      case "HEAD":
    ...
    request.timeout = 1e3 * timeout, 
    request.onerror = function on_error(evt) {
        TRACE_CHAN && log(`### on_error, id=${id}, url=${url}, ready=${request.readyState}, status=${request.status}`);
        setv(mod, loc, response, F_http_phase, HTTP_DONE), 
        setv(mod, loc, response, F_http_status, 400), 
        called || (called = !0, 
        unfreeze_calc(callback));
    }, request.onabort = function on_abort(evt) {
        TRACE_CHAN && log(`### on_abort, id=${id}, url=${url}, ready=${request.readyState}, status=${request.status}`);
        setv(mod, loc, response, F_http_phase, HTTP_DONE), 
        setv(mod, loc, response, F_http_status, 400), 
        called || (called = !0, 
        unfreeze_calc(callback));
    }, request.ontimeout = function on_timeout(evt) {
        TRACE_CHAN && log(`### on_timeout, id=${id}, url=${url}, ready=${request.readyState}, status=${request.status}`);
        setv(mod, loc, response, F_http_phase, HTTP_TIMEOUT), 
        setv(mod, loc, response, F_http_status, 408), 
        called || (called = !0, 
        unfreeze_calc(callback));
    }, request.onreadystatechange = function on_change(evt) {
        TRACE_CHAN && log(`### on_change, id=${id}, url=${url}, ready=${request.readyState}, status=${request.status}`);
        switch (request.readyState) {
          case 2:
            setv(mod, loc, response, F_http_phase, HTTP_HEADERS), 
            setv(mod, loc, response, F_http_headers, request.getAllResponseHeaders());
            break;
          ...
          case 4:
            switch (setv(mod, loc, response, F_http_phase, HTTP_DONE), 
            setv(mod, loc, response, F_http_status, request.status), 
            ...
            "" != request.responseText && setv(mod, loc, response, F_http_body, request.responseText), 
            TRACE_CHAN && log(`### on_change response=${request.responseText.substring(0, 40)}`), 
            0 == request.status || called || (called = !0, 
            unfreeze_calc(callback));
        }
...
export function browser_nav_vis(vis) {
    OS_KIND == OS_AND && (vis == Y ? document.exitFullscreen() : document.documentElement.requestFullscreen({
        navigationUI: "hide"
    }));
};

export function browser_has_bluetooth(flagptr) {
    if (console.log("browser_has_bluetooth start"), 
    void 0 == navigator.bluetooth) return console.log(">> Browser no Bluetooth!"), 
    setv(_M, 0, flagptr, Y), 
    void setv(_M, 0, flagptr, ERR);
    navigator.bluetooth.getAvailability().then(available => {
        available ? (console.log(">> This browser supports Bluetooth!"), 
        setv(_M, 0, flagptr, Y)) : (console.log(">> Bluetooth is not supported"), 
        setv(_M, 0, flagptr, N));
    });
};
...
export function browser_bluetooth_getc(service_ss, char_ss, finish_func) {
    trunc_tree(_M, 0, adr(g_bluetooth)), 
    setv(_M, 0, g_bluetooth, -1, 1), 
    navigator.bluetooth.requestDevice({
        filters: [ {
            services: [ "battery_service" ]
        } ]
    }).then(device => (log(">> Connecting to GATT Server..."), 
    device.gatt.connect())).then(server => (setv(_M, 0, g_bluetooth, -1, 2), 
    log(`>> Getting Service ${service_ss}...`), 
    server.getPrimaryService(service_ss))).then(service => (setv(_M, 0, g_bluetooth, -1, 3), 
    log(">> Getting Characteristic ${char_ss}..."), 
    service.getCharacteristic(char_ss))).then(characteristic => (setv(_M, 0, g_bluetooth, -1, 4), 
    log(">> Reading value..."), 
    characteristic.readValue())).then(value => {
        setv(_M, 0, g_bluetooth, -1, 5), 
        setv(_M, 0, g_bluetooth, 0, Y);
        let batteryLevel = value.getUint8(0);
        log(">> Battery Level is " + batteryLevel + "%"), 
        finish_func.code(g_bluetooth);
    }).catch(error => {
        setv(_M, 0, g_bluetooth, 0, N), 
...
export function quit(errcode = 0) {
    if (log(`inside quit, errorcode=${errcode}`), 
    IS_NODE) debugger;
    window.open("", "_parent", ""), 
    window.close();
};
...
export function debug_where_to_name(targ_mod, targ_line) {
    ...
    for (;probe = (lo + hi) / 2, 
    !(hi - lo < 2); ) targ_line < node.children[probe].sub ? hi = probe : lo = probe;
...
export function debug_jump(targ_major, is_blackbox) {
    if (is_numeric(targ_major) == Y) {
        ...
        is_blackbox == Y ? (curr_micros = g_blackbox_micro, 
        curr_runtime = blackbox_runtime) : (curr_micros = g_micro, 
        curr_runtime = runtime);
        var nmicro = curr_micros.length, nmajor = count(adr(curr_runtime, F_major_steps));
        if (targ_raw = (targ_major = clamp(1, Math.round(targ_major), nmajor + 1)) > nmajor ? nmicro : getn(curr_runtime, F_major_steps, targ_major, F_hist_rawx), 
        curr_raw = (curr_stepx = getn(curr_runtime, F_major_stepx)) > nmajor ? nmicro : getn(curr_runtime, F_major_steps, curr_stepx, F_hist_rawx), 
        TRACE_JUMP && log(str.conv("-- debug_jump, major_step={}, targ_raw={}, curr_raw={}", targ_major, targ_raw, curr_raw)), 
        curr_raw < targ_raw) do_micro_fwd(curr_raw, targ_raw, curr_micros); else if (curr_raw > targ_raw) for (;curr_raw > targ_raw; ) (micro = curr_micros[curr_raw -= 1]).kind == LOGK_DELTA && (fix_value(micro.path, micro.oldval), 
...
function do_micro_fwd(curr_raw, targ_raw, micros) {
    for (var micro; curr_raw < targ_raw; ) (micro = micros[curr_raw]).kind == LOGK_DELTA && fix_value(micro.path, micro.newval), 
    curr_raw += 1;
}
...
export function debug_get_micro(targ_major) {
    ...
    if (targ_major = clamp(1, Math.round(targ_major), nmajor + 1), 
    trunc_tree(_M, 0, adr(runtime, F_micro_steps)), 
    targ_major == U) start_raw = 1, 
    end_raw = nmicro; else {
        if (targ_major > nmajor) return;
        targ_major = max(targ_major, 1), 
        start_raw = getn(runtime, F_major_steps, targ_major, F_hist_rawx), 
        end_raw = targ_major + 1 > nmajor ? nmicro : getn(runtime, F_major_steps, targ_major + 1, F_hist_rawx) - 1;
    }
    end_raw -= 1;
    let skipflag = !1;
    for (rx = start_raw -= 1; rx <= end_raw; rx++) {
        ...
        0 == kind || skipflag || (setv(_M, 0, runtime, F_micro_steps, ix = rx + 1, F_ms_kind, kind), 
        setv(_M, 0, runtime, F_micro_steps, ix, F_ms_module, g_micro[rx].mod), 
        setv(_M, 0, runtime, F_micro_steps, ix, F_ms_where, g_micro[rx].where), 
        funcname = debug_where_to_name(g_micro[rx].mod, g_micro[rx].where), 
        setv(_M, 0, runtime, F_micro_steps, ix, F_ms_funcname, funcname), 
        setv(_M, 0, runtime, F_micro_steps, ix, F_ms_dest, g_micro[rx].path), 
        setv(_M, 0, runtime, F_micro_steps, ix, F_ms_oldval, g_micro[rx].oldval), 
        setv(_M, 0, runtime, F_micro_steps, ix, F_ms_newval, g_micro[rx].newval));
    }
};
...
export function debug_clear_future() {
    ...
    node.children.length > major_sub && (micro_end = getn(runtime, F_major_steps, major_end, F_hist_rawx), 
    node.children.length = major_sub, 
    node.seq = ++g_currseq, 
    g_micro.length = micro_end, 
    TRACE_JUMP && log(str.conv(">> debug_clear_future, major_sub={} micro_end={}", major_sub, micro_end)));
};
...
export function set_auto_size(myblock, width, height) {
    ...
    setv(_M, 0, myblock.extra, F_box, F_width, width), 
    setv(_M, 0, myblock.extra, F_box, F_height, height);
};
...
export function val_of(modname, varname) {
    ...
    return result instanceof a_tree && (result = adr(result)), 
    void 0 == result ? U : result;
};
...
export function sound_play(snd, _options = null) {
    ...
    for (var myaudio, tryx = startx; ;) {
        ...
        if (TRACE_SOUND && log(`-- slot ix ${tryx} is busy`), 
        (tryx += 1) >= snd.numx && (tryx = 0), 
        tryx == startx) {
            (myaudio = myaudio.cloneNode()).currentTime = 0, 
            snd.audio.push(myaudio), 
            snd.curx = snd.numx, 
            snd.numx += 1, TRACE_SOUND && log(`-- wrapped around, added copy, num=${snd.numx}`);
            break;
        }
    }
    0 != offset && (myaudio.currentTime = offset), 
    snd.loopf && (myaudio.loop = !0), 
    myaudio.play(), TRACE_SOUND && log("playing sound");
};
...
export function lg(n, options) {
    let base = E;
    return is_err_enum_b(n) || n < 0 ? ERR : n === U ? U : null !== options && options.hasOwnProperty("base") ? (base = options.base, 
    Number.isNaN(base) || base < 0 ? ERR : base === U ? U : Math.log(n) / Math.log(base)) : Math.log(n);
};
...
export function to_meas(mag, unit) {
    let familyx;
    return (familyx = getn(unit_to_family, unit)) === U ? (halt(), 
    meas(ERR, N_each, Y_Scalar)) : meas(mag, unit, familyx);
};
...
function unit_to_pixels(block, meas, unit, roundf = !1) {
    ...
    unit) {
      ...
      case pt:
        result = pt_to_dots(block, meas), 
        roundf && (result = Math.round(result));
        break;
...
export function interpolate(a_cur, a_min, a_max, b_min, b_max, _options = null) {
    ...
    if (null !== _options && (_options.hasOwnProperty("round") && (rounded = _options.round), 
    _options.hasOwnProperty("clamp") && (clamped = _options.clamp)), 
    is_err_enum_b(a_cur) || is_err_enum_b(a_min) || is_err_enum_b(a_max) || is_err_enum_b(b_min) || is_err_enum_b(b_max)) return ERR;
...
export function solve_point(options) {
    ...
    for (let property in options) switch (property) {
      case "basis":
        options.basis instanceof Rectangle ? (basis_l = (basis = options.basis).left, 
        basis_t = basis.top, 
        basis_w = basis.width, 
        basis_h = basis.height) : options.basis instanceof a_path || options.basis instanceof a_tree ? (basis_l = getn(basis = options.basis, F_left), 
        basis_t = getn(basis, F_top), 
        basis_w = getn(basis, F_width), 
        basis_h = getn(basis, F_height)) : argument_err("basis must be a a_path or a_rect");
        break;
    ...
    switch (null === basis && argument_err("missing basis"), 
    is_numeric(basis_l) == Y && is_numeric(basis_t) == Y && is_numeric(basis_w) == Y && is_numeric(basis_h) == Y || argument_err("bad basis"), 
    (pin - 1) % 3) {
      case 0:
        x = basis_l;
        break;
...
export function solve_rect(options) {
    ...
    for (let property in options) switch (property) {
      case "basis":
        ...
        basis_t = getn(basis, F_top), 
        basis_w = getn(basis, F_width), 
        basis_h = getn(basis, F_height)) : argument_err("basis must be a path or tree");
        break;
      ...
      case "corner1":
        left = getn(options.corner1, F_x), 
        top = getn(options.corner1, F_y), 
        corner1_specified = !0;
        break;

      case "corner2":
        right = getn(options.corner2, F_x), 
        bottom = getn(options.corner2, F_y), 
        corner2_specified = !0;
        break;
    ...
    corner1_specified && corner2_specified && (left > right && (temp = left, 
    left = right, right = temp), 
    top > bottom && (temp = top, 
    top = bottom, bottom = temp), 
    horz_specified = !0, 
    vert_specified = !0);
    if (null !== basis) {
        if (dx !== U && (basis_l += dx), 
        dy !== U && (basis_t += dy), 
        inset !== U && (basis_l += inset, 
        basis_t += inset, basis_w -= 2 * inset, 
        basis_h -= 2 * inset), 
        inset_y !== U && (basis_t += inset_y, 
        basis_h -= 2 * inset_y), 
        inset_x !== U && (basis_l += inset_x, 
        basis_w -= 2 * inset_x), 
        inset_n !== U && (basis_t += inset_n, 
        basis_h -= inset_n), 
        inset_s !== U && (basis_h -= inset_s), 
        inset_e !== U && (basis_w -= inset_e), 
        inset_w !== U && (basis_l += inset_w, 
        basis_w -= inset_w), 
        !horz_specified) switch ((pin - 1) % 3) {
          case 0:
            left = basis_l;
            break;
    ...
    CHECKS && error && argument_err("insufficient info to determine rectangle"), 
    round && (left = Math.round(left), 
    top = Math.round(top), 
    width = Math.round(width), 
    height = Math.round(height)), 
    rect_as_tree(left, top, width, height);
};
...
function calc_slice_edge(orient, sizing, targix, gap_fract, slices) {
    ...
    for (drawx = 0, cum_gap = 0, 
    sx = 0; ;) if (slice = slices[sx], 
    sx += 1, null === slice.drawer) cum_gap += orient === HORZ ? slice.box.width : slice.box.height; else {
        if ((drawx += 1) === targix) {
    ...
    if (sizing === SMALLER) edge -= cum_gap * gap_fract; else {
        for (cum_gap = 0; !(sx >= slices.length) && (slice = slices[sx], 
        sx += 1, null === slice.drawer); ) cum_gap += orient === HORZ ? slice.box.width : slice.box.height;
...
export function solve_grid_dim(ncells, avail, cellwidth, gapwidth, indent) {
    ...
    for (avail -= 2 * indent + cellwidth; avail >= celltot; ) ncols += 1, 
    avail -= celltot;
    let nrows = round_up(ncells / ncols);
    for (;(ncols - 1) * nrows >= ncells; ) ncols -= 1;
    let result = new a_tree(_M, "$solve_grid_dim");
    return setv(_M, 0, result, F_x, ncols), 
    setv(_M, 0, result, F_y, nrows), 
    result;
};
...
export function solve_grid_rect(b, col1, row1, col2, row2, _options = null) {
    ...
    let spec = null;
    if (null !== b && null !== b.div && null != b.div.grid_func && (spec = b.div), 
    null === spec || is_err_enum_b(col1) || is_err_enum_b(row1) || is_err_enum_b(col2) || is_err_enum_b(row2)) return CHECKS && argument_err("bad arguments to function"), 
    ERR_TREE;
    ...
    return icol2 = col2, 
    irow1 = row1, irow2 = row2, 
    (icol1 = col1) < 1 && (icol1 = 1), 
    icol2 < 1 && (icol2 = 1), 
    irow1 < 1 && (irow1 = 1), 
    irow2 < 1 && (irow2 = 1), 
    icol1 > ncols && (icol1 = ncols), 
    icol2 > ncols && (icol2 = ncols), 
    irow1 > nrows && (irow1 = nrows), 
    irow2 > nrows && (irow2 = nrows), 
    rect_as_tree(left = calc_slice_edge(HORZ, SMALLER, icol1, gap, spec.horz_slices), top = calc_slice_edge(VERT, SMALLER, irow1, gap, spec.vert_slices), (right = calc_slice_edge(HORZ, BIGGER, icol2, gap, spec.horz_slices)) - left, (bottom = calc_slice_edge(VERT, BIGGER, irow2, gap, spec.vert_slices)) - top);
};
...
export function mod(input, divisor, options = null) {
    ...
    if (one_flag) {
        if (neg_flag && argument_err("neg_flag and one_flag together not yet implemented"), 
        input <= 0) {
            return divisor - -input % divisor;
        }
        return 1 + (input - 1) % divisor;
    }
    return result1 = input % divisor, 
    neg_flag ? (result2 = result1 < 0 ? result1 + divisor : result1 - divisor, 
    Math.abs(result1) < Math.abs(result2) ? result1 : result2) : (result1 < 0 && (result1 += divisor), 
    result1);
};
...
export function idiv(input, divisor, options) {
    ...
    if (0 === input) return 0;
    if ((divisor = Math.floor(divisor)) <= 0) return ERR;
    if (neg = !1, input < 0 && (neg = !0, 
    input = -input), null !== options) for (let property in options) switch (property) {
      case "one":
        one_flag = options.one === Y;
        break;
    ...
    return result = one_flag ? 1 + Math.floor((input - 1) / divisor) : Math.floor(input / divisor), 
    neg && (result = -result), 
    result;
};
...
export function cos(n) {
    let radians;
    return radians = n.mag, 
    Number.isNaN(radians) || radians >= FIRST_ENUM && radians <= LAST_ENUM ? ERR : radians === U ? U : Math.cos(radians);
};

export function sin(n) {
    let radians;
    return radians = n.mag, 
    Number.isNaN(radians) || radians >= FIRST_ENUM && radians <= LAST_ENUM ? ERR : radians === U ? U : Math.sin(radians);
};

export function tan(n) {
    let radians;
    return radians = n.mag, 
    Number.isNaN(radians) || radians >= FIRST_ENUM && radians <= LAST_ENUM ? ERR : radians === U ? U : Math.tan(radians);
};
...
export function arc_tan(n) {
    let mag;
    return Number.isNaN(n) || n >= FIRST_ENUM && n <= LAST_ENUM ? mag = ERR : n === U ? mag = U : (mag = Math.atan(n)) < 0 && (mag += TAU), 
...
export function perlin_noise(x) {
    ...
    for (let o = 0; o < perlin_octaves; o++) {
        ...
        n1 += ryf * ((n2 += rxf * (perlin[of + PERLIN_YWRAP + 1 & PERLIN_SIZE] - n2)) - n1), 
        n2 = perlin[(of += PERLIN_ZWRAP) & PERLIN_SIZE], 
        n2 += rxf * (perlin[of + 1 & PERLIN_SIZE] - n2), 
        n3 = perlin[of + PERLIN_YWRAP & PERLIN_SIZE], 
        n2 += ryf * ((n3 += rxf * (perlin[of + PERLIN_YWRAP + 1 & PERLIN_SIZE] - n3)) - n2), 
        r += (n1 += scaled_cosine(zf) * (n2 - n1)) * ampl, 
        ampl *= perlin_attenuation, 
        xi <<= 1, yi <<= 1, 
        yf *= 2, zi <<= 1, zf *= 2, 
        (xf *= 2) >= 1 && (xi++, 
        xf--), yf >= 1 && (yi++, 
...
export function set_clock(targ_sec) {
    ...
    TRACE_TIME && log(str.conv(" >> set_clock, targ={n3}, now={n3}, delta={n3}, old_dilated={n3}", targ_sec, now, delta, g_era_dilated)), 
    g_era_dilated += delta, 
    loom_delta_clock(delta);
};

export function set_clock_scale(newscale) {
    TRACE_TIME && log(str.conv(" >> set_clock_scale, {n3}", newscale)), 
    newscale != g_time_scale && (g_time_scale = newscale, 
    g_era_dilated = now, 
    g_era_raw = now_raw);
};
...
function align_to_ss(align) {
    switch (align) {
      ...
      default:
        return argument_err("bad alignment: " + str.to_str(align)), 
        null;
    }
}
...
export function scroll_get_limits(id) {
    ...
    "main_draw" == id && (id = ROOT_BLABEL);
    var targ = document.getElementById(id);
    return null != targ ? (setv(_M, 0, result, F_x, targ.scrollWidth - targ.clientWidth), 
    setv(_M, 0, result, F_y, targ.scrollHeight - targ.clientHeight)) : CHECKS && console.log(`>> can't find ${id}`), 
    result;
};
...
export function scroll_get_pos(id) {
    ...
    "main_draw" == id && (id = ROOT_BLABEL);
    var targ = document.getElementById(id);
    return null != targ ? targ.nesting > 1 ? (setv(_M, 0, result, F_x, targ.old_scrollh_pos), 
    setv(_M, 0, result, F_y, targ.old_scrollv_pos)) : (setv(_M, 0, result, F_x, targ.scrollLeft), 
    setv(_M, 0, result, F_y, targ.scrollTop)) : CHECKS && console.log(`>> can't find ${id}`), 
    result;
};
...
export function css_append(css) {
    null != g_style && g_style.parentNode.removeChild(g_style), 
    (g_style = document.createElement("style")).innerHTML = css, 
...
export function os_write_image(filename, data, img, options) {
    ...
    temp.href = url, document.body.appendChild(temp), 
    temp.click(), document.body.removeChild(temp);
};
...
export function os_pick_files(title, callback, options) {
    var extra = null;
    (os_input = document.createElement("input")).type = "file", 
    os_input.value = null;
    for (let property in options) switch (property) {
      case "extra":
        extra = options.extra;
    ...
    os_input.onchange = (e => {
        ...
        for (let f of e.target.files) setv(_M, 0, list, ix, f), 
        ix += 1;
...
export function timer_start(func, options) {
    ...
    return elem.nreps < 1 ? U : (elem.func = func, 
    elem.evkind = EV_TIMER, 
    TRACE_LOOM && log("-- timer_start, id=" + String(elem.id)), 
    now >= elem.time && elem.prereq == U && (TRACE_LOOM && log("-- timer_start, firing immediate"), 
    elem.countx = 1, call_deferred_func(elem), 
    elem.last_seen = now, 
    elem.time = elem.interval, 
    elem.is_relative = !0, 
    1 == elem.nreps) ? U : (g_loom.push(elem), 
    elem.id));
};
...
export function timer_clear(options) {
    ...
    for (lx = g_loom.length; lx--; ) g_loom[lx].group != group && g_loom[lx].id != id || (TRACE_LOOM && log(str.conv("timer_clear, lx={}, group={}, id={}", lx, group, id)), 
    timer_clearx(lx));
};

export function timer_clear_all() {
    let lx, item;
    for (lx = g_loom.length; lx--; ) (item = g_loom[lx]).group == IDE_GROUP && item.evkind == EV_TIMER && call_deferred_func(item), 
    timer_clearx(lx);
};

export function loom_delta_clock(delta) {
    let lx, item;
    for (lx = g_loom.length; lx--; ) item = g_loom[lx], 
    TRACE_LOOM && log(str.conv("loom_delta, lx={}, time={n3}, is_rel={bool}", lx, item.time, item.is_relative)), 
    item.is_relative && (item.time += delta, 
...
export function loom_animate_num_bezier(valpath, x1, y1, x2, y2, options) {
    ...
    if (TRACE_ANIMATION && log(str.conv("adding bez animation startval={n,3}, endval={n,3}, duration={}", startval, endval, duration)), 
    startval !== endval) {
        let anim = new a_loom_anim_num();
        return g_num_animations.push(anim), 
        path_setv(_M, WHERE_LOOM, busypath, Y), 
        anim.valpath = valpath.clone(), 
        anim.busypath = busypath.clone(), 
        anim.startval = startval, 
        anim.endval = endval, 
        anim.is_dilated = is_dilated, 
        anim.starttime = is_dilated ? now : now_raw, 
        anim.duration = duration, 
        anim.bez_x1 = x1, anim.bez_y1 = y1, 
        anim.bez_x2 = x2, anim.bez_y2 = y2, 
...
export function hsl(h, s, l) {
    ...
    return 0 <= h && h < 60 ? (r = c, 
    g = x, b = 0) : 60 <= h && h < 120 ? (r = x, 
    g = c, b = 0) : 120 <= h && h < 180 ? (r = 0, 
    g = c, b = x) : 180 <= h && h < 240 ? (r = 0, 
    g = x, b = c) : 240 <= h && h < 300 ? (r = x, 
    g = 0, b = c) : 300 <= h && h < 360 && (r = c, 
    g = 0, b = x), Math.round(255 * (r + m)) << 16 | Math.round(255 * (g + m)) << 8 | Math.round(255 * (b + m));
};
...
export function color_to_hsl(rgb) {
    ...
    h = 0 == delta ? 0 : cmax == r ? (g - b) / delta % 6 : cmax == g ? (b - r) / delta + 2 : (r - g) / delta + 4, 
    (h = Math.round(60 * h)) < 0 && (h += 360), 
    l = (cmax + cmin) / 2, 
    s = 0 == delta ? 0 : delta / (1 - Math.abs(2 * l - 1));
    var hsl = new a_tree(_M, "$hsl");
    return setv(_M, 0, hsl, F_hsl_h, Math.round(h)), 
    setv(_M, 0, hsl, F_hsl_s, Math.round(100 * s)), 
    setv(_M, 0, hsl, F_hsl_l, Math.round(100 * l)), 
    hsl;
};
...
export function color_to_hsv(rgb) {
    ...
    hue = max === min ? 0 : max === r ? (60 * (g - b) / (max - min) + 360) % 360 : max === g ? 60 * (b - r) / (max - min) + 120 : 60 * (r - g) / (max - min) + 240, 
    value = max, saturation = 0 === max ? 0 : (max - min) / max;
    var hsv = new a_tree(_M, "$hsv");
    return setv(_M, 0, hsv, F_hsv_h, Math.round(hue)), 
    setv(_M, 0, hsv, F_hsv_s, Math.round(100 * saturation)), 
    setv(_M, 0, hsv, F_hsv_v, Math.round(value / 255 * 100)), 
    hsv;
};
...
export function rgb(r, g, b, a = 0) {
    let ri, gi, bi, ai;
    return is_err_enum_b(r) || is_err_enum_b(g) || is_err_enum_b(b) || is_err_enum_b(a) ? ERR : r === U || g === U || b === U || a === U ? U : (ri = r < 0 ? 0 : r > 255 ? 255 : r, 
    gi = g < 0 ? 0 : g > 255 ? 255 : g, 
    bi = b < 0 ? 0 : b > 255 ? 255 : b, 
    (ai = a < 0 ? 0 : a > 255 ? 255 : a) << 24 | ri << 16 | gi << 8 | bi);
};

export function rgb1(r, g, b, a = 0) {
    let ri, gi, bi, ai;
    return is_err_enum_b(r) || is_err_enum_b(g) || is_err_enum_b(b) || is_err_enum_b(a) ? ERR : r === U || g === U || b === U || a === U ? U : (r < 0 ? r = 0 : r > 1 && (r = 1), 
    g < 0 ? g = 0 : g > 1 && (g = 1), 
    b < 0 ? b = 0 : b > 1 && (b = 1), 
    a < 0 ? a = 0 : a > 1 && (a = 1), 
    (ai = Math.round(255 * a)) << 24 | (ri = Math.round(255 * r)) << 16 | (gi = Math.round(255 * g)) << 8 | (bi = Math.round(255 * b)));
};

export function count(targ) {
    var pathx, node;
    if (targ instanceof a_tree) node = targ, 
    null != g_curr_block && node.eflags & NF_STATEFUL && add_node_dependency(node); else if (targ instanceof a_path) {
        if (pathx = path_to_pathx(targ), 
        null != g_curr_block && pathx.basepath.base.eflags & NF_STATEFUL && add_pathx_dependency(pathx), 
        null === pathx || !pathx.exists) return 0;
...
export function find_min(targ) {
    var pathx, node, best;
    if (targ instanceof a_tree) node = targ, 
    null != g_curr_block && node.eflags & NF_STATEFUL && add_node_dependency(node); else {
    ...
    var child, empty = !0;
    for (var ix in node.children) is_numeric((child = node.children[ix]).val) && (empty ? (best = child.val, 
    empty = !1) : child.val < best && (best = child.val));
    return empty ? U : best;
};
...
export function tree_lo(targ, dependency = !0) {
    var pathx, node;
    if (targ instanceof a_tree) node = targ, 
    dependency && null != g_curr_block && node.eflags & NF_STATEFUL && add_node_dependency(node); else if (targ instanceof a_path) {
        if (pathx = path_to_pathx(targ), 
        dependency && null != g_curr_block && pathx.basepath.base.eflags & NF_STATEFUL && add_pathx_dependency(pathx), 
        null === pathx || !pathx.exists) return U;
        node = pathx.lev[pathx.lev.length - 1];
    } else argument_err("expected path or tree");
    if (null === node || null === node.children) return U;
    var curr, sub, best = INFINITY;
    for (var ix in node.children) is_numeric(sub = node.children[ix].sub) == Y && (curr = sub) < best && (best = curr);
...
export function tree_sibling_hi(curr) {
    ...
    if (is_numeric(refsub) != Y) return null;
    var sub, parent = px.lev[px.lev.length - 2], bestsub = INFINITY;
    for (var ix in refsub++, 
    parent.children) if (is_numeric(sub = parent.children[ix].sub) == Y && sub >= refsub && sub < bestsub && (bestsub = sub) == refsub) break;
    if (bestsub == INFINITY) return null;
    var result = curr.clone();
    return result.key[result.key.length - 1] = bestsub, 
    result;
};
...
export function tree_index(base, sortfunc) {
    for (var p, result = new a_tree(_M, "$tree_index"), list = [], myloop = new a_loop({
        across: base
    }); myloop.next(); ) p = myloop.path.clone(), 
    list.push(p);
    for (p of (list.sort(sortfunc.code), 
    list)) append_val(_M, 0, p, adr(result));
    return result;
};
...
function escape_plain_for_html(old) {
    ...
    for (;pos < len; ) {
        switch (cc = old.charAt(pos), 
        pos += 1, cc) {
          case "\r":
            pos < len && "\n" == old.charAt(pos) && (pos += 1), 
            cc = "<br>";
            break;
...
export function draw_str(container, ss, options) {
    ...
    for (let property in options) switch (property) {
      ...
      case "strike":
        is_strikeout = options.strike === Y;
        break;

      case "hide_u":
        is_u_empty = options.hide_u === Y;
        break;

      case "und":
        is_underline = options.und === Y;
        break;
    ...
    if (just == RIGHT && 0 != indent && rindent == U && (rindent = indent, 
    indent = 0), dest_x != U) {
    ...
    if (null != box && (box.height != U && box.width != U || (vert = 0, 
    shrink = !1), size <= 1 && (size = box.height * size)), 
    null != metrics && (shrink = !1, 
    just = LEFT, vert = 0, 
    backcolor = U, border = 0), 
    leading == U ? leading = 1.16 * size : leading < 2 && (leading *= size), 
    net_ss = ss === U_STR ? is_u_empty ? "" : "U" : null == ss || "" == ss ? " " : ss, 
    null == box ? (avail_h = GIANT_SIZE, 
    avail_v = GIANT_SIZE, 
    backcolor = U, border = 0, 
    shrink = !1, indent = 0, 
    rindent = 0) : (box.width == U ? avail_h = GIANT_SIZE : just == CENTER ? rindent == U ? (avail_h = box.width - 2 * indent, 
    rindent = 0) : avail_h = box.width - indent : avail_h = just == LEFT ? box.width - indent : box.width, 
    rindent != U ? avail_h -= rindent : rindent = 0, 
    avail_v = box.height == U ? GIANT_SIZE : box.height, 
    backcolor !== U && draw_rect(container, {
        box: box,
        color: backcolor,
        corner: corner
        ...
        corner: corner,
        pos: 1
    })), avail_h <= 10 || "" == net_ss || " " == net_ss) null !== metrics && (setv(_M, 0, metrics, F_x, 0), 
    setv(_M, 0, metrics, F_y, 0)); else if (null == container.can) {
        if ((mydiv = document.createElement("div")).style.position = "absolute", 
        shadowr > 0 && (mydiv.style.textShadow = `${shadowx}px ${shadowy}px ${shadowr}px ${js_color_to_str(shadowc)}`), 
        is_selectable || (mydiv.style.webkitUserSelect = "none", 
        mydiv.style.pointerEvents = "none", 
        mydiv.style.userSelect = "none"), 
        mydiv.style.font = gen_font_ss(font, size, leading, weight, is_italic), 
        set_attrib(mydiv, just, opacity, color, is_underline, is_strikeout), 
        0 != angle && (mydiv.style.transform = "rotate(" + angle.toFixed(3) + "rad)"), 
        0 != maxlines && (mydiv.style.textOverflow = "ellipsis", 
        mydiv.style.overflow = "hidden", 
        mydiv.style.display = "-webkit-box", 
        mydiv.style.webkitLineClamp = String(maxlines), 
        mydiv.style.webkitBoxOrient = "vertical"), 
        null == box) mydiv.style.left = to_px(dest_x), 
        mydiv.style.top = to_px(dest_y), 
        elem_set_text(mydiv, net_ss), 
        container.appendChild(mydiv); else {
            switch (mydiv.style.display = "flex", 
            mydiv.style.top = to_px(box.top), 
            null == metrics && (mydiv.style.height = to_px(box.height)), 
            just) {
              case LEFT:
              case FLUSH:
                mydiv.style.justifyContent = "left", 
                mydiv.style.left = to_px(box.left + indent), 
                mydiv.style.width = to_px(avail_h);
                break;

              case RIGHT:
                mydiv.style.justifyContent = "right", 
                mydiv.style.left = to_px(box.left), 
                mydiv.style.width = to_px(avail_h);
                break;
            ...
            if (0 == vert) mydiv.style.alignItems = "start", 
            elem_set_text(myspan = add_span(mydiv), net_ss); else if (.5 == vert) mydiv.style.alignItems = "center", 
            elem_set_text(myspan = add_span(mydiv), net_ss); else if (1 == vert) mydiv.style.alignItems = "end", 
            elem_set_text(myspan = add_span(mydiv), net_ss); else {
                switch (mydiv.style.flexDirection = "column", 
                just) {
                  case LEFT:
                  case FLUSH:
                ...
                (myspan = add_span(mydiv)).style.flexBasis = to_percent(vert), 
                elem_set_text(myspan = add_span(mydiv), net_ss), 
                (myspan = add_span(mydiv)).style.flexBasis = to_percent(1 - vert);
            }
            if (shrink) {
                mydiv.style.visibility = "hidden", 
                container.appendChild(mydiv), 
                rbox = tag_with_text.getBoundingClientRect(), 
                trailing = 0, leading > size && (trailing = leading - size);
                ...
                if (rbox.width > avail_h) {
                    ...
                    if (size *= net_ratio, 
                    leading *= net_ratio, 
                    mydiv.style.fontSize = to_px(size), 
                    mydiv.style.lineHeight = to_px(leading), 
                    fit_ratio < min_ratio && !is_html) {
                        var targlen = round_down(net_ss.length * avail_h / rbox.width) - 2;
                        net_ss = net_ss.substr(0, targlen) + "...", 
                        elem_set_text(tag_with_text, net_ss);
                    }
                }
        ...
        if (null !== metrics) {
            rbox = mydiv.getBoundingClientRect(), 
            trailing = 0, leading > size && (trailing = leading - size);
            let renderedv = rbox.height - trailing;
            setv(_M, 0, metrics, F_x, rbox.width), 
            setv(_M, 0, metrics, F_y, renderedv), 
        ...
        leading == U && (leading = 1.2 * size), 
        ctx.font = gen_font_ss(font, size, leading, weight, is_italic), 
        ctx.fillStyle = js_color_to_str(color), 
        0 != shadowr && (ctx.shadowColor = js_color_to_str(shadowc), 
        ...
        just) {
          ...
          case FLUSH:
            ctx.textAlign = "left", 
            null != box && (dest_x = box.left + indent);
            break;

          case CENTER:
            ctx.textAlign = "center", 
            null != box && (dest_x = box.left + box.width / 2);
            break;

          case RIGHT:
            ctx.textAlign = "right", 
            null != box && (dest_x = box.left + box.width - rindent);
            break;

          default:
            CHECKS && argument_err("bad just"), 
            ctx.textAlign = "left";
        }
        ctx.textBaseline = "top", 
        null != box && (dest_y = box.top);
        ...
        if (null !== metrics) {
            ...
            setv(_M, 0, metrics, F_x, tot_width), 
            setv(_M, 0, metrics, F_y, tot_height);
        } else for (leftover = Math.max(0, box.height - tot_height), 
        dest_y = box.top + Math.round(leftover * vert), 
        ii = 0; ii < nlines; ii++) ctx.fillText(lines[ii], dest_x, dest_y), 
        dest_y += leading;
        ctx.restore();
    }
    ...
    function elem_set_text(elem, myss) {
        var final_ss;
        is_html ? (wrap || (elem.style.whiteSpace = "nowrap"), 
        final_ss = myss) : (elem.style.whiteSpace = wrap ? "pre-wrap" : "pre", 
        final_ss = escape_plain_for_html(myss)), 
        elem.innerHTML = final_ss, 
        tag_with_text = elem;
    }
};
...
function on_input_focus(evt) {
    ...
    TRACE_INPUT && log(`INPUT: on_input_focus, label=${gets(field, F_in_label)}`), 
    g_focus_elem = elem, 
    g_focus_field = field.clone();
}

function on_input_blur(evt) {
    var elem = evt.target, field = elem.my_field;
    TRACE_INPUT && log(`INPUT: on_input_blur, label=${gets(field, F_in_label)}, val=${elem.value}`), 
    g_focus_elem = null, 
    g_focus_field = null;
}
...
function on_input_change(evt) {
    var elem = evt.target, field = elem.my_field;
    TRACE_INPUT && log(str.conv("INPUT: on_input_change, newval=[{}]", elem.value)), 
    add_major_step(EV_INPUT);
    var val_ptr = get_ptr(field, F_in_value_ptr);
    null != val_ptr ? setv(_M, 1, val_ptr, elem.value) : setv(_M, 1, field, F_in_value, elem.value), 
    validate_input(field, elem.value), 
    TRACE_INPUT && log(str.conv(" selStart={}, selEnd={}", elem.selectionStart, elem.selectionEnd));
    ...
    if (null != warning) {
        ...
        warning.innerText = warnss, 
        TRACE_INPUT && log(`INPUT: warning text updated to [${warnss}]`);
    }
}
...
export function validate_input(field, val) {
    var result = Y;
    null == val && (val = ""), 
    TRACE_INPUT && log(`validateinput, label=${gets(field, F_in_label)}, val=[${val}]`);
    var minlen = getn(field, F_in_minlen), maxlen = getn(field, F_in_maxlen), is_required = getn(field, F_in_required) == Y;
    if (0 == val.length) is_required ? (result = N, 
    TRACE_INPUT && log("  failed because empty, but required")) : (result = Y, 
    TRACE_INPUT && log("  okay because empty, not required")); else if (minlen != U && val.length < minlen) result = N, 
    TRACE_INPUT && log("  failed because too short"); else if (maxlen != U && val.length > maxlen) result = N, 
    TRACE_INPUT && log("  failed because too long"); else {
        var vpatt = get_object(field, F_in_pattern);
        null != vpatt && (vpatt.lastIndex = 0, 
        vpatt.test(val) || (result = N, 
        TRACE_INPUT && log("  failed because pattern match failed")), 
        TRACE_INPUT && log(`  regular expression pattern, lastindex=${vpatt.lastIndex}`));
        var vfunc = get_func(field, F_in_validator);
        if (null != vfunc) {
            TRACE_INPUT && log("  has custom validation function");
            var validf = vfunc.code(field);
            validf != Y && (result = N, 
            TRACE_INPUT && log(`  failed because custom validator failed, val=${validf}`));
        }
    }
    return setv(_M, 0, field, F_in_ok, result), 
    TRACE_INPUT && log(str.conv("  ..end validate, ok={}", result)), 
    result;
};
...
function calc_warnss(elem, field) {
    ...
    var result = "";
    return elem.required && 0 == elem.value.trim().length ? elem.my_show_star ? result = FIELD_EMPTY_SYMBOL : !0 : ok_flag == N && (result = FIELD_BAD_SYMBOL), 
    result;
}

export function draw_input(container, field, options) {
    let backcolor = U, border = 2, bordercolor = BLACK, borderempty = TOMATO, box = null, boxarg = null, caret = U, color = BLACK, corner = 0, font = "", indent = 0, is_multiline = getn(field, F_in_multiline) == Y, is_italic = !1, just = LEFT, leading = U, opacity = 1, show_star = !1, show_errs = !0, size = pt_to_dots(container, 10), weight = 400;
    for (let property in options) switch (property) {
      case "bold":
        options.bold === Y && (weight = 700);
        break;
    ...
    show_star && (show_errs = !0);
    var elem = get_object(field, F_in_sysobj);
    if (null != elem) {
        var tip = gets(field, F_in_tip);
        if (tip != U_STR && (elem.title = tip), 
        elem.oninput = on_input_change, 
        elem.onblur = on_input_blur, 
        elem.onfocus = on_input_focus, 
        elem.my_field = field, 
        elem.my_show_star = show_star, 
        size < 1 && (size = box.height * size), 
        is_multiline ? leading == U ? leading = 1.16 * size : leading < 2 && (leading *= size) : leading = U, 
        elem.style.position = "absolute", 
        elem.style.left = to_px(box.left), 
        elem.style.top = to_px(box.top), 
        elem.style.width = to_px(box.width), 
        elem.style.height = to_px(box.height), 
        0 != corner && (elem.style.borderRadius = to_px(corner)), 
        elem.style.border = `${border.toFixed(1)}px solid ${js_color_to_str(bordercolor)}`, 
        TRACE_DINPUT && log(`  input border, color=${js_color_to_str(bordercolor)}`), 
        backcolor != U && (elem.style.backgroundColor = js_color_to_str(backcolor)), 
        elem.style.boxSizing = "border-box", 
        elem.style.paddingLeft = to_px(indent), 
        elem.style.font = gen_font_ss(font, size, leading, weight, is_italic), 
        set_attrib(elem, just, opacity, color, !1, !1), 
        caret != U && (elem.style.caretColor = js_color_to_str(caret)), 
        container.appendChild(elem), 
        show_errs) {
            let warnjust;
            var warn = document.createElement("div");
            warn.id = "warn_" + String(g_focus_id++), 
            elem.my_warning_id = warn.id, 
            container.appendChild(warn), 
            warn.style.userSelect = "none", 
            warn.style.webkitUserSelect = "none", 
            warn.style.cursor = "", 
            warn.style.position = "absolute";
            var wbox_l, hindent = .12 * box.height, wbox_width = (box.height, 
            box.height, box.height);
            just == RIGHT ? (wbox_l = box.left + hindent, 
            warnjust = LEFT) : (wbox_l = box.left + box.width - hindent - wbox_width, 
            warnjust = RIGHT), warn.style.left = to_px(wbox_l), 
            warn.style.top = to_px(box.top + 2), 
            warn.style.width = to_px(wbox_width), 
            warn.style.height = to_px(box.height - 4), 
            warn.style.verticalAlign = "middle", 
            warn.style.pointerEvents = "none";
            var warnsize = is_multiline ? size : pt_to_dots(container, 10);
            warn.style.font = gen_font_ss("", warnsize, U, 400, !1), 
            set_attrib(warn, warnjust, 1, FIELD_EMPTY_COLOR, !1, !1), 
...
function gen_font_ss(font, size, leading, weight, is_italic) {
    var ss = "";
    return is_italic && (ss += "italic"), 
    400 != weight && (ss += " " + weight), 
    ss += " " + to_px(size), 
    leading !== U && (ss += `/${leading.toFixed(1)}px`), 
    "" === font || "_sans" === font ? font = "sans-serif" : "_serif" === font ? font = "serif" : "_typewriter" === font && (font = "monospace"), 
    ss + " " + font;
}
...
function set_attrib(mydiv, just, opacity, color, is_underline, is_strikeout) {
    ...
    mydiv.style.textAlign = align_ss, 
    1 != opacity && (mydiv.style.opacity = opacity.toFixed(3)), 
    mydiv.style.color = js_color_to_str(color);
    let decoration = "";
    is_underline && (decoration += " underline"), 
    is_strikeout && (decoration += " line-through"), 
    "" != decoration && (mydiv.style.textDecoration = decoration);
}
...
export function draw_image(container, image, options) {
    ...
    if (null === image) return TRACE_BITMAP && log("skipping draw_image of null picture"), 
    null;
    TRACE_IMG && log(`draw_image, src=${image.sysobj.url.substr(0, 70)}`);
    for (let property in options) switch (property) {
      case "angle":
        !CHECKS || options.angle instanceof a_meas || argument_err("angle argument must be a meas"), 
        0 !== options.angle.mag && (angle = options.angle.mag);
        break;
      ...
      case "horz":
        horz = options.horz, 
        CHECKS && (horz < 0 || horz > 1) && argument_err("bad horz");
        break;
      ...
      case "opacity":
        if (0 == (opacity = clamp(0, numeric_arg(options.opacity), 1))) return TRACE_IMG && log("draw_image, ignoring transparent image"), 
        null;
        break;

      case "origin":
        options.origin instanceof a_tree || internal_err("bad arg"), 
      ...
      case "vert":
        vert = options.vert, 
        CHECKS && (vert < 0 || vert > 1) && argument_err("bad horz");
        break;

      case "xy":
        options.xy instanceof a_tree || internal_err("bad arg"), 
        dest_x = getn(options.xy, F_x), 
        dest_y = getn(options.xy, F_y);
        break;
    ...
    var tag;
    if (image != ERR_IMAGE) return (tag = image.sysobj.cloneNode(!1)).url = image.sysobj.url, 
    tag.style.position = "absolute", 
    tag.style.userDrag = "none", 
    filters != U_STR && filters != ERR_STR && (tag.style.filter = filters), 
    tag.id = null != idss ? idss : `img${++g_svg_id}`, 
    0 != corner && (tag.style.borderRadius = js_corner_to_str(corner)), 
    1 != opacity && (tag.style.opacity = opacity.toFixed(3)), 
    void 0 == tag.naturalWidth || 0 == tag.naturalWidth ? (tag.src = tag.url, 
    TRACE_IMG && log(`image not preloaded, src=${tag.src}`), 
    tag.onload = function callback_on_load() {
        TRACE_IMG && log(`...in callback_on_load of ${this.src.substr(0, 70)}, natural=${this.naturalWidth} x ${this.naturalHeight}`);
        this.onload = null, 
        this.onerror = null, 
        draw_image2(this);
    }, tag.onerror = function callback_on_err() {
        TRACE_IMG && log(`...in callback_on_err of ${this.src.substr(0, 70)}`);
        return this.onload = null, 
        this.onerror = null, 
        void draw_missing_image(container, box, this.src);
    }, tag.style.visibility = "hidden") : (TRACE_IMG && log(`image ready, size=${tag.naturalWidth}`), 
    draw_image2(tag)), void (null == container.can && container.appendChild(tag));
    if (TRACE_IMG && log("draw_image, ERR"), 
    null == box) return null;
    draw_rect(container, {
        box: box,
        fill: PINK,
        corner: 10
    ...
    function draw_image2(mytag) {
        ...
        if (raw_h = mytag.naturalWidth, 
        raw_v = mytag.naturalHeight, 
        null !== box) net_boxR = tree_to_rect(solve_rect({
            basis: box,
            pin: 5,
            inset: indent
        })), TRACE_IMG && log(` draw_image2, box=[${net_boxR.left}, ${net_boxR.top}], ${net_boxR.width} x ${net_boxR.height}, raw=${raw_h} x ${raw_v}`), 
        horz_ratio = net_boxR.width / raw_h, 
        vert_ratio = net_boxR.height / raw_v, 
        preserve_aspect && (vert_ratio = horz_ratio = min(horz_ratio, vert_ratio)), 
        shrink || (horz_ratio = max(horz_ratio, 1), 
        vert_ratio = max(vert_ratio, 1)), 
        grow || (horz_ratio = min(horz_ratio, 1), 
        vert_ratio = min(vert_ratio, 1)), 
        TRACE_IMG && log(`  net ratios=${horz_ratio.toFixed(3)}, ${vert_ratio.toFixed(3)}`), 
        horz_size = horz_ratio * raw_h, 
        vert_size = vert_ratio * raw_v, 
        dest_x = net_boxR.left + (net_boxR.width - horz_size) * horz, 
        dest_y = net_boxR.top + (net_boxR.height - vert_size) * vert, 
        mytag.style.left = `${dest_x.toFixed(1)}px`, 
        mytag.style.top = `${dest_y.toFixed(1)}px`, 
        mytag.style.width = `${horz_size.toFixed(1)}px`, 
        mytag.style.height = `${vert_size.toFixed(1)}px`, 
        null != container.can && ((ctx = container.ctx).save(), 
        ctx.globalAlpha = opacity, 
        ctx.drawImage(mytag, dest_x, dest_y, horz_size, vert_size), 
        ctx.restore()); else if (null != container.can) ctx = container.ctx, 
        TRACE_IMG && log(` draw_image2 canv nobox, ox=${origin_x}, oy=${origin_y}, size=${raw_h},${raw_v}`), 
        ctx.save(), ctx.globalAlpha = opacity, 
        ctx.translate(dest_x, dest_y), 
        0 != angle && ctx.rotate(angle), 
        ctx.translate(-origin_x, -origin_y), 
        ctx.drawImage(mytag, 0, 0), 
        ctx.restore(); else {
            let transform_ss;
            if (net_boxR = new Rectangle(dest_x, dest_y, raw_h, raw_v), 
            TRACE_IMG && log(` draw_image2 nobox, dest=[${net_boxR.left}, ${net_boxR.top}], ${net_boxR.width} x ${net_boxR.height}`), 
...
function svg_define_blur(blurr, wrapper) {
    ...
    return filter_elem.appendChild(blurf), 
    blurf.setAttribute("stdDeviation", `${blurr}`), 
    filter_id;
}
...
function svg_define_pattern(tile, wrapper) {
    ...
    pattern.setAttribute("id", myid), 
    pattern.setAttribute("patternUnits", "userSpaceOnUse"), 
    pattern.setAttribute("width", known_width.toFixed(0)), 
    pattern.setAttribute("height", known_height.toFixed(0)), 
    defs.appendChild(pattern);
    let image = document.createElementNS(SVG_NS, "image");
    return image.setAttributeNS(XLINK_NS, "xlink:href", tile.sysobj.url), 
    image.setAttribute("width", known_width.toFixed(0)), 
...
function svg_define_gradient(gradpath, wrapper) {
    ...
    for (;iter.next(); ) {
        ...
        1 != stop_opacity && stop.setAttribute("stop-opacity", stop_opacity.toFixed(3)), 
...
export function canvas_to_image(canvpath) {
    ...
    return rec.sysobj.src = obj.can.toDataURL(), 
    rec;
};
...
function canvas_define_gradient(ctx, box, gradpath) {
    ...
    switch (getn(node, F_grad_shape)) {
      ...
      case RADIAL_GRADIENT:
        cx = getn(node, F_grad_centerx), 
        cy = getn(node, F_grad_centery), 
        inner_radius = getn(node, F_grad_radius_inner), 
        outer_radius = getn(node, F_grad_radius_outer), 
        gradient = ctx.createRadialGradient(cx, cy, inner_radius, cx, cy, outer_radius);
        break;

      default:
        return argument_err("bad gradient shape"), 
        null;
    }
    for (var iter = new a_loop({
        ...
        stop_opacity === U && (stop_opacity = 1), 
        gradient.addColorStop(stop_pos / 100, js_color_to_str(stop_color, stop_opacity));
    }
    return gradient;
}
...
export function clear_rect(container, boxarg) {
    let box;
    if (boxarg instanceof a_tree || boxarg instanceof a_path ? box = tree_to_rect(boxarg) : boxarg instanceof Rectangle ? box = boxarg : argument_err("bad box"), 
    null != container.can) {
...
export function draw_rect(container, options) {
    ...
    for (let property in options) switch (property) {
      ...
      case "corner":
        if ("number" == typeof (cornerlist = options.corner)) ncorners = 1, 
        corner1 = cornerlist; else if (ncorners = count(cornerlist), 
        corners = [], 1 == ncorners || 2 == ncorners || 4 == ncorners || 8 == ncorners) {
      ...
      case "pos":
        is_numeric(options.pos) !== Y && argument_err("bad pos"), 
        pos = clamp(0, options.pos, 1);
        break;
      ...
      case "thick":
        is_numeric(options.thick) !== Y && argument_err("bad thick"), 
    ...
    if (null == boxarg && (box = container.bounds), 
    box.width < 1 || box.height < 1) return;
    if (!(filling = null !== grad || null !== tile || fill !== U) && strokewidth <= 0) return;
    if (null != container.can) {
        let ctx = container.ctx;
        if (ctx.save(), ctx.globalAlpha = opacity, 
        filling && (canv_rr_path(ctx, box), 
        canv_fill(ctx, box, fill, grad, tile)), 
        strokewidth > 0) canv_rr_path(ctx, calc_middle_of_stroke(box, strokewidth, pos)), 
        ctx.lineWidth = strokewidth, 
        ctx.strokeStyle = js_color_to_str(strokecolor), 
        ctx.stroke();
        return void ctx.restore();
    }
    function canv_rr_path(ctx, box) {
        let net_l = box.left, net_t = box.top, net_w = box.width, net_h = box.height;
        switch (ctx.beginPath(), 
        ncorners) {
          case 0:
    ...
    if (pass1 && pass2 && 1 != opacity && (group = document.createElementNS(SVG_NS, "g"), 
    wrapper.appendChild(group), 
    wrapper = group, 1 != opacity && (group.setAttribute("opacity", opacity.toFixed(3)), 
    opacity = 1)), pass1) {
        var did;
        if (null != tile ? did = svg_define_pattern(tile, wrapper) : null != grad && (did = svg_define_gradient(grad, wrapper)), 
        shape = svg_define_roundrect(box), 
        null != tile ? shape.setAttribute("fill", `url(#${did})`) : null != grad ? shape.setAttribute("fill", `url(#${did})`) : fill == U ? shape.setAttribute("fill", "none") : shape.setAttribute("fill", js_color_to_str(fill)), 
        strokewidth > 0 && .5 == pos && (shape.setAttribute("stroke", js_color_to_str(strokecolor)), 
        ...
        null != shadow_ptr && (shape.style.boxShadow = js_shadow_ss(shadow_ptr)), 
        wrapper.appendChild(shape);
    }
    if (pass2) {
        (shape = svg_define_roundrect(calc_middle_of_stroke(box, strokewidth, pos), interpolate(pos, 0, 1, -strokewidth / 2, strokewidth / 2))).setAttribute("stroke", js_color_to_str(strokecolor)), 
        shape.setAttribute("stroke-width", strokewidth.toFixed(1)), 
    ...
    function svg_define_roundrect(mybox, adjust = 0) {
        ...
        switch (ncorners) {
          ...
          case 1:
          case 2:
            (myshape = document.createElementNS(SVG_NS, "rect")).setAttribute("x", mybox.left.toFixed(1)), 
            myshape.setAttribute("y", mybox.top.toFixed(1)), 
            myshape.setAttribute("width", mybox.width.toFixed(1)), 
            myshape.setAttribute("height", mybox.height.toFixed(1)), 
            1 == ncorners ? (ss = corn(corner1, adjust).toFixed(1), 
            myshape.setAttribute("rx", ss), 
            myshape.setAttribute("ry", ss)) : 2 == ncorners && (myshape.setAttribute("rx", corn(corners[0], adjust).toFixed(1)), 
            myshape.setAttribute("ry", corn(corners[1], adjust).toFixed(1)));
            break;

          case 4:
          case 8:
            (myshape = document.createElementNS(SVG_NS, "path")).setAttribute("d", function svg_roundrect_path(left, top, h, v, adjust = 0) {
                var tlh, tlv, trh, trv, brh, brv, blh, blv;
                4 == ncorners ? (tlh = corn(corners[0], adjust), 
                tlv = tlh, trh = corn(corners[1], adjust), 
                trv = trh, brh = corn(corners[2], adjust), 
                brv = brh, blh = corn(corners[3], adjust), 
                blv = blh) : (tlh = corn(corners[0], adjust), 
                tlv = corn(corners[1], adjust), 
                trh = corn(corners[2], adjust), 
                trv = corn(corners[3], adjust), 
                brh = corn(corners[4], adjust), 
                brv = corn(corners[5], adjust), 
                blh = corn(corners[6], adjust), 
                blv = corn(corners[7], adjust));
...
export function draw_with_transform(parent, drawfunc, options) {
    ...
    console.log(`draw_with_trans, drawf=${drawfunc.name}, pushx=${pushx.toFixed(1)}, opacity=${opacity.toFixed(3)}`), 
    0 != pushx ? area = rect_as_tree(pushx, 0, parent.bounds.width, parent.bounds.height) : 0 != pushy && (area = rect_as_tree(0, pushy, parent.bounds.width, parent.bounds.height));
...
export function draw_oval(container, options) {
    ...
    if (null == box && (box = container.bounds), 
    null != container.can) {
        let ctx = container.ctx;
        if (ctx.save(), ctx.globalAlpha = opacity, 
        filling && (canv_oval_path(ctx, box), 
        canv_fill(ctx, box, fill, grad, tile)), 
        thick > 0) canv_oval_path(ctx, calc_middle_of_stroke(box, thick, pos)), 
        ctx.lineWidth = thick, 
        ctx.strokeStyle = js_color_to_str(strokecolor), 
        ctx.stroke();
        return void ctx.restore();
    ...
    opacity = 1)), pass1) {
        ...
        rx = box.width / 2, 
        ry = box.height / 2, 
        (shape = document.createElementNS(SVG_NS, "ellipse")).setAttribute("cx", (box.left + rx).toFixed(1)), 
        shape.setAttribute("cy", (box.top + ry).toFixed(1)), 
        shape.setAttribute("rx", rx.toFixed(1)), 
        shape.setAttribute("ry", ry.toFixed(1)), 
        null != tile ? shape.setAttribute("fill", `url(#${did})`) : null != grad ? shape.setAttribute("fill", `url(#${did})`) : fill == U ? shape.setAttribute("fill", "transparent") : shape.setAttribute("fill", js_color_to_str(fill)), 
        thick > 0 && .5 == pos && (shape.setAttribute("stroke", js_color_to_str(strokecolor)), 
        shape.setAttribute("stroke-width", thick.toFixed(1))), 
        1 != opacity && shape.setAttribute("opacity", opacity.toFixed(3)), 
        0 != blurr) {
            var blur_id = svg_define_blur(blurr, wrapper);
            shape.setAttribute("filter", `url(#${blur_id})`);
    ...
    if (pass2) {
        ...
        rx = newbox.width / 2, 
        ry = newbox.height / 2, 
        (shape = document.createElementNS(SVG_NS, "ellipse")).setAttribute("cx", (newbox.left + rx).toFixed(1)), 
...
export function draw_circle(container, options) {
    ...
    for (let property in options) switch (property) {
      ...
      case "xy":
        cx = getn(options.xy, F_x), 
        cy = getn(options.xy, F_y);
        break;
    ...
    if (is_numeric(cx) === Y) if (is_numeric(cy) === Y) {
        ...
        options.box = box, draw_oval(container, options), 
        delete options.box;
...
export function draw_line(container, options) {
    ...
    if (width < 0) return;
    if (null !== angle && (dx = mul(cos(angle), len), 
    dy = mul(sin(angle), len)), 
    dx !== U && (x2 = add(x1, dx)), 
    dy !== U && (y2 = add(y1, dy)), 
    is_numeric(x1) !== Y || is_numeric(y1) !== Y || is_numeric(x2) !== Y || is_numeric(y2) !== Y) return void (CHECKS && argument_err("bad args"));
    switch (joinx) {
      case JOINT_ROUND:
        joint = "round";
        break;
      ...
      default:
        CHECKS && argument_err("bad line cap"), 
        joint = "round";
    }
    switch (capx) {
      case CAP_BUTT:
    ...
    if (null != container.can) {
        ...
        ctx.lineCap = caps, 
        ctx.lineJoin = joint, 
        ctx.strokeStyle = js_color_to_str(color, opacity), 
        null != dash) {
    ...
    if (shape.setAttribute("x1", x1.toFixed(1)), 
    ...
    null != dash) {
        for (var dash_spec = "", first = !0, loop1 = new a_loop({
            across: dash
        }); loop1.next(); ) first ? first = !1 : dash_spec += ",", 
        dash_spec += loop1.val.toFixed(1);
...
export function draw_polygon(container, polygon, options) {
    ...
    for (let property in options) switch (property) {
      ...
      case "viewbox":
        viewbox_l = getn(options.viewbox, F_left), 
        viewbox_t = getn(options.viewbox, F_top), 
        scalex = .01 * getn(options.viewbox, F_width), 
        scaley = .01 * getn(options.viewbox, F_height);
        break;
    ...
    var did;
    null != container.can ? (ctx = container.ctx, 
    null != tile ? internal_err("not yet") : null != grad ? internal_err("not yet") : fill == U || (ctx.fillStyle = js_color_to_str(fill, opacity)), 
    ctx.lineWidth = width, 
    ctx.lineCap = caps, 
    ctx.lineJoin = joint, 
    ctx.strokeStyle = js_color_to_str(strokecolor, opacity), 
    ctx.beginPath()) : (wrapper = js_svg_wrapper(container, 1), 
    null != tile ? did = svg_define_pattern(tile, wrapper) : null != grad && (did = svg_define_gradient(grad, wrapper)), 
    shape = document.createElementNS(SVG_NS, "polygon"), 
    null != tile ? shape.setAttribute("fill", `url(#${did})`) : null != grad ? shape.setAttribute("fill", `url(#${did})`) : fill == U ? shape.setAttribute("fill", "none") : shape.setAttribute("fill", js_color_to_str(fill)), 
    width > 0 && (shape.setAttribute("stroke", js_color_to_str(strokecolor)), 
    shape.setAttribute("stroke-width", width.toFixed(1)), 
    shape.setAttribute("stroke-linecap", caps), 
    shape.setAttribute("stroke-linejoin", joint)), 
    1 != opacity && shape.setAttribute("opacity", opacity.toFixed(3)));
    ...
    if (fill !== U) ; else if (did_fill = !1, 
    0 == width) return void (CHECKS && argument_err("polygon with no fill or stroke"));
    let iter, mypath, p, first = !0, is_matrix = !1;
    for (polygon instanceof a_path ? poly_path = polygon : polygon instanceof a_tree ? poly_path = adr(polygon) : argument_err("polygon must be a_tree or a_path"), 
    iter = new a_loop({
        across: poly_path
    }); iter.next(); ) mypath = iter.path, 
    first && exists(mypath, 1) && (is_matrix = !0), 
    is_matrix ? (x = getn(mypath, 1), 
    y = getn(mypath, 2)) : (x = getn(mypath, F_x), 
    y = getn(mypath, F_y)), 
    viewbox_l != U && (x = x * scalex + viewbox_l, 
    y = y * scaley + viewbox_t), 
    is_numeric(x) === Y && is_numeric(y) === Y ? first ? (first = !1, 
    null != container.can ? ctx.moveTo(x, y) : ((p = wrapper.createSVGPoint()).x = x, 
    p.y = y, shape.points.appendItem(p))) : null != container.can ? ctx.lineTo(x, y) : ((p = wrapper.createSVGPoint()).x = x, 
    p.y = y, shape.points.appendItem(p)) : CHECKS && argument_err("bad point");
    null != container.can ? (ctx.closePath(), 
    did_fill && ctx.fill(), 
    0 != width && ctx.stroke()) : wrapper.appendChild(shape);
};
...
export function draw_polycurve(container, data, options) {
    ...
    outer: for (;iter.next(); ) switch (opcode = getn(recp = iter.path, 1)) {
      case STEP_MOVE:
        x = getn(recp, 2), y = getn(recp, 3), 
        null != container.can ? ctx.moveTo(x, y) : pathss += ` M${P(x)},${P(y)}`;
        break;

      case STEP_LINE:
        x = getn(recp, 2), y = getn(recp, 3), 
        null != container.can ? ctx.lineTo(x, y) : pathss += ` L${P(x)},${P(y)}`;
        break;

      case STEP_BEZIER1:
        c1x = getn(recp, 2), 
        c1y = getn(recp, 3), 
        x = getn(recp, 4), y = getn(recp, 5), 
        null != container.can ? ctx.quadraticCurveTo(c1x, c1y, x, y) : pathss += ` Q${P(c1x)},${P(c1y)},${P(x)},${P(y)}`;
        break;
      ...
      case STEP_BEZIER2:
        ...
        c2x = getn(recp, 4), 
        c2y = getn(recp, 5), 
        x = getn(recp, 6), y = getn(recp, 7), 
        null != container.can ? ctx.bezierCurveTo(c1x, c1y, c2x, c2y, x, y) : pathss += ` C${P(c1x)},${P(c1y)},${P(c2x)},${P(c2y)},${P(x)},${P(y)}`;
        break;
    ...
    null != container.can ? (ctx.closePath(), 
    did_fill && ctx.fill(), 
    0 != width && ctx.stroke()) : (shape.setAttribute("d", pathss), 
    wrapper.appendChild(shape));
};
...
export async function ixdb_write(schema, objstore, rec_key, datapath) {
    ...
    open.onsuccess = function() {
        ...
        rec_key === U ? store.add(node) : store.put(node, rec_key), 
        tx.oncomplete = function() {
            TRACE_FILE && log(`${performance.now().toFixed(1)}: ixdb_write tx finished`), 
            db.close();
        };
    };
};
    ...
    open.onsuccess = function() {
        ...
        readreq.onsuccess = function(e) {
            if (void 0 !== readreq.result) {
                ...
                TRACE_FILE && log(`${performance.now().toFixed(1)}: ixdb_read IXDB_OK`);
            } else TRACE_FILE && log("ixdb_read: read request null result");
            null != callback && callback.code();
        }, tx.oncomplete = function() {
            TRACE_FILE && log(`${performance.now().toFixed(1)}: ixdb_read tx finished`), 
            db.close();
        };
    };
};
...
export function ixdb_del(schema, objstore, rec_key) {
    ...
    open.onsuccess = function() {
        ...
        rec_key === U ? store.clear() : store.delete(rec_key), 
        TRACE_FILE && log(`${performance.now().toFixed(1)}: ixdb_delete IXDB_OK`), 
        tx.oncomplete = function() {
            TRACE_FILE && log(`${performance.now().toFixed(1)}: ixdb_delete tx finished`), 
            db.close();
        };
    };
};
...
function gen_db_struct(schema, udb) {
    ...
    for (;iter.next(); ) {
        ...
        if (TRACE_FILE && log(`gen_db_struct checking store ${s_name}`), 
        !udb.objectStoreNames.contains(s_name)) {
            ...
            for (;iter2.next(); ) {
                ...
                TRACE_FILE && log(`gen_db_struct creating index ${ix_name}`), 
...
export function seconds_to_date(seconds, options = null) {
    ...
    return seconds === U && (seconds = now), 
    jsd_to_datetime(new Date(1e3 * seconds), citycode == TIMEZONE_GMT);
};
...
function datetime_to_jsd(datetime) {
    ...
    return year = getn(datetime, F_dt_year), 
    month = getn(datetime, F_dt_month) - 1, 
    day = getn(datetime, F_dt_day), 
    hour = getn(datetime, F_dt_hour), 
    minute = getn(datetime, F_dt_minute), 
    second = getn(datetime, F_dt_second), 
    seconds_whole = Math.floor(second), 
    new Date(year, month, day, hour, minute, seconds_whole, msec = 1e3 * (second - seconds_whole));
}

function jsd_to_datetime(jsd, gmtflag) {
    let net_seconds, result = new a_tree(_M, "$jsd_to_datetime", 0);
    return gmtflag ? (setv(_M, 0, result, F_dt_year, jsd.getUTCFullYear()), 
    setv(_M, 0, result, F_dt_month, jsd.getUTCMonth() + 1), 
    setv(_M, 0, result, F_dt_day, jsd.getUTCDate()), 
    setv(_M, 0, result, F_dt_hour, jsd.getUTCHours()), 
    setv(_M, 0, result, F_dt_minute, jsd.getUTCMinutes()), 
    net_seconds = jsd.getUTCSeconds() + .001 * jsd.getUTCMilliseconds(), 
    setv(_M, 0, result, F_dt_second, net_seconds)) : (setv(_M, 0, result, F_dt_year, jsd.getFullYear()), 
    setv(_M, 0, result, F_dt_month, jsd.getMonth() + 1), 
    setv(_M, 0, result, F_dt_day, jsd.getDate()), 
    setv(_M, 0, result, F_dt_hour, jsd.getHours()), 
    setv(_M, 0, result, F_dt_minute, jsd.getMinutes()), 
    net_seconds = jsd.getSeconds() + .001 * jsd.getMilliseconds(), 
    setv(_M, 0, result, F_dt_second, net_seconds)), 
    result;
}
...
function add_select_list(sel, list) {
    ...
    for (;myloop.next(); ) if (exists(list, ix = myloop.index, F_opt_children)) {
        ...
        add_select_list(sel, adr(list, ix, F_opt_children));
    } else (option = document.createElement("option")).text = gets(list, ix, F_opt_label), 
    option.value = option.text, 
    exists(list, ix, F_opt_val) && (option.value = gets(list, ix, F_opt_val)), 
    getn(list, ix, F_opt_selected) === Y && (option.selected = !0), 
    getn(list, ix, F_opt_disabled) === Y && (option.disabled = !0), 
...
export function browser_download(url, options) {
    ...
    elem.href = url, "" != filename && (elem.download = filename), 
...
export function browser_draw_pulldown(container, box, items, field_ptr, options) {
    ...
    sel.style.left = to_px(getn(box, F_left)), 
    sel.style.top = to_px(getn(box, F_top)), 
    sel.style.width = to_px(getn(box, F_width)), 
    sel.style.height = to_px(getn(box, F_height)), 
    sel.style.fontSize = to_px(size), 
    getn(field_ptr, F_in_required === Y) && (sel.required = !0), 
    null != (ss = gets(field_ptr, F_in_autofill)) && (sel.autocomplete = ss), 
    nitems != U && (sel.size = nitems), 
    is_multiple && (sel.multiple = !0), 
...
export function js_resize() {
    TRACE_RESIZE && log("js_resize"), 
    js_set_screen_size(), 
    OS_KIND != OS_AND || SCREEN_H != g_last_screen_h || SCREEN_V != g_last_screen_v ? (g_last_screen_h = SCREEN_H, 
    g_last_screen_v = SCREEN_V, 
    rebuild_all()) : TRACE_RESIZE && log("js_resize, ignoring spurious resize");
};

export function js_orient() {
    TRACE_RESIZE && log("js_orient"), 
    js_set_screen_size(), 
    rebuild_all();
};
...
function js_touchstart(evt) {
    ...
    if (TRACE_TOUCH && log(`### js_touchstart id=${evt.target.id}, name=${elem_name}, state=${g_touch.state}`), 
    "input" != elem_name && "select" != elem_name && "textarea" != elem_name) {
        var t = evt.changedTouches[0];
        switch (TRACE_TOUCH && log(`  touch id=${t.identifier} page=${t.pageX.toFixed(1)},${t.pageY.toFixed(1)}`), 
        g_touch.state) {
          case TSTATE_IDLE:
            g_touch.id = t.identifier, 
            g_touch.state = TSTATE_IN_TOUCH, 
            g_touch.origintime = elapsed_raw, 
            g_touch.originx = t.pageX, 
            g_touch.originy = t.pageY, 
            g_touch.lastx = t.pageX, 
            g_touch.lasty = t.pageY, 
            g_touch.cum_pixels = 0, 
            g_touch.mod_shf = evt.shiftKey, 
            g_touch.mod_alt = evt.altKey, 
            g_touch.mod_cmd = evt.metaKey, 
            g_touch.mod_ctl = evt.ctrlKey;
            break;
...
function is_suppress_default(elem) {
    for (;elem !== document.body && elem !== document; ) {
        ...
        if (style) {
            ...
            if (TRACE_TOUCH && log(` suppress, ${elem.localName}, overy=${overflow_y}, has_enough=${has_enough}`), 
            ("auto" == overflow_y || "scroll" == overflow_y) && has_enough) return !1;
        }
...
function js_touchmove(evt) {
    if (TRACE_TOUCH && log("### js_touchmove"), 
    is_suppress_default(evt.target) && (TRACE_TOUCH && log(">>> move preventDefault"), 
    evt.preventDefault()), 
    g_touch.state !== TSTATE_IDLE) for (var t, cum_mm, touches = evt.changedTouches, i = 0; i < touches.length; i++) if (t = touches[i], 
    TRACE_TOUCH && log(`js_touchmove id=${t.identifier} page=${t.pageX.toFixed(1)},${t.pageY.toFixed(1)}`), 
    t.identifier === g_touch.id) {
        if (g_touch.state === TSTATE_IN_TOUCH) {
            var dx = t.pageX - g_touch.originx, dy = t.pageY - g_touch.originy;
            cum_mm = 25.4 * sqrt(dx * dx + dy * dy) / SCREEN_DPI, 
            TRACE_TOUCH && log(str.conv("  cum pixels={n} mm={n3} threshold={n}", g_touch.cum_pixels, cum_mm, DRAG_THRESHOLD_MM)), 
            cum_mm >= DRAG_THRESHOLD_MM ? (g_touch.state = TSTATE_IN_DRAG, 
            do_pointer_event(EV_DRAG_BEGIN, elapsed, g_touch.originx, g_touch.originy, g_touch), 
            do_pointer_event(EV_DRAG_MOVE, elapsed, t.pageX, t.pageY, g_touch)) : elapsed_raw - g_touch.origintime > 300 && (do_pointer_event(EV_HOLD, elapsed, g_touch.originx, g_touch.originy, g_touch), 
...
function js_touchend(evt) {
    if (null === g_audioCtx) {
        ...
        AudioContext ? (g_audioCtx = new AudioContext(), 
        TRACE_SOUND && log("audio context created")) : TRACE_SOUND && log("audio context not supported");
    }
    if (TRACE_TOUCH && log(`### js_touchend, state=${g_touch.state}, id=${evt.target.id}`), 
    g_touch.state !== TSTATE_IDLE) {
        for (var t, touches = evt.changedTouches, i = 0; i < touches.length; i++) t = touches[i], 
        TRACE_TOUCH && log(`touchend ${t.identifier} page=${t.pageX.toFixed(1)},${t.pageY.toFixed(1)}`), 
        g_touch.state != TSTATE_IDLE && t.identifier === g_touch.id && (g_touch.state === TSTATE_IN_TOUCH ? (TRACE_TOUCH && log("--- touch converted to TAP"), 
        do_pointer_event(EV_TAP, elapsed, g_touch.originx, g_touch.originy, g_touch), 
        evt.preventDefault()) : g_touch.state === TSTATE_IN_DRAG && 0 != g_capture_blabel && do_pointer_event(EV_DRAG_END, elapsed, t.pageX, t.pageY, g_touch), 
        TRACE_TOUCH && g_touch.state != TSTATE_IDLE && log("--- fixup to IDLE"), 
        g_touch.state = TSTATE_IDLE);
        0 === evt.touches.length && (TRACE_TOUCH && g_touch.state != TSTATE_IDLE && log("--- fixup to IDLE"), 
        g_touch.state = TSTATE_IDLE);
    }
}
...
function js_mousedown(evt) {
    ...
    if (!(evt.button >= 3)) {
        ...
        if (TRACE_MOUSE_CLICK) {
            var flags = "";
            evt.shiftKey && (flags += "SHIFT "), 
            evt.altKey && (flags += "ALT "), 
            evt.metaKey && (flags += "CMD "), 
            evt.ctrlKey && (flags += "CTRL"), 
            TRACE_MOUSE_CLICK && log(str.conv("{n3} js_mousedown, name={} button={}, id={}, page=[{},{}], scrollTop={}, scrollY={} {}", elapsed_raw, target.localName, evt.button, target.id, evt.pageX, evt.pageY, document.body.scrollTop, window.pageYOffset, flags));
        }
        if ("input" != target.localName && "select" != target.localName && "textarea" != target.localName) {
            var gg = g_mouse[evt.button];
            switch (gg.mod_shf = evt.shiftKey, 
            gg.mod_alt = evt.altKey, 
            gg.mod_cmd = evt.metaKey, 
            gg.mod_ctl = evt.ctrlKey, 
            gg.state) {
              case TSTATE_IDLE:
                TRACE_MOUSE_CLICK && log("--- now IN_TOUCH"), 
                ex = evt.pageX, ey = evt.pageY, 
                gg.button = evt.button, 
                gg.state = TSTATE_IN_TOUCH, 
                gg.origintime = elapsed_raw, 
                gg.originx = ex, gg.originy = ey, 
                gg.lastx = ex, gg.lasty = ey, 
                gg.cum_pixels = 0;
                break;

              case TSTATE_AFTER_TOUCH:
                TRACE_MOUSE_CLICK && log("--- AFTER_TOUCH emit double tap"), 
                do_pointer_event(EV_DOUBLE_TAP, elapsed, gg.originx, gg.originy, gg), 
                evt.preventDefault(), 
                evt.stopPropagation();
                break;
...
function js_mouseup(evt) {
    ...
    let reset = !0;
    if (clearTimeout(gg.timer), 
    gg.state === TSTATE_IN_TOUCH) if (g_report_tap_double) gg.state = TSTATE_AFTER_TOUCH, 
    gg.timer = setTimeout(js_mouse_timeout, DOUBLE_TAP_DELAY, gg), 
    TRACE_MOUSE_LOW && log("  start timer for double tap"), 
    reset = !1; else {
        switch (TRACE_MOUSE_LOW && log("  treating as EV_TAP"), 
        gg.button) {
          case 0:
            evkind = EV_TAP;
            break;
...
function js_wheel(evt) {
    if (null != g_root_block) {
        ...
        setv(_M, WHERE_LOOM, event, F_evkind, EV_WHEEL), 
        setv(_M, WHERE_LOOM, event, F_when, elapsed), 
        setv(_M, WHERE_LOOM, event, F_global_x, evt.pageX), 
        setv(_M, WHERE_LOOM, event, F_global_y, evt.pageY), 
        setv(_M, WHERE_LOOM, event, F_z, evt.deltaY), 
        TRACE_XY && log(str.conv("about to call visit_all, mouse=({n},{n})", evt.pageX, evt.pageY)), 
        visit_all_containing(g_root_block, evt.pageX, evt.pageY, EV_WHEEL, event);
    }
    evt.stopPropagation();
}
...
function js_mousemove(evt) {
    ...
    var gg = g_mouse[evt.button];
    let distance;
    gg.mod_shf = evt.shiftKey, 
    gg.mod_alt = evt.altKey, 
    gg.mod_cmd = evt.metaKey, 
    gg.mod_ctl = evt.ctrlKey, 
    TRACE_MOUSE_MOVE && log(str.conv("at {n3} js_mousemove, mouse=[{},{}], state={}", elapsed_raw, evt.pageX, evt.pageY, gg.state)), 
    gg.state === TSTATE_IN_TOUCH ? (gg.cum_pixels += Math.abs(evt.pageX - gg.lastx) + Math.abs(evt.pageY - gg.lasty), 
    distance = 25.4 * gg.cum_pixels / SCREEN_DPI, 
    TRACE_MOUSE_MOVE && log(str.conv("  cum pixels={n} mm={n3} threshold={n}", gg.cum_pixels, distance, DRAG_THRESHOLD_MM)), 
    distance >= DRAG_THRESHOLD_MM && (gg.state = TSTATE_IN_DRAG, 
    clearTimeout(gg.timer), 
    do_pointer_event(EV_DRAG_BEGIN, elapsed, gg.originx, gg.originy, gg), 
    do_pointer_event(EV_DRAG_MOVE, elapsed, evt.pageX, evt.pageY, gg))) : gg.state === TSTATE_IDLE ? do_pointer_event(EV_HOVER, elapsed, evt.pageX, evt.pageY, gg) : gg.state === TSTATE_IN_DRAG && (TRACE_MOUSE_MOVE && log("  in drag, send to do_pointer"), 
    do_pointer_event(EV_DRAG_MOVE, elapsed, evt.pageX, evt.pageY, gg)), 
    gg.lastx = evt.pageX, 
    gg.lasty = evt.pageY;
}
...
function js_mouse_timeout(event, gg) {
    if (null !== gg) switch (gg.state) {
      case TSTATE_IN_TOUCH:
        g_report_tap_hold ? (TRACE_MOUSE_LOW && log("  touch timer complete, converting to TAP_HOLD"), 
        do_pointer_event(EV_HOLD, elapsed, gg.originx, gg.originy, gg), 
        gg.state = TSTATE_IN_HOLD) : (TRACE_MOUSE_LOW && log("  touch timer complete, converting to TAP_SINGLE"), 
        do_pointer_event(EV_TAP, elapsed, gg.originx, gg.originy, gg), 
        gg.state = TSTATE_IDLE);
        break;

      case TSTATE_AFTER_TOUCH:
        TRACE_MOUSE_LOW && log("  touch timer complete, emit TAP_SINGLE"), 
        do_pointer_event(EV_TAP, elapsed, gg.originx, gg.originy, gg), 
        gg.state = TSTATE_IDLE;
        break;
...
function js_keydown(event) {
    ...
    if (null != active) {
        ...
        if ("input" == active.localName && event.keyCode != KEYCODE_ESC) return event.keyCode === KEYCODE_ENTER && (TRACE_KEYS && log("  enter key, try to close"), 
        active.blur()), void (TRACE_KEYS && log(`--js_keydown passing to browser input, active=${active.localName}`));
    }
...
function on_enter_frame(timestamp) {
    clock_tick(), loom_process_events(), 
    SKIP_ANIMATION || requestAnimationFrame(on_enter_frame);
}

function canv_fill(ctx, box, fill, grad, tile) {
    ctx.fillStyle = null != tile ? ctx.createPattern(fill.sysobj, "repeat") : null != grad ? canvas_define_gradient(ctx, box, grad) : js_color_to_str(fill), 
    ctx.fill();
}
...
export function new_block(arg_parent, arg_bounds, arg_kind, arg_label, arg_drawer = null, arg_dest = FOR_SCREEN) {
    ...
    arg_kind == BKIND_SLICE_H ? (newb.style.position = "absolute", 
    newb.style.left = to_px(arg_bounds.left), 
    newb.style.top = "0px", 
    arg_bounds.width == U ? newb.style.width = "auto" : newb.style.width = to_px(arg_bounds.width), 
    newb.style.height = to_px(arg_bounds.height)) : arg_kind == BKIND_SLICE_V ? (newb.style.position = "absolute", 
    newb.style.left = "0px", 
    newb.style.top = to_px(arg_bounds.top), 
    newb.style.width = to_px(arg_bounds.width), 
    arg_bounds.height == U ? newb.style.height = "auto" : newb.style.height = to_px(arg_bounds.height)) : (newb.style.position = "absolute", 
    newb.style.left = to_px(arg_bounds.left), 
    newb.style.top = to_px(arg_bounds.top), 
    newb.style.width = to_px(arg_bounds.width), 
    newb.style.height = to_px(arg_bounds.height));
    var cc, net = null;
    return "string" == typeof arg_label && ("@" != (cc = arg_label.charAt(0)) && "_" != cc || (net = arg_label)), 
    null === net && ("$" == (cc = (net = arg_drawer.name).charAt(0)) ? newb.style.overflow = "scroll" : "_" != cc && (net += ":" + String(g_next_bid))), 
    newb.id = net, newb.blabel = net, 
    newb.bid = g_next_bid++, 
    newb.bkind = arg_kind, 
    newb.bseq = 0, newb.locator = 0, 
    newb.dest = arg_dest, 
    newb.old_scrollh_pos = 0, 
    newb.old_scrollv_pos = 0, 
    null === arg_parent ? (newb.bflags = 0, 
    newb.dpi = SCREEN_DPI) : (newb.bflags = arg_parent.bflags, 
    newb.dpi = arg_parent.dpi, 
    arg_parent.appendChild(newb)), 
    newb.nesting = 0, newb.drawer = arg_drawer, 
    newb.usesz = [], newb.is_inside = !1, 
    newb.div = null, newb.tbl = null, 
    newb.extra = new a_tree(_M, "$a_block.extra"), 
    setv(_M, 0, newb.extra, F_bname, net), 
    k_set_bounds(newb, arg_bounds), 
    newb;
};
...
export function a_micro() {
    this.when = U, this.seq = g_currseq, 
    this.module = null, 
    this.where = 0, this.kind = 0, 
    this.path = null, this.oldval = null, 
    this.newval = null;
};
...
export function pathx_to_hard(pathx) {
    var path, lev, ix = 0;
    for (lev of pathx.lev) 0 == ix ? path = new a_path(lev) : path.key.push(lev.sub), 
    ix++;
    return path;
};
...
export function pathx_to_soft(pathx) {
    ...
    for (lev of (path.base = pathx.lev[0].sub, 
    pathx.lev)) 0 == ix ? path.base = lev : path.key.push(lev.sub), 
    ix++;
    return path;
};

export function micro_add_delta(arg_kind, arg_module, arg_where, arg_pathx, arg_oldval, arg_newval) {
    if (!CHECKS || 0 != arg_where && 0 != arg_module || argument_err("log_delta requires where"), 
    arg_pathx.lev[0].eflags & NF_LOGGED) {
        var rec = {
            when: elapsed,
            seq: g_currseq,
            module: arg_module,
...
function add_major_step(evkind) {
    ...
    return TRACE_MAJOR && log(str.conv("add_major_step at {}, kind={}, rawx={}", sx, evkind, g_micro.length)), 
    setv(_M, 0, runtime, F_major_steps, sx, F_evkind, evkind), 
    setv(_M, 0, runtime, F_major_steps, sx, F_when, elapsed), 
    setv(_M, 0, runtime, F_major_steps, sx, F_hist_rawx, g_micro.length), 
    setv(_M, 0, runtime, F_major_stepx, sx + 1), 
    sx;
}

export function add_major_callback(elem) {
    let sx = add_major_step(EV_TIMER);
    TRACE_MAJOR && log(str.conv("add_major_callback at {}, func={}", sx, elem.func.name)), 
    setv(_M, 0, runtime, F_major_steps, sx, F_callback, elem.func);
};
...
export function add_major_event(arg_event, absorber) {
    ...
    if (TRACE_MAJOR) {
        let ss = "null";
        null != absorber && (ss = absorber.blabel), 
        log(str.conv("add_major_event[{}] evkind={}, absorber={}, rawx={}", sx, getn(arg_event, F_evkind), ss, g_micro.length));
    }
    return setv(_M, 0, runtime, F_major_steps, sx, F_hist_rawx, g_micro.length), 
    setv(_M, 0, runtime, F_major_stepx, sx + 1), 
    null != absorber && setv(_M, 0, runtime, F_major_steps, sx, F_hist_absorber, absorber.blabel), 
    sx;
};
...
export function a_loom_element() {
    this.func = null, this.nreps = 1, 
    this.countx = 0, this.last_seen = 0, 
    this.field_vec = [];
};
...
export function a_div_slice() {
    this.box = null, this.id = U, 
    this.backcolor = U, 
    this.npasses = 1, this.min_pt = U, 
    this.max_pt = U, this.drawer = null;
};

export function a_div_spec(arg_gridf = null, arg_tblr = !1, arg_scrollh = !1, arg_scrollv = !1) {
    this.horz_slices = [], 
    this.vert_slices = [], 
    this.scrollh = arg_scrollh, 
    this.scrollv = arg_scrollv, 
    this.open = !0, this.grid_func = arg_gridf, 
...
function str_to_unitxx(compact) {
    ...
    TRACE_UNITX && log("expand compact=" + compact), 
    CHECKS && "" == compact && internal_err("unit string missing from measure");
    let chunk, pos_caret, pos_bar, term, chunks = compact.split(",");
    for (chunk of chunks) term = {}, 
    pos_caret = chunk.indexOf("^"), 
    term.name = chunk.substring(0, pos_caret), 
    (pos_bar = (exp = chunk.substring(pos_caret + 1)).indexOf("|")) <= 0 ? (term.num = Math.floor(exp.substring(0, pos_bar)), 
    term.den = Math.floor(exp.substring(pos_bar + 1))) : (term.num = Math.floor(exp), 
    term.den = 1), unitx.terms.push(term);
    return unitx;
}

a_unitxx.prototype.unitxx_to_str = function() {
    let term;
    if (this.terms.sort(compare_unit_terms), 
    TRACE_UNITX) for (term of (log("unitxx_to_str start, terms="), 
    this.terms)) log("  term.name=" + term.name + ", " + String(term.num) + " / " + String(term.den));
    let ss = "", first = !0;
    for (term of this.terms) if (0 !== term.num) {
        for (;;) {
            let active = !1;
            if (term.num % 2 == 0 && term.den % 2 == 0 && (term.num /= 2, 
            term.den /= 2, active = !0), 
            term.num % 3 == 0 && term.den % 3 == 0 && (term.num /= 3, 
            term.den /= 3, active = !0), 
            term.num % 5 == 0 && term.den % 5 == 0 && (term.num /= 5, 
            term.den /= 5, active = !0), 
            !active) break;
        }
        first ? first = !1 : ss += ",", 
        ss += term.name + "^" + String(term.num), 
        1 != term.den && (ss += "|" + String(term.den));
    }
    return TRACE_UNITX && log("unitxx_to_str end, unitss=" + ss), 
    ss;
    ...
    null === existing_term ? this.terms.push(newterm) : (1 != newterm.den && (existing_term.num *= newterm.den, 
...
export function ByteArray(arg_len, arg_order) {
    ...
    this.position = 0, this.little = arg_order != BIG_ENDIAN;
};
...
export function a_loop_step() {
    this.index = null, this.val = null, 
    this.key = null, this.leaf = null, 
    this.path = null;
};

export function a_loop_level() {
    this.nodes = null, this.nx = null, 
    this.curnode = null;
};

export function a_loop(options) {
    for (var property in this.count = 0, 
    this.kind = KIND_DEFAULT, 
    this.index = U, this.range_from = U, 
    this.range_to = INFINITY, 
    this.range_delta = 1, 
    this.reps = INFINITY, 
    this.trap = !1, this.rev = !1, 
    this.method = METHOD_INFINITE, 
    this.done = !0, this.include_root = !1, 
    this.recursive_field = U, 
    this.sort_mode = 0, 
    this.sort_field = U, 
    this.levels = [], options) switch (property) {
      case "across":
        this.set_basis(options.across), 
        this.method = METHOD_ACROSS;
        break;

      case "by":
        if (this.range_delta = options.by, 
        is_numeric(this.range_delta) != Y) return void argument_err("bad 'delta' value");
        if (this.range_delta <= 0) return void argument_err("delta must be positive");
        break;

      case "from":
        if (this.range_from = options.from, 
        is_numeric(this.range_from) != Y) return void argument_err("bad from: value");
        this.method = METHOD_RANGE;
        break;

      case "bottom_up":
        this.set_basis(options.bottom_up), 
        this.method = METHOD_BOT_UP;
        break;

      case "recursive":
        this.set_basis(options.recursive), 
      ...
      case "top_down":
        this.set_basis(options.top_down), 
      ...
      case "root":
        this.include_root = options.root === Y;
        break;
      ...
      case "to_":
        if (this.range_to = options.to_, 
        is_numeric(this.range_to) != Y) return void argument_err("bad 'to:' value");
        break;
    ...
    switch (this.method) {
      ...
      case METHOD_ACROSS:
        if (this.base_node = path_to_node(this.basis, !0), 
        !this.deeper_level(this.base_node, !1)) return;
      ...
      case METHOD_RECURSIVE:
        if (this.base_node = path_to_node(this.basis, !0), 
        null == this.base_node || null == this.base_node.children) return;
        this.lev = new a_loop_level(), 
        this.lev.curnode = this.base_node, 
        this.state = STATE_EMIT;
        break;
    ...
    this.done = !1, 0 != this.sort_mode && (this.sort_list = new Array(), 
    this.build_loop_steps(), 
    null == this.sort_func ? this.sort_list.sort(default_sort_func) : (g_sort_func = this.sort_func, 
    this.sort_list.sort(custom_sort_func)), 
    this.done = !1, this.method = METHOD_PASS2, 
    this.count = 0);
};

function is_in_kindset(sub, kind) {
    return "string" == typeof sub ? kind & KIND_STR : is_enum(sub) == Y ? kind & KIND_ENUM : kind & KIND_INT;
}
...
a_loop.prototype.set_basis = function(basearg) {
    basearg instanceof a_tree ? this.basis = adr(basearg) : basearg instanceof a_path ? this.basis = basearg : argument_err("loop target must be a_path or a_tree"), 
    ...
    return newlev.nodes = this.get_nodes_of_kind(node.children), 
    0 != newlev.nodes.length && (this.rev ? newlev.nx = newlev.nodes.length - 1 : newlev.nx = 0, 
    newlev.curnode = newlev.nodes[newlev.nx], 
    recursive && this.path.key.push(this.recursive_field), 
    this.path.key.push(newlev.curnode.sub), 
    this.levels.push(newlev), 
    this.lev = newlev, !0);
    ...
    if (this.include_root && 0 == this.count) return this.leaf = this.base_node, 
    ...
    outer: for (;;) {
        switch (this.state) {
          case STATE_EMIT:
            return this.update_loop_vars(), 
            console.log("  returning node at " + str.to_str(this.path)), 
            this.state = STATE_CHILDREN, 
            !0;
    ...
    for (;;) {
        switch (this.state) {
          case STATE_EMIT:
            if (this.update_loop_vars(), 
            !this.include_root && this.leaf == this.base_node) break;
            return console.log("  returning node at " + str.to_str(this.path)), 
            this.state = 2, !0;
    ...
    if (observe_reps && ge2(this.count, this.reps)) return this.trap && internal_err("Aborting because loop limit of " + String(this.reps) + " was reached"), 
    this.done = !0, !1;
    switch (this.method) {
      ...
      case METHOD_RANGE:
        if (this.rev) {
            if (this.index -= this.range_delta, 
            this.index < this.range_from) return this.done = !0, 
            !1;
        } else if (this.index += this.range_delta, 
        this.index > this.range_to) return this.done = !0, 
        !1;
        break;

      case METHOD_ACROSS:
        if (!this.next_across()) return this.done = !0, 
        !1;
        break;

      case METHOD_TOP_DOWN:
        if (!this.next_topdown()) return this.done = !0, 
        !1;
        break;

      case METHOD_BOT_UP:
        if (!this.next_bottomup()) return this.done = !0, 
        !1;
        break;

      case METHOD_RECURSIVE:
        if (!this.next_recursive()) return this.done = !0, 
        !1;
        break;

      case METHOD_PASS2:
        var rec;
        if (this.count >= this.sort_list.length) return this.done = !0, 
        !1;
        rec = this.rev ? this.sort_list[this.sort_list.length - 1 - this.count] : this.sort_list[this.count], 
        this.index = rec.index, 
        this.val = get_leaf_value(rec.val), 
        this.leaf = rec.leaf, 
        this.path = rec.path;
        break;
    ...
    this.leaf = this.lev.curnode, 
    this.index = this.leaf.sub, 
    this.val = get_leaf_value(this.leaf.val);
    ...
    if (this.rev) {
        if (this.lev.nx -= 1, 
        this.lev.nx < 0) return !1;
    } else if (this.lev.nx += 1, 
    this.lev.nx >= this.lev.nodes.length) return !1;
    return this.lev.curnode = this.lev.nodes[this.lev.nx], 
    !0;
}, a_loop.prototype.build_loop_steps = function() {
    for (var rec, ix = 0; this.next(!1); ) {
        switch ((ix += 1) > 1e5 && internal_err("sanity check error: 100k sort item limit reached in loop."), 
        (rec = new a_loop_step()).index = this.index, 
        rec.val = get_leaf_value(this.val), 
...
export function a_tree(arg_mod, arg_label, arg_nodeflags = 0, arg_initval = null) {
    this.children = null, 
    this.val = U, this.eflags = EF_VAL_IS_U | arg_nodeflags, 
    this.seq = 0, this.sub = 0, 
    this.label = arg_label, 
    0 != (arg_nodeflags & NF_TOPLEVEL) && (this.sub = str.str_hash_fnv(arg_label + ":" + MODULES[arg_mod]), 
    TREES[this.sub] = this), 
    null != arg_initval && merge_tree(arg_mod, 0, arg_initval, adr(this));
};
...
export function a_loom_anim_curve() {
    this.phase = 0, this.draw_end_time = 0, 
...
a_path.prototype.clone = function() {
    ...
    return result.base = this.base, 
    result.key = this.key.concat(), 
    result;
}, a_path.prototype.clone_to_soft = function() {
    var result = new a_path();
    return result.base = this.base.sub, 
    result.key = this.key.concat(), 
    result;
}, a_path.prototype.clone_to_hard = function() {
    var result = new a_path();
    return result.base = TREES[this.base], 
    void 0 !== result.base && null !== result.base || argument_err("A soft path should have a label in the base field"), 
    result.key = this.key.concat(), 
    result;
};
...
export function a_image(arg_url, arg_hint = "", arg_preload = !1, arg_h = 0, arg_v = 0) {
    ...
    if (this.tile_width = arg_h, 
    this.tile_height = arg_v, 
    null === arg_url) this.sysobj = null; else {
        var tag = document.createElement("img");
        tag.url = arg_url, arg_preload && (tag.onload = function callback_on_load() {
...
export function image_size(img) {
    ...
    if (null != img) {
        ...
        0 == image.naturalWidth || (setv(_M, 0, result, F_x, image.naturalWidth), 
        setv(_M, 0, result, F_y, image.naturalHeight));
    }
    return result;
};
...
export function a_meas() {
    this.mag = U, this.unit = U, 
    this.family = U, this.unitss = null;
};

a_meas.prototype.clone = function() {
    let my = new a_meas();
    return my.mag = this.mag, 
    my.unit = this.unit, 
    my.family = this.family, 
    my.unitss = this.unitss, 
    my;
};

export function a_function(arg_modnum = 0, arg_name = "", arg_code = null, arg_track = null, arg_parms = null) {
    if (this.modnum = arg_modnum, 
    this.name = arg_name, 
    this.code = arg_code, 
    this.tracker = arg_track, 
    CHECKS && null === arg_code) debugger;
    this.hash = str.str_hash_fnv(arg_name + ":" + MODULES[arg_modnum]), 
    this.parms = arg_parms;
};
...
export function a_sound(src) {
    if (this.loopf = !1, 
    this.notify = !1, this.id = 0, 
    this.pause_position = 0, 
    this.curx = 0, this.numx = 1, 
    IS_NODE) this.audio = null; else {
        let myaudio = new Audio(src);
        myaudio.preload = "auto", 
        myaudio.load(), this.audio = [ myaudio ];
    }
};

export function new_changelist(...args) {
    for (var find, repl, list = new a_tree(_M, "$new_changelist"), nargs = args.length, argx = 0, slotx = 1; argx < nargs; ) find = args[argx++], 
    repl = args[argx++], 
    setv(_M, 0, list, slotx, str.F_find_patt, find), 
    setv(_M, 0, list, slotx, str.F_repl_patt, repl), 
    slotx += 1;
    return null;
};
...
export function solve_rectR(options) {
    ...
    if (null != basis && (dx != U && (basis = new Rectangle(basis.left + dx, basis.top, basis.width, basis.height)), 
    dy != U && (basis = new Rectangle(basis.left, basis.top + dy, basis.width, basis.height)), 
    inset != U && (basis = new Rectangle(basis.left + inset, basis.top + inset, basis.width - 2 * inset, basis.height - 2 * inset)), 
    inset_y != U && (basis = new Rectangle(basis.left, basis.top + inset_y, basis.width, basis.height - 2 * inset_y)), 
    inset_x != U && (basis = new Rectangle(basis.left + inset_x, basis.top, basis.width - 2 * inset_x, basis.height)), 
    inset_n != U && (basis = new Rectangle(basis.left, basis.top + inset_n, basis.width, basis.height - inset_n)), 
    inset_s != U && (basis = new Rectangle(basis.left, basis.top, basis.width, basis.height - inset_s)), 
    inset_e != U && (basis = new Rectangle(basis.left, basis.top, basis.width - inset_e, basis.height)), 
    inset_w != U && (basis = new Rectangle(basis.left + inset_w, basis.top, basis.width - inset_w, basis.height))), 
    null != basis) {
        switch ((pin - 1) % 3) {
          case 0:
            left = basis.left;
...
export function load_code(mod_name, mod_enum, mod_hash) {
    ...
    debugger;
    null === lines && (lines = []).push(mod_name), 
...
export function collate_num(a, b) {
    ...
    -1) : cat_a > cat_b ? (TRACE_COLLATE && log(str.conv("collate_num, a={}, b={}, category +1", a, b)), 
    1) : 0 === cat_a ? a < b ? (TRACE_COLLATE && log(str.conv("collate_num, a={}, b={}, result=-1", a, b)), 
    -1) : a > b ? (TRACE_COLLATE && log(str.conv("collate_num, a={}, b={}, result=+1", a, b)), 
    1) : (TRACE_COLLATE && log(str.conv("collate_num, a={}, b={}, result=0", a, b)), 
    0) : 2 === cat_a ? (TRACE_COLLATE && log(str.conv("collate_num, a={}, b={} enum", a, b)), 
    collate_str(str.enum_to_str(a), str.enum_to_str(b))) : (TRACE_COLLATE && log(str.conv("collate_num, a={}, b={}, other", a, b)), 
    0);
};
...
export function collate_open(a, b) {
    ...
    return TRACE_COLLATE && log(str.conv("collate open, equal, a={}, b={}", a, b)), 
    0;
};
...
export function q_start(on_done = null, on_progress = null) {
    TRACE_TASKS && log("## q_start, ntasks=" + String(q_tasks.length)), 
    q_donef = on_done, q_progressf = on_progress, 
    q_taskx = 0, q_next_task();
};

function q_next_task() {
    let mytask, ntasks = q_tasks.length;
    q_taskx < ntasks ? (TRACE_TASKS && log(str.conv("## task {int}/{int} running", q_taskx, ntasks)), 
    (mytask = q_tasks[q_taskx]).state = a_qtask.TASK_BUSY, 
    q_loader = new a_loader_x(mytask.folder, mytask.fname, TYPE_BYTES, on_task_complete, Y), 
    null !== q_progressf && q_progressf(q_taskx, ntasks)) : (TRACE_TASKS && log("## q_tasks all done"), 
    null !== q_donef && q_donef());
}

function on_task_complete(okay, data) {
    okay === Y ? (TRACE_TASKS && log(str.conv("## task {int} okay", q_taskx)), 
    q_tasks[q_taskx].state = a_qtask.TASK_DONE) : (TRACE_TASKS && log(str.conv("## task {int} failed", q_taskx)), 
...
function rect_intersect(rect1, rect2) {
    ...
    netl = r1l > r2l ? r1l : r2l, 
    nett = r1t > r2t ? r1t : r2t, 
    netr = r1r < r2r ? r1r : r2r, 
    netb = r1b < r2b ? r1b : r2b;
    let result = new a_tree(_M, "$rect_intersect");
    return setv(_M, 0, result, F_left, netl), 
    setv(_M, 0, result, F_top, nett), 
    setv(_M, 0, result, F_width, netr - netl), 
    setv(_M, 0, result, F_height, netb - nett), 
    result;
}
...
export function rect_is_portrait(r) {
    ...
    return is_err_enum_b(h) || is_err_enum_b(v) ? (CHECKS && argument_err("bad rectangle"), 
    ERR) : h === U || v === U ? U : h < v ? Y : N;
};
...
function rect_is_intersecting(rect1, rect2) {
    ...
    return is_err_enum_b(r1l) || is_err_enum_b(r1t) || is_err_enum_b(r1w) || is_err_enum_b(r1h) || is_err_enum_b(r2l) || is_err_enum_b(r2t) || is_err_enum_b(r2w) || is_err_enum_b(r2h) ? ERR : r1l === U || r1t === U || r1w === U || r1h === U ? U : r2l === U || r2t === U || r2w === U || r2h === U ? U : r1l + r1w <= r2l || r2l + r2w <= r1l || r1t + r1h <= r2t || r2t + r2h <= r1t ? N : Y;
}
...
export function rect_is_inside(r, options) {
    ...
    for (let property in options) switch (property) {
      ...
      case "xy":
        p1x = getn(options.xy, F_x), 
        p1y = getn(options.xy, F_y);
        break;
...
export function rect_as_tree(left, top, width, height) {
    ...
    return setv(_M, 0, t, F_left, left), 
    setv(_M, 0, t, F_top, top), 
    setv(_M, 0, t, F_width, width), 
    setv(_M, 0, t, F_height, height), 
    t;
};
...
export function add_family(family, dimensions, nonlinear = N) {
    nonlinear === Y && setv(_M, 0, families, family, F_fam_nonlinear, Y), 
    "" !== dimensions && setv(_M, 0, families, family, F_fam_dim, dimensions);
};

export function add_unit(family, unit, factor, base_to_unitf = null, unit_to_basef = null) {
    setv(_M, 0, unit_to_family, unit, family), 
    1 === factor && setv(_M, 0, families, family, F_fam_base, unit), 
    null === unit_to_basef ? setv(_M, 0, families, family, F_fam_units, unit, F_unit_factor, factor) : (setv(_M, 0, families, family, F_fam_units, unit, F_base_to_unit, base_to_unitf), 
...
export function xy_in_rect(x, y, box) {
    return x -= box.left, 
    y -= box.top, x >= 0 && y >= 0 && x < box.width && y < box.height;
};
...
export function add(a, b) {
    if ("number" == typeof a && "number" == typeof b) return is_err_enum_b(a) || is_err_enum_b(b) ? (TRAP_ERR && internal_err("bad arg"), 
    ERR) : a === U || b === U ? U : a + b;
    if (a instanceof a_meas && b instanceof a_meas && a.unitss === b.unitss) {
        let c = a.clone();
        return c.mag = a.mag + b.mag, 
        c;
    }
    return TRAP_ERR && internal_err("bad arg"), 
    ERR;
};
...
export function mul(a, b) {
    ...
    if (a_is_num && b_is_num) return Number.isNaN(a) || Number.isNaN(b) ? (TRAP_ERR && internal_err("bad arg"), 
    ERR) : (a === Y ? a = 1 : a === N && (a = 0), 
    b === Y ? b = 1 : b === N && (b = 0), 
    0 === a || 0 === b ? 0 : a === U || b === U ? U : a * b);
    let cm, a_is_mea = a instanceof a_meas, b_is_mea = b instanceof a_meas;
    if (a_is_mea && b_is_mea) {
        (cm = new a_meas()).family = U, 
        cm.mag = a.mag * b.mag, 
        cm.unit = U;
        let term, ax = a_unitxx.str_to_unitxx(a.unitss), bx = a_unitxx.str_to_unitxx(b.unitss);
        for (term of bx) ax.add_unit_term(term, 1);
        return cm.unitss = ax.unitxx_to_str(), 
        cm;
    }
    return a_is_num && b_is_mea ? is_err_enum_b(a) ? (TRAP_ERR && internal_err("bad arg"), 
    ERR_MEAS) : (cm = b.clone(), 
    a === Y || (cm.mag = a === N || 0 === a ? 0 : a === U ? U : b.mag * a), 
    cm) : a_is_mea && b_is_num ? is_err_enum_b(b) ? (TRAP_ERR && internal_err("bad arg"), 
    ERR_MEAS) : (cm = a.clone(), 
    b === Y || (cm.mag = b === N || 0 === b ? 0 : b === U ? U : a.mag * b), 
    cm) : (TRAP_ERR && internal_err("bad arg"), 
    ERR);
};
...
export function div(a, b) {
    ...
    if (a_is_num && b_is_num) {
        if (0 === a) return 0;
        if (is_err_enum_b(a) || is_err_enum_b(b)) return TRAP_ERR && internal_err("bad arg"), 
        ERR;
        if (a === U || b === U) return U;
        let cn;
        return cn = a / b, Number.isNaN(cn) ? (TRAP_ERR && internal_err("bad arg"), 
        ERR) : cn;
    }
    let cm, term, a_is_mea = a instanceof a_meas, b_is_mea = b instanceof a_meas;
    if (a_is_mea && b_is_mea) {
        if ((cm = new a_meas()).family = U, 
        cm.unit = U, 0 === a.mag) cm.mag = 0; else if (cm.mag = a.mag / b.mag, 
        Number.isNaN(cm.mag)) return ERR_MEAS;
    ...
    if (a_is_num && b_is_mea) {
        if (is_err_enum_b(a)) return TRAP_ERR && internal_err("bad arg"), 
        ERR_MEAS;
        if (0 === a) return 0;
        if (a === U) return U;
        if ((cm = new a_meas()).family = U, 
        cm.unit = U, 0 === a) cm.mag = 0; else if (cm.mag = a / b.mag, 
        Number.isNaN(cm.mag)) return ERR_MEAS;
        let nmx = str_to_unitxx(b.unitss);
        for (term of nmx) term.num = -term.num;
        return cm.unitss = nmx.unitxx_to_str(), 
        cm;
    }
    return a_is_mea && b_is_num ? is_err_enum_b(b) ? (TRAP_ERR && internal_err("bad arg"), 
    ERR) : b === U ? U : ((cm = a.clone()).mag = a.mag / b, 
    Number.isNaN(cm.mag) ? ERR_MEAS : cm) : (TRAP_ERR && internal_err("bad arg"), 
    ERR);
};
...
export function negate(a) {
    return is_err_enum_b(a) ? (TRAP_ERR && internal_err("bad arg"), 
    ERR) : a === U ? U : -a;
};

export function meas(arg_mag, arg_unitx, arg_familyx) {
    let factor, convfunc, result = new a_meas();
    return result.family = arg_familyx, 
    result.unit = arg_unitx, 
    arg_unitx !== U ? (result.unitss = gets(families, arg_familyx, F_fam_dim), 
    (factor = getn(families, arg_familyx, F_fam_units, arg_unitx, F_unit_factor)) === U ? (convfunc = get_func(families, arg_familyx, F_fam_units, arg_unitx, F_unit_to_base), 
    result.mag = convfunc.code(arg_mag)) : result.mag = 1 === factor ? arg_mag : mul(arg_mag, factor)) : result.mag = arg_mag, 
    result;
};

export function negate_meas(a) {
    (void 0).mag = negate(a.mag), 
    (void 0).family = a.family, 
    (void 0).unit = a.unit, 
    (void 0).unitss = a.unitss;
};

export function mul_meas_n(a, b) {
    let result = a.clone();
    return result.mag = mul(a.mag, b), 
    result;
};
...
export function eq4(a, b) {
    ...
    switch (typea) {
      ...
      case TYPE_FUNC:
        return typeb != TYPE_FUNC ? ERR : a.code == b.code ? Y : N;
...
export function ne4(a, b) {
    ...
    switch (typea) {
      ...
      case TYPE_FUNC:
        return typeb != TYPE_FUNC ? ERR : a.code == b.code ? N : Y;
...
export function args_to_path(nskip, list) {
    ...
    if (first instanceof a_path) {
        if (path = first.clone(), 
        1 == nparms) return path;
        startx = 1;
    } else first instanceof a_tree ? (path = new a_path(first), 
    startx = 1) : argument_err("bad arg");
    for (ix = startx; ix < nparms; ix++) ((sub = list[ix]) == U || Number.isNaN(sub)) && (path.key.push(sub), 
    argument_err("not allowed to use U or ERR as a subscript, path=" + str.to_str(path))), 
...
export function adr(first, ...list) {
    ...
    for (ix = 0; ix < nparms; ix++) {
        if ((sub = list[ix]) === U || sub != sub) {
            ...
            for (sx of list) flag ? flag = !1 : msg += ", ", 
            msg += str.to_str(sx);
            argument_err("bad subscript in addr: " + (msg += "]"));
        }
        sub === FOL ? 0 == path.key.length || (null != (link = path_getv(path, EF_VAL_IS_PATH)) && link != ERR_PATH || argument_err("indirecting on something that is not a pointer"), 
        path = link) : path.key.push(sub);
    }
    return path;
};
...
export function addr_head(src, ndrop) {
    let result;
    return 0 === ndrop ? src : ((result = src.clone()).key.length = result.key.length - ndrop, 
    result);
};

export function addr_tail(path, nsteps) {
    let ix = path.key.length - 1 - nsteps;
    return ix >= 0 ? path.key[ix] : (CHECKS && argument_err("gone off the head of the path"), 
    U);
};

export function alloc_derived(loc, ...args) {
    var elem, pathx, path, lx;
    lx = (pathx = path_create_pathx(path = args_to_path(1, args))).lev.length - 1, 
    (elem = pathx.lev[lx].elem).derivef = args[args.length - 1], 
    elem.eflags |= EF_DIRTY_OR_DERIVED, 
    g_currseq += 1, TRACE_SEQ && console.log(str.conv("+SEQ {} in alloc_derived {}", g_currseq, path)), 
    elem.seq = ++g_currseq, 
    TRACE_SEQ && log(str.conv("+SEQ {} in alloc_derived {}", g_currseq, path));
};
...
export function toggle(module, loc, ...args) {
    ...
    val = val == Y ? N : val == N || val == U ? Y : ERR, 
    TRACE_LOG && 0 !== loc && log(">> toggle"), 
...
export function touch(mod, loc, path) {
    if (null != path) {
        ...
        is_stateful && (TRACE_SEQ && console.log(str.conv("+SEQ {} in touch, path={}", g_currseq, path)), 
        pathx.lev[pathx.lev.length - 1].seq = ++g_currseq), 
        is_logged && micro_add_delta(LOGK_TOUCH, mod, loc, pathx, null, null);
    }
};

export function inc(module, loc, ...args) {
    let pathx = path_create_pathx(args_to_path(0, args)), val = leaf_getv(pathx, EF_VAL_IS_NUM);
    val = val == U ? 1 : add(val, 1), 
    TRACE_LOG && 0 !== loc && log(">> inc"), 
    setv_x(module, loc, pathx, val);
};

export function dec(module, loc, ...args) {
    let pathx = path_create_pathx(args_to_path(0, args)), val = leaf_getv(pathx, EF_VAL_IS_NUM);
    val = val == U ? -1 : sub(val, 1), 
    TRACE_LOG && 0 !== loc && log(">> dec"), 
    setv_x(module, loc, pathx, val);
};
...
export function tree_lit(module, loc, ...args) {
    ...
    for (argx = 0; argx < nargs; ) subscript = args[argx], 
    argx += 1, subscript === POP ? mypath.key.pop() : subscript == VAL || subscript == VNP ? (val = args[argx], 
    argx += 1, val instanceof a_tree ? merge_tree(module, loc, val, mypath) : val != U && path_setv(module, loc, mypath, val), 
...
export function merge_lit(module, loc, dest, ...args) {
    ...
    for (dest instanceof a_path ? mypath = dest.clone() : dest instanceof a_tree ? mypath = adr(dest) : argument_err("expected a_tree, or a_path"), 
    argx = 0; argx < nargs; ) {
...
export function table_lit(...args) {
    ...
    for (argx = 0; field = args[argx], 
    argx += 1, field !== VAL; ) fields.push(field);
    for (fieldx = 0, rowx = 1; argx < nargs; ) val = args[argx], 
    argx += 1, val === VAL ? (rowx += 1, 
    fieldx = 0) : (setv(_M, 0, mytree, rowx, fields[fieldx], val), 
    fieldx += 1);
    return mytree;
};
...
export function path_to_pathx(path) {
    ...
    px.basepath = path, 
    px.exists = !0, px.lev.push(path.base);
...
export function path_create_pathx(path) {
    ...
    for (targsub of (px.lev.push(currnode), 
    path.key)) null == currnode.children && (currnode.children = []), 
    null == (subnode = currnode.children[targsub]) && (!added && path.base.eflags & NF_STATEFUL && (added = !0, 
    g_currseq += 1), (subnode = new a_tree(_M, "$path_create_pathx")).seq = g_currseq, 
    subnode.sub = targsub, 
    currnode.seq = g_currseq, 
    currnode.children[targsub] = subnode, 
    added = !0), px.lev.push(subnode), 
    currnode = subnode;
    return px.exists = !0, 
    px;
};
...
export function path_to_node(path, depend = !1) {
    ...
    return node = px.exists ? px.lev[px.lev.length - 1] : null, 
    depend && null != g_curr_block && path.base.eflags & NF_STATEFUL && add_path_dependency(path, node), 
    node;
};
...
export function add_path_dependency(path, node) {
    ...
    dep.path = path.clone(), 
    dep.seq = null != node ? node.seq : 0, 
    TRACE_IMPLIED && log(str.conv("--adding pathx add_path_dependency: {path} of seq {} in {}", dep.path, dep.seq, g_curr_block.blabel)), 
...
export function add_pathx_dependency(pathx) {
    ...
    pathx.exists ? dep.seq = pathx.lev[pathx.lev.length - 1].seq : dep.seq = 0, 
    TRACE_IMPLIED && console.log(str.conv("--adding pathx dependency: {path} of seq {} in {}", dep.path, dep.seq, g_curr_block.blabel)), 
    g_curr_block.usesz.push(dep);
};
...
export function add_node_dependency(node) {
    ...
    dep.path = adr(node), 
    dep.seq = node.seq, 
    TRACE_IMPLIED && log(str.conv("--adding node dependency: {path} of seq {} in {}", dep.path, dep.seq, g_curr_block.blabel)), 
    g_curr_block.usesz.push(dep);
};

export function leaf_getv(pathx, expected_type, add_dependency = !0) {
    if (null === pathx) return U_of_type(expected_type);
    if (null != g_curr_block && add_dependency && pathx.basepath.base.eflags & NF_STATEFUL && add_pathx_dependency(pathx), 
    !pathx.exists) return U_of_type(expected_type);
    var leaf = pathx.lev[pathx.lev.length - 1];
    return (EF_DIRTY_OR_DERIVED & leaf.eflags) === EF_DIRTY_OR_DERIVED && (internal_err("not yet"), 
    leaf.eflags &= ~EF_IS_DIRTY), 
    0 == expected_type || 0 != (leaf.eflags & expected_type) ? 0 != (leaf.eflags & EF_VAL_IS_PATH) ? leaf.val.clone_to_hard() : leaf.val : 0 != (leaf.eflags & EF_VAL_IS_U) ? U_of_type(expected_type) : 0 != (leaf.eflags & EF_VAL_IS_ERR) ? ERR_of_type(expected_type) : (TRAP_TYPE_MISMATCH && internal_err(`getv: path ${str.conv_pathx(pathx)} expected type=0x${expected_type.toString(16)} eflags=0x${leaf.eflags.toString(16)}`), 
...
export function path_setv(mod, loc, path, val) {
    var pathx;
    if (TRACE_LOG && 0 !== loc && console.log(str.conv(">> path_setv {}={}", path, val)), 
    val == U) {
        if (!(pathx = path_to_pathx(path)).exists) return;
        setv_x(mod, loc, pathx, U), 
        pathx.lev.length >= 1 && null == pathx.lev[pathx.lev.length - 1].children && orphan_upwards(pathx);
...
function setv_x(mod, loc, pathx, newval) {
    ...
    switch (newval_type) {
      case TYPE_U:
        0 == (oldflags & EF_VAL_IS_U) && (changed = !0, 
        elem.val = U, elem.eflags = oldflags & EF_CLEAR_TYPE | EF_VAL_IS_U);
        break;

      case TYPE_ERR:
        0 == (oldflags & EF_VAL_IS_ERR) && (changed = !0, 
        elem.val = ERR, elem.eflags = oldflags & EF_CLEAR_TYPE | EF_VAL_IS_ERR);
        break;

      case TYPE_NUM:
      case TYPE_ENUM:
        0 != (oldflags & EF_VAL_IS_NUM) && oldval === newval || (elem.val = newval, 
        elem.eflags = oldflags & EF_CLEAR_TYPE | EF_VAL_IS_NUM, 
        changed = !0);
        break;

      case TYPE_STR:
        0 != (oldflags & EF_VAL_IS_STR) && oldval === newval || (elem.val = newval, 
        elem.eflags = oldflags & EF_CLEAR_TYPE | EF_VAL_IS_STR, 
        changed = !0);
        break;

      case TYPE_PTR:
        0 != (oldflags & EF_VAL_IS_PATH) && is_path_eq2(oldval, newval) || (newval.base instanceof a_tree ? elem.val = newval.clone_to_soft() : elem.val = newval, 
        elem.eflags = oldflags & EF_CLEAR_TYPE | EF_VAL_IS_PATH, 
        changed = !0);
        break;

      case TYPE_MEAS:
        0 != (oldflags & EF_VAL_IS_MEAS) && is_meas_eq(oldval, newval) || (changed = !0, 
        elem.val = newval, elem.eflags = oldflags & EF_CLEAR_TYPE | EF_VAL_IS_MEAS);
        break;

      case TYPE_IMAGE:
        0 != (oldflags & EF_VAL_IS_STR) && oldval === newval || (changed = !0, 
        elem.val = newval, elem.eflags = oldflags & EF_CLEAR_TYPE | EF_VAL_IS_IMAGE);
        break;

      case TYPE_FUNC:
        0 != (oldflags & EF_VAL_IS_FUNC) && oldval === newval || (changed = !0, 
        elem.val = newval, elem.eflags = oldflags & EF_CLEAR_TYPE | EF_VAL_IS_FUNC);
        break;

      case TYPE_SOUND:
        0 != (oldflags & EF_VAL_IS_SOUND) && oldval === newval || (changed = !0, 
        elem.val = newval, elem.eflags = oldflags & EF_CLEAR_TYPE | EF_VAL_IS_SOUND);
        break;

      case TYPE_OBJECT:
        0 != (oldflags & EF_VAL_IS_OBJECT) && oldval === newval || (changed = !0, 
        elem.val = newval, elem.eflags = oldflags & EF_CLEAR_TYPE | EF_VAL_IS_OBJECT);
        break;
    ...
    changed && (is_stateful && (elem.seq = ++g_currseq, 
    TRACE_SEQ && log(str.conv("+SEQ {} in setv, path={}", g_currseq, pathx)), 
    elem.markf !== NULL_FUNC && elem.markf(pathx)), 
    is_logged && micro_add_delta(LOGK_DELTA, mod, loc, pathx, oldval, newval)), 
    changed;
}

export function append_val(mod, loc, val, dest) {
    let sub = tree_next_hi(dest, !1);
    return setv(mod, loc, dest, sub, val), 
    sub;
};
...
export function insert_val(mod, loc, val, dest) {
    ...
    if (bx.exists && bx.lev.length > 1 && is_numeric(bx.lev[bx.lev.length - 1].sub)) {
        ...
        for (var i = 0; i < naffected; i++) {
            ...
            (mov_src = dest.clone()).key[mov_src.key.length - 1] += offset - 1, 
            mov_dst = dest.clone(), 
            mov_src.key[mov_src.key.length - 1] += offset, 
            move_tree(mod, loc, mov_src, mov_dst);
        }
    }
...
function is_path_in_path(a, b) {
    if (TRACE_INPATH && log(str.conv("is_path_in_path, a={}, b={}", a, b)), 
    a.base != b.base) return TRACE_INPATH && log("  failed because base mismatch"), 
    !1;
    let blen = b.key.length;
    if (a.key.length < blen) return TRACE_INPATH && log("  failed because obvious above"), 
    !1;
    for (let i = 0; i < blen; i++) if (a.key[i] != b.key[i]) return TRACE_INPATH && log(str.conv("  failed because key {} mismatch", i)), 
    !1;
    return TRACE_INPATH && log("  INSIDE"), 
    !0;
}

export function append_tree(mod, loc, a, b) {
    let sub = tree_next_hi(b, !1);
    return merge_tree(mod, loc, a, addr_append(b, sub)), 
    sub;
};
...
export function merge_tree(mod, loc, a, b) {
    var src_path;
    if (a instanceof a_path ? src_path = a : a instanceof a_tree ? src_path = adr(a) : argument_err("bad arg"), 
    !is_path_eq(src_path, b)) {
        var ax = path_to_pathx(src_path);
        if (null !== ax && ax.exists) {
            var tempp, is_logged = loc > 0 && b.base.eflags & NF_LOGGED;
            is_logged && micro_add_marker(MARK_MERGE_BEG, mod, loc), 
            (is_path_in_path(src_path, b) || is_path_in_path(b, src_path)) && (tempp = adr(new a_tree(_M, "$merge_tree")), 
            merge_tree(_M, 0, src_path, tempp), 
            ax = path_to_pathx(tempp)), 
            merge_x(mod, loc, ax, path_create_pathx(b), 0), 
            is_logged && micro_add_marker(MARK_MERGE_END, mod, loc);
        }
    }
};
...
export function copy_tree(mod, loc, a, b) {
    ...
    !is_path_eq(src_path, b)) {
        ...
        is_logged && micro_add_marker(MARK_COPY_BEG, mod, loc), 
        (is_path_in_path(src_path, b) || is_path_in_path(b, src_path)) && (merge_tree(mod, loc, src_path, tempp = adr(new a_tree(_M, "$copy_tree"))), 
        ax = path_to_pathx(tempp)), 
        bx.exists && trunc_x(mod, loc, bx), 
        null != ax && ax.exists && merge_x(mod, loc, ax, bx = path_create_pathx(b), 0), 
        is_logged && micro_add_marker(MARK_COPY_END, mod, loc);
    }
};

export function move_tree(mod, loc, a, b) {
    var is_logged = loc > 0 && b.base.eflags & NF_LOGGED;
    is_logged && micro_add_marker(MARK_MOVE_BEG, mod, loc), 
    merge_tree(mod, loc, a, b), 
    trunc_tree(mod, loc, a), 
    is_logged && micro_add_marker(MARK_MOVE_END, mod, loc);
};

export function swap_tree(mod, loc, a, b) {
    var ax, bx, tx;
    if (!is_path_eq(a, b)) if (is_path_in_path(a, b) || is_path_in_path(b, a)) TRAP_ERR && internal_err("cannot swap overlapping trees"); else if (ax = path_to_pathx(a), 
    bx = path_to_pathx(b), 
    ax.exists || bx.exists) if (null !== ax && null !== bx) {
        tx = path_to_pathx(adr(new a_tree(_M, "$swap_tree")));
        var is_logged = loc > 0 && (NF_LOGGED & a.base.eflags || NF_LOGGED & b.base.eflags);
        is_logged && micro_add_marker(MARK_SWAP_BEG, mod, loc), 
        merge_x(mod, loc, ax, tx, 0), 
        trunc_x(mod, loc, ax), 
        merge_x(mod, loc, bx, ax = path_create_pathx(a), 0), 
        trunc_x(mod, loc, bx), 
        merge_x(mod, loc, tx, bx = path_create_pathx(b), 0), 
        is_logged && micro_add_marker(MARK_SWAP_END, mod, loc), 
        null, tx = null;
...
export function clear_tree(mod, loc, b) {
    ...
    if (bx.exists) {
        ...
        is_logged && micro_add_marker(MARK_CLEAR_BEG, mod, loc), 
        trunc_x(mod, loc, bx), 
        is_logged && micro_add_marker(MARK_CLEAR_END, mod, loc);
    }
};
...
export function trunc_tree(mod, loc, b) {
    ...
    if (bx.exists) {
        ...
        var is_logged = loc > 0 && NF_LOGGED & b.base.eflags;
        is_logged && micro_add_marker(MARK_TRUNC_BEG, mod, loc), 
        trunc_x(mod, loc, bx, !1), 
        is_logged && micro_add_marker(MARK_TRUNC_END, mod, loc);
    }
};
...
function trunc_x(mod, loc, px, clear_val = !0) {
    ...
    var changed = !1;
    clear_val && apex_node.val != U && (apex_node.val = U, 
    is_stateful && (changed = !0, 
    apex_node.seq = ++g_currseq)), 
    null != apex_node.children && (apex_node.children = null, 
    !changed && is_stateful && (apex_node.seq = ++g_currseq)), 
...
function trunc_log(mod, loc, px, nesting, apex) {
    ...
    if (nesting > 0 || apex) {
        var oldval = node.val;
        oldval != U && (TRACE_LOG && console.log(str.conv("  atomic trunc {pathx} was {}", px, oldval)), 
        micro_add_delta(LOGK_DELTA, mod, loc, px, oldval, U));
    }
    if (null != node.children) for (var ix in node.children) child = node.children[ix], 
    px.lev.push(child), 
    trunc_log(mod, loc, px, nesting + 1, !0), 
    px.lev.pop();
}

function orphan_upwards(px) {
    for (var delsub, node; px.lev.length > 1 && (delsub = px.lev[px.lev.length - 1].sub, 
    px.lev.pop(), delete (node = px.lev[px.lev.length - 1]).children[delsub], 
    no_children(node) && (node.children = null), 
...
function merge_x(mod, loc, ax, bx, nesting) {
    ...
    if ((val = leaf_getv(ax, 0)) != U && setv_x(mod, loc, bx, val), 
    null !== srcnode.children) for (var ix in null === dstnode.children && (dstnode.children = []), 
    srcnode.children) sub = (src_child = srcnode.children[ix]).sub, 
    ax.lev.push(src_child), 
    null == (dst_child = dstnode.children[sub]) && ((dst_child = new a_tree(mod, "$merge_x")).sub = sub, 
    dstnode.children[sub] = dst_child), 
    bx.lev.push(dst_child), 
    merge_x(mod, loc, ax, bx, nesting + 1), 
...
export function renum_tree(mod, loc, b) {
    ...
    if (null !== bx && bx.exists) {
        ...
        for (child of (g_currseq++, 
        subx = 0, numlist)) subx++, 
        child.sub != subx && (child.sub = subx, 
        child.seq = g_currseq, 
        changed = !0), children2[subx] = child;
        changed && (lastlev.children = children2, 
        lastlev.seq = g_currseq);
    }
};
...
export function is_path_eq2(a, b) {
    ...
    if (null === b) return !1;
    if (a.key.length !== b.key.length) return !1;
    if (a_hash = a.base instanceof a_tree ? a.base.sub : a.base, 
    b_hash = b.base instanceof a_tree ? b.base.sub : b.base, 
    !CHECKS || 0 != a_hash && 0 != b_hash || internal_err("ptr to local tree not permitted"), 
...
function is_meas_eq(m1, m2) {
    let meas1, meas2;
    return m1 instanceof a_meas && m2 instanceof a_meas && (meas2 = m2, 
    (meas1 = m1).mag === meas2.mag && meas1.unitss === meas2.unitss);
}

export function mark_all_instances(field) {
    TRACE_IMPLIED && log(str.conv("mark_all_instances, deferring all instances of {n}", field)), 
    g_fields_to_mark.indexOf(field) < 0 && (g_fields_to_mark.push(field), 
    TRACE_IMPLIED && log(str.conv("  not yet in dirty array, adding {n}", field)));
};
...
export function loom_find_id(id) {
    let lx, len;
    for (len = g_loom.length, 
    lx = 0; lx < len; lx++) if (g_loom[lx].id === id) return lx;
    return -1;
};
...
export function alloc_loom_elem(options) {
    ...
    return nreps > 1 && interval == U && argument_err("when repetitions is greater than 1, you must specify an interval or rate"), 
    is_numeric(interval) === Y && interval < .01 && argument_err("cannot use repeat intervals less than 10 msec"), 
    elem.is_relative = !1, 
    currtime = is_dilated ? now : now_raw, 
    CHECKS && delay !== U && time !== U && argument_err("cannot specify both absolute time and delay from now"), 
    prereq != U ? (delay == U && (delay = 0), 
    time = delay) : delay != U ? (time = currtime + delay, 
    elem.is_relative = !0) : time != U || (time = currtime), 
    id == U && (id = loom_next_id()), 
    elem.prereq = prereq, 
    elem.group = group, 
    elem.id = id, elem.time = time, 
    elem.nreps = nreps, 
    elem.countx = 0, elem.interval = interval, 
...
function loom_dump1(loomx) {
    let fx, msg, elem;
    if (msg = str.conv("  loom[{int}]: ", loomx), 
    elem = g_loom[loomx], 
    ...
    elem.evkind === EV_TIMER && null != elem.func.parms) {
        for (msg += ", parms=[", 
        fx = 0; fx < elem.func.parms.length; fx++) 0 !== fx && (msg += ", "), 
        msg += str.conv("{}", elem.func.parms[fx]);
        msg += "]";
    }
    log(msg);
}
...
function loom_release_id(cleared_id) {
    let lx, item;
    for (TRACE_LOOM && log(str.conv("  loom_release_id id={n}", cleared_id)), 
    lx = 0; lx < g_loom.length; lx++) (item = g_loom[lx]).prereq === cleared_id && (CHECKS && is_numeric(item.time) !== Y && internal_err("loom time not numeric"), 
    item.time += now, TRACE_LOOM && log(str.conv("  dependency at {int} cleared, new time {n3}", lx, item.time)), 
    item.prereq = U, item.is_relative = !0);
}

export function timer_clearx(loomx) {
    loom_release_id(g_loom[loomx].id), 
    g_loom.splice(loomx, 1);
};

export function call_deferred_func(elem) {
    elem.group != IDE_GROUP && add_major_callback(elem), 
    unfreeze_calc(elem.func);
};
...
export function block_animation_leave() {
    let a;
    for (a of g_block_animations) g_overlay.appendChild(a.block), 
    a.block.x = a.startx, 
    a.block.y = a.starty;
...
export function loom_animate_block(block, reference, targx, targy, duration, curvefunc, funcparm) {
    ...
    return a.id = ++g_loom_next_id, 
    a.block = block, a.oparent = block.parent, 
    a.duration = duration, 
    a.parm = 0, a.startx = start_g.x, 
    a.starty = start_g.y, 
    a.stopx = stop_g.x, 
    a.stopy = stop_g.y, 
    ...
    TRACE_ANIMATION && log(str.conv("animation add, id={n}", a.id)), 
    a.id;
};
...
function abort_animate_num(ax, va, finishflag = N) {
    finishflag === Y && path_setv(_M, WHERE_LOOM, va.valpath, va.endval), 
    path_setv(_M, WHERE_LOOM, va.busypath, N), 
    loom_release_id(va.id), 
    g_num_animations.splice(ax, 1);
}

function pause_start(va) {
    va.paused = !0, va.pause_start = now, 
    TRACE_ANIMATION && log(str.conv("pause_start, time={}", va.pause_start));
}

function pause_end(va) {
    va.paused = !1, va.starttime += now - va.pause_start, 
    TRACE_ANIMATION && log(str.conv("pause_end, new start={}", va.starttime));
}
...
export function loom_animate_num_ctrl(id, options) {
    ...
    (ax = find_num_animation(id)) >= 0 ? (va = g_num_animations[ax], 
    pause === Y ? va.paused || pause_start(va) : resume === Y ? va.paused && pause_end(va) : toggle === Y ? va.paused ? pause_end(va) : pause_start(va) : finish === Y ? abort_animate_num(ax, va, Y) : abort === Y && abort_animate_num(ax, va, N)) : CHECKS && argument_err("not found");
};
...
export function solve_unit_bezier(x, epsilon, p1x, p1y, p2x, p2y) {
    ...
    return function sampleCurveY(t) {
        return ((ay * t + by) * t + cy) * t;
    }(function solveCurveX(x, epsilon) {
        var t0, t1, t2, x2, d2, i, t;
        for (t2 = x, i = 0; i < 8; i++) {
            if (x2 = sampleCurveX(t2) - x, 
            TRACE_SOLVE && log(str.conv("inside SolveCurvex,iteration {int}, x={n,3}, err={n,6}, eps={n,3}", i, x, Math.abs(x2), epsilon)), 
            Math.abs(x2) < epsilon) return t2;
            if (d2 = (3 * ax * (t = t2) + 2 * bx) * t + cx, 
            Math.abs(d2) < 1e-6) break;
            t2 -= x2 / d2;
        }
        if (t1 = 1, (t2 = x) < (t0 = 0)) return t0;
        if (t2 > t1) return t1;
        for (;t0 < t1; ) {
            if (x2 = sampleCurveX(t2), 
            Math.abs(x2 - x) < epsilon) return t2;
            x > x2 ? t0 = t2 : t1 = t2, 
            t2 = .5 * (t1 - t0) + t0;
        }
        return t2;
...
export function loom_process_events() {
    ...
    if (currtime = now, 
    TRACE_LOOM && g_loom.length > 0 && log(str.conv("loom_process_events, now={n3}, loom.n={}", currtime, g_loom.length)), 
    g_block_animations.length > 0) {
        for (TRACE_ANIMATION && log(str.conv("block animation loop len={}, now={n3}...", g_block_animations.length, now)), 
        ax = 0; ax < g_block_animations.length; ) ba = g_block_animations[ax], 
        percent = 100 * (now - g_block_animations_otime) / ba.duration, 
        ...
        percent >= 100 ? (loom_release_id(ba.id), 
        ba.oparent.appendChild(ba.block), 
        g_block_animations.splice(ax, 1), 
        TRACE_ANIMATION && log(str.conv("..animation {int} finished, after delete, len={int}", ax, g_block_animations.length))) : ax += 1;
    ...
    for (ax = 0; ax < g_num_animations.length; ) currtime = (va = g_num_animations[ax]).is_dilated ? now : now_raw, 
    va.paused ? ax += 1 : (va_elapsed = currtime - va.starttime, 
    TRACE_ANIMATION && log(str.conv("val animation ax={}, elapsed={n3}, duration={}", ax, va_elapsed, va.duration)), 
    va_elapsed >= va.duration ? (path_setv(_M, WHERE_LOOM, va.busypath, N), 
    path_setv(_M, WHERE_LOOM, va.valpath, va.endval), 
    loom_release_id(va.id), 
    g_num_animations.splice(ax, 1), 
    TRACE_ANIMATION && log(str.conv("..num_animation {int} finished, after delete, len={int}", ax, g_num_animations.length))) : (va.bez_x1 === U ? newval = interpolate(va_elapsed, 0, va.duration, va.startval, va.endval, {
        clamp: Y
    }) : (progress = solve_unit_bezier(va_elapsed / va.duration, .001, va.bez_x1, va.bez_y1, va.bez_x2, va.bez_y2), 
    TRACE_ANIMATION && log(str.conv("..bez, X={n,3}, Y={n,3}", va_elapsed / va.duration, progress)), 
    newval = interpolate(progress, 0, 1, va.startval, va.endval, {})), 
    TRACE_ANIMATION && log(str.conv("  setval, va_elapsed={}, newval={n,1}", va_elapsed, newval)), 
    path_setv(_M, WHERE_LOOM, va.valpath, newval), 
    ax += 1));
    for (lx = 0; lx < g_loom.length; ) if (clearflag = !1, 
    currtime = (elem = g_loom[lx]).is_dilated ? now : now_raw, 
    TRACE_LOOM && log(str.conv("loom_process_events[{int}], currtime={n3}, id={n}, pre={n}, elem.time={n3}", lx, currtime, elem.id, elem.prereq, elem.time)), 
    elem.prereq === U) if (currtime < elem.time) TRACE_LOOM && log(str.conv("  loom: waiting for time, elem.time={n3}, currtime={n3}", elem.time, currtime)), 
    lx += 1; else {
        switch (elem.evkind) {
          case EV_TIMER:
            elem.countx += 1, TRACE_LOOM && log("  loom: calling deferred function"), 
            call_deferred_func(elem), 
            elem.interval_mon != U && getn(runtime, F_monitor_active) == Y ? elem.time = currtime + elem.interval_mon : elem.time = currtime + elem.interval, 
            elem.is_relative = !0;
            break;
        ...
        TRACE_LOOM && log(str.conv("  loom: event was fired, count={n}, nreps={n}", elem.countx, elem.nreps)), 
        clearflag || elem.countx >= elem.nreps ? (TRACE_LOOM && log("  loom: event used up"), 
        void 0 != g_loom[lx] && g_loom[lx].id == elem.id && timer_clearx(lx)) : lx += 1, 
        TRACE_LOOM && log("loom: about to call trickle"), 
        trickle(0), did_trickle = !0;
    } else TRACE_LOOM && log("  loom: waiting for a prereq"), 
    lx += 1;
...
function do_anim_curve(elem, currtime) {
    ...
    switch (bez.phase) {
      case 0:
        (t1 = (anim_elapsed = currtime - elem.time) / elem.interval) > 1 && (t1 = 1), 
        TRACE_CURVE && log(str.conv("curve drawing, now={}, t1={n3}, elapsed={}", currtime, t1, anim_elapsed)), 
        bez.shape.graphics.clear();
        debugger;
        t1 >= 1 && (bez.draw_end_time = now, 
        bez.phase = 1, TRACE_CURVE && log("  --advancing to hold phase"), 
        0 == elem.hold_duration && (bez.hold_end_time = currtime, 
        bez.phase = 2));
        break;

      case 1:
        elem.hold_duration == INFINITY ? (clearflag = !0, 
        TRACE_CURVE && log("curve holding forever, remove from loom")) : (anim_elapsed = currtime - bez.draw_end_time, 
        TRACE_CURVE && log(str.conv("curve holding, duration={n1}, elapsed={}", elem.hold_duration, anim_elapsed)), 
        anim_elapsed >= elem.hold_duration && (bez.hold_end_time = currtime, 
        bez.phase = 2, TRACE_CURVE && log("--advancing to fadeout phase")));
        break;

      case 2:
        anim_elapsed = currtime - bez.hold_end_time;
        var opacity = 1 - elapsed / elem.fadeout_duration;
        TRACE_CURVE && log(str.conv("curve fadeout, elapsed={}, opacity={n3}", anim_elapsed, opacity)), 
        opacity < .05 ? (bez.shape.parent.removeChild(bez.shape), 
        clearflag = !0, TRACE_CURVE && log("  --fadeout finished, removing from screen")) : (bez.shape.alpha = opacity, 
...
function get_os_version(appver) {
    ...
    if (pos > 0) {
        for (;"x)" != (cc = appver.charAt(pos)) && "" != cc; ) "_" == cc && (cc = "."), 
        result += cc, pos += 1;
        return result;
    }
    if ((pos = appver.indexOf("(Win")) > 0) {
        for (pos += 1; ";" != (cc = appver.charAt(pos)) && ")" != cc && "" != cc; ) result += cc, 
        pos += 1;
        return result;
    }
...
export function rtl_init() {
    if (is_rtl) TRACE_INIT && log("--- ignoring second rtl init"); else {
        is_rtl = !0, TRACE_INIT && log("--- rtl_init a"), 
        zap_clock(), add_family(Y_Angle, "angle^1"), 
        ...
        add_unit(Y_Angle, N_radian, 1), 
        add_unit(Y_Angle, N_deg, .017453292519943295), 
        add_unit(Y_Angle, N_grad, .015707963267948967);
        add_unit(Y_Angle, N_turn, 6.283185307179586), 
        add_unit(Y_Area, N_sq_m, 1), 
        add_unit(Y_Area, N_acre, 4046.8564300507887), 
        add_unit(Y_Area, N_hectare, 1e4), 
        add_unit(Y_Area, N_sq_cm, 1e-4), 
        add_unit(Y_Area, N_sq_ft, .09290303999749462), 
        add_unit(Y_Area, N_sq_in, .0006451600000025807), 
        add_unit(Y_Area, N_sq_mi, 2589988.110285327), 
        add_unit(Y_Area, N_sq_mm, 1e-6), 
        add_unit(Y_Area, N_sq_yd, .8361273600007553), 
        add_unit(Y_Energy, N_joule, 1), 
        ...
        add_unit(Y_Energy, N_gigajoule, 1e9), 
        add_unit(Y_Energy, N_hp_hr, 2684519.537686219), 
        add_unit(Y_Energy, N_kw_hr, 3599999.9971200004), 
        add_unit(Y_Energy, N_therm_us, 105480400), 
        add_unit(Y_Energy, N_therm_ec, 105506e3), 
        add_unit(Y_Force, N_newton, 1);
        add_unit(Y_Force, N_lbf, 4.448221618990737), 
        add_unit(Y_Frequency, N_hz, 1), 
        add_unit(Y_Frequency, N_rpm, .016666666666666666), 
        add_unit(Y_Length, N_meter, 1), 
        add_unit(Y_Length, N_angstrom, 1e-11), 
        add_unit(Y_Length, N_cm, .01), 
        add_unit(Y_Length, N_dm, .1), 
        add_unit(Y_Length, N_foot, .30479999999999996), 
        add_unit(Y_Length, N_inch, .0254), 
        add_unit(Y_Length, N_km, 1e3), 
        add_unit(Y_Length, N_mile, 1609.3439999999998), 
        add_unit(Y_Length, N_mm, .001), 
        add_unit(Y_Length, N_nautmile, 1852), 
        add_unit(Y_Length, N_nm, 1e-10), 
        add_unit(Y_Length, N_um, 1e-6), 
        add_unit(Y_Length, N_yard, .9143999999999999), 
        add_unit(Y_Mass, N_kg, 1), 
        add_unit(Y_Mass, N_grain, 6479890999975407e-20), 
        add_unit(Y_Mass, N_gram, .001), 
        add_unit(Y_Mass, N_ounce, .02834952316484755);
        add_unit(Y_Mass, N_pound, .4535923703803783), 
        add_unit(Y_Mass, N_slug, 14.593903), 
        add_unit(Y_Mass, N_ton, 907.1847407607567), 
        add_unit(Y_Mass, N_tonne, 1e3), 
        add_unit(Y_Mass, N_troy_ounce, .031103476769649887), 
        add_unit(Y_Mass, N_troy_pound, .3732417216026466), 
        add_unit(Y_Power, N_watt, 1), 
        add_unit(Y_Power, N_gigawatt, 1e9), 
        add_unit(Y_Power, N_hp, 745.6998713570781), 
        add_unit(Y_Power, N_kilowatt, 1e3), 
        add_unit(Y_Power, N_megawatt, 1e6), 
        add_unit(Y_Power, N_milliwatt, .001), 
        add_unit(Y_Pressure, N_pascal, 1), 
        add_unit(Y_Pressure, N_bar, 1e5), 
        add_unit(Y_Pressure, N_psi, 6894.7572798677575), 
        add_unit(Y_Pressure, N_torr, 133.32236836846923), 
        add_unit(Y_Scalar, N_each, 1), 
        add_unit(Y_Scalar, N_dozen, 12), 
        add_unit(Y_Scalar, N_gross, 144), 
        add_unit(Y_Speed, N_m_per_sec, 1);
        add_unit(Y_Speed, N_ft_per_min, .0050799999922784), 
        add_unit(Y_Speed, N_ft_per_sec, .304799999536704), 
        add_unit(Y_Speed, N_km_per_hr, .2777777777777778), 
        add_unit(Y_Speed, N_m_per_min, .016666666666666666), 
        add_unit(Y_Speed, N_mi_per_hr, .4470400004105615), 
        add_unit(Y_Temperature, N_degK, 1), 
        add_unit(Y_Temperature, N_degC, U, new a_function("std", "conv_degK_to_degC", conv_degK_to_degC), new a_function("std", "conv_degC_to_degK", conv_degC_to_degK)), 
        add_unit(Y_Temperature, N_degF, U, new a_function("std", "conv_degK_to_degF", conv_degK_to_degF), new a_function("std", "conv_degF_to_degK", conv_degF_to_degK)), 
        add_unit(Y_Time, N_sec, 1), 
        add_unit(Y_Time, N_day, 86400), 
        add_unit(Y_Time, N_hour, 3600), 
        add_unit(Y_Time, N_microsec, 1e-6), 
        add_unit(Y_Time, N_millisec, .001), 
        add_unit(Y_Time, N_minute, 60), 
        add_unit(Y_Time, N_nanosec, 1e-9), 
        add_unit(Y_Time, N_picosec, 1e-12), 
        add_unit(Y_Time, N_week, 604800), 
        add_unit(Y_Time, N_avg_month, SECONDS_PER_MONTH), 
        add_unit(Y_Time, N_avg_year, SECONDS_PER_YEAR), 
        add_unit(Y_Volume, N_l, 1);
        add_unit(Y_Volume, N_cu_ft, 28.316579357213648), 
        add_unit(Y_Volume, N_cu_yd, 764.5552200007645), 
        add_unit(Y_Volume, N_cup, .23658823637296003), 
        add_unit(Y_Volume, N_gal, 3.785411789132032), 
        add_unit(Y_Volume, N_ml, .001), 
        add_unit(Y_Volume, N_oz, .029573529564111873), 
        add_unit(Y_Volume, N_pint, .47317647274592006), 
        add_unit(Y_Volume, N_quart, .94635294549184), 
        add_unit(Y_Volume, N_tbsp, .014786764780962696), 
        add_unit(Y_Volume, N_tsp, .004928921598877499), 
        "virtualKeyboard" in navigator && (navigator.virtualKeyboard.overlaysContent = !0), 
        ERR_TREE = new a_tree(_M, "$ERR_TREE", 0), 
        std_add_meta(), str.str_add_meta(), 
        loom_history = new a_tree(_M, "$loom_history", NF_TOPLEVEL | NF_STATEFUL), 
        runtime = new a_tree(_M, "$runtime", NF_TOPLEVEL | NF_STATEFUL), 
        blackbox_runtime = new a_tree(_M, "$blackbox_runtime", NF_TOPLEVEL | NF_STATEFUL), 
        js_set_screen_size(), 
        g_last_screen_h = SCREEN_H, 
        g_last_screen_v = SCREEN_V;
        setv(_M, 0, runtime, F_app_version, "1");
        var verss = navigator.userAgent;
        IS_NODE || (verss += " ratio:" + window.devicePixelRatio.toFixed(1)), 
        setv(_M, 0, runtime, F_env_version, verss), 
        TRACE_INIT && log(`userAgent=${verss}`);
        var os_ver = IS_NODE ? "node.mjs" : get_os_version(navigator.appVersion);
        setv(_M, 0, runtime, F_os_version, os_ver);
        var os_lang = js_to_langx(navigator.language);
        setv(_M, 0, runtime, F_os_language, os_lang), 
        setv(_M, 0, runtime, F_ui_language, os_lang);
        ...
        if (!IS_NODE) {
            const iOS_1to12 = /(iPad|iPhone|iPod)/i.test(navigator.platform), iOS13_iPad = (is_MAC = /mac/i.test(navigator.platform)) && navigator.maxTouchPoints > 1, iOS1to12quirk = function() {
                var audio = new Audio();
                return audio.volume = .5, 
                1 === audio.volume;
            };
            isIOS = !window.MSStream && (iOS_1to12 || iOS13_iPad || iOS1to12quirk());
        }
        IS_MOBILE_SAFARI = !1;
        var is_full_screen = N, cpu = CPU_INTEL;
        if (/arm/i.test(navigator.platform) && (cpu = CPU_ARM), 
        isIOS ? (IS_MOBILE_SAFARI = /safari/i.test(navigator.userAgent), 
        OS_KIND = OS_IOS, setv(_M, 0, runtime, F_os_kind, OS_IOS), 
        cpu = CPU_ARM, is_full_screen = Y) : /android/i.test(navigator.userAgent) ? (OS_KIND = OS_AND, 
        setv(_M, 0, runtime, F_os_kind, OS_AND), 
        cpu = CPU_ARM, is_full_screen = Y) : is_MAC ? (OS_KIND = OS_OSX, 
        setv(_M, 0, runtime, F_os_kind, OS_OSX)) : /win/i.test(navigator.platform) ? (OS_KIND = OS_WIN, 
        setv(_M, 0, runtime, F_os_kind, OS_WIN)) : (OS_KIND = OS_WEB, 
        setv(_M, 0, runtime, F_os_kind, OS_WEB)), 
        setv(_M, 0, runtime, F_cpu_kind, cpu), 
        setv(_M, 0, runtime, F_full_screen, is_full_screen), 
        setv(_M, 0, runtime, F_hardware_id, g_hardware_id), 
        setv(_M, 0, runtime, F_os_modal, 0), 
        setv(_M, 0, runtime, F_is_connected, is_true(navigator.onLine)), 
        TRACE_MAJOR && log("emitting major[1] EV_INIT"), 
        setv(_M, 0, runtime, F_major_steps, 1, F_evkind, EV_INIT), 
        setv(_M, 0, runtime, F_major_steps, 1, F_hist_rawx, 0), 
        setv(_M, 0, runtime, F_major_stepx, 2), 
        setv(_M, 0, runtime, F_touch_kind, TOUCH_NONE), 
        !IS_NODE) {
            window.matchMedia("(pointer: coarse)").matches && (setv(_M, 0, runtime, F_touch_kind, TOUCH_FINGER), 
            g_touch_device = !0), 
            k_init(), window.addEventListener("keydown", js_keydown), 
            window.addEventListener("paste", js_paste), 
            window.addEventListener("mousedown", js_mousedown), 
            window.addEventListener("mouseup", js_mouseup), 
            window.addEventListener("mousemove", js_mousemove), 
            window.addEventListener("wheel", js_wheel), 
            window.addEventListener("touchstart", js_touchstart, {
                passive: !1
            ...
            !1)), window.addEventListener("resize", js_resize), 
            window.addEventListener("orientationchange", js_orient);
...
function a_tbl_col(arg_id, arg_start, arg_stop) {
    ...
    this.startdot = 0, this.stopdot = 0;
}
...
function a_tbl_rowkind(arg_id, arg_drawf) {
    this.kindID = arg_id, 
    this.curcolx = 1, this.curspanx = 1, 
...
function a_tbl_spec() {
    this.backcolor = U, 
    this.prepped = !1, this.nrows = 0, 
    this.tot_height = 0, 
    this.rawgrid = [], this.rawdots = [], 
    this.rowm = [], this.rows = [], 
    this.s = [];
}

function capture_mouse(b) {
    TRACE_CAPTURE && log("capture_mouse by " + b.blabel), 
    0 != g_capture_blabel && argument_err("nested calls to capture not supported"), 
    g_capture_blabel = b.blabel, 
    g_capture_not_ide = is_not_ide(b);
}

function release_mouse() {
    TRACE_CAPTURE && log("release_mouse"), 
    g_capture_blabel = 0;
}
...
export function k_layer(parent, drawf, options) {
    ...
    for (let property in options) switch (property) {
      case "area":
        null != options.area && (left = getn(options.area, F_left), 
        top = getn(options.area, F_top), 
        width = getn(options.area, F_width), 
        height = getn(options.area, F_height));
        break;
      ...
      case "indent":
        ...
        width = parent.bounds.width - 2 * indent, 
        height = parent.bounds.height - 2 * indent;
        break;
      ...
      case "no_touch":
        notouch = options.no_touch == Y;
        break;

      case "opacity":
        is_numeric(options.opacity) != Y && argument_err("bad arg"), 
        opacity = options.opacity;
        break;

      case "pin":
        is_numeric(options.pin) != Y && argument_err("bad arg"), 
        pin = options.pin;
        break;

      case "dpi":
        is_numeric(options.dpi) == Y && (dpi = options.dpi);
        break;

      case "tx":
        is_numeric(options.tx) != Y && argument_err("bad arg"), 
        left = options.tx;
        break;

      case "ty":
        is_numeric(options.ty) != Y && argument_err("bad arg"), 
        top = options.ty;
        break;

      case "skewx":
        is_numeric(options.skewx) != Y && argument_err("bad arg"), 
        skewx = options.skewx;
        break;

      case "skewy":
        is_numeric(options.skewy) != Y && argument_err("bad arg"), 
        skewy = options.skewy;
        break;
    ...
    if (!CHECKS || is_numeric(left) == Y && is_numeric(top) == Y && is_numeric(width) == Y && is_numeric(height) == Y || argument_err("bad bounds"), 
    null != canvas_dest) {
        var canvas, need_canv = !0, oldb = get_object(canvas_dest, F_sysobj);
        if (null != oldb && null != oldb.can && oldb.bounds.width == width && oldb.bounds.height == height && (need_canv = !1, 
        canvas = oldb.can, newb = oldb), 
        need_canv) {
            canvas = document.createElement("canvas"), 
            null == id && (id = `canvas${++g_svg_id}`), 
            canvas.id = id, canvas.width = width, 
            canvas.height = height, 
            canvas.style.left = to_px(left), 
            canvas.style.top = to_px(top), 
            canvas.style.width = to_px(width), 
            canvas.style.height = to_px(height), 
            canvas.style.position = "absolute", 
            (newb = {}).bounds = new Rectangle(0, 0, width, height), 
            newb.can = canvas, newb.ctx = canvas.getContext("2d"), 
            newb.dpi = dpi;
            var extra = new a_tree(_M, "$new_layer");
            setv(_M, 0, extra, F_box, F_left, 0), 
            setv(_M, 0, extra, F_box, F_top, 0), 
            setv(_M, 0, extra, F_box, F_width, width), 
            setv(_M, 0, extra, F_box, F_height, height), 
            newb.extra = extra, 
            setv(_M, 0, canvas_dest, F_width, width), 
            setv(_M, 0, canvas_dest, F_height, height), 
            setv(_M, 0, canvas_dest, F_sysobj, newb);
        }
        parent.appendChild(canvas);
    } else if (null == id && (id = drawf.name), 
    (newb = new_block(parent, new Rectangle(left, top, width, height), BKIND_SUBSET, id, drawf, null, parent.dest)).style.opacity = opacity.toFixed(3), 
    notouch && (newb.style.pointerEvents = "none"), 
    dpi != U && (newb.dpi = dpi), 
    5 == pin) {
...
function k_set_bounds(b, screenbox) {
    ...
    b.style.width = screenbox.width, 
    b.style.height = screenbox.height, 
    b.bounds = new Rectangle(0, 0, screenbox.width, screenbox.height), 
    setv(_M, 0, b.extra, F_box, F_left, 0), 
    setv(_M, 0, b.extra, F_box, F_top, 0), 
    setv(_M, 0, b.extra, F_box, F_width, screenbox.width), 
    setv(_M, 0, b.extra, F_box, F_height, screenbox.height), 
    setv(_M, 0, b.extra, F_boxtot, F_left, 0), 
    setv(_M, 0, b.extra, F_boxtot, F_top, 0), 
    setv(_M, 0, b.extra, F_boxtot, F_width, screenbox.width), 
    setv(_M, 0, b.extra, F_boxtot, F_height, screenbox.height);
}
...
function refresh_all(refresh_menubar = !1) {
    g_last_drawn_seq = 0, 
    g_root_block.bseq = 1, 
    refresh_menubar && (g_menu_block.bseq = 1);
}
...
export function k_enter(b, bflags = 0) {
    b.nesting += 1, 1 == b.nesting ? (g_block_stack.push(g_curr_block), 
    g_curr_block = b, b.usesz.length = 0, 
    bflags & SYMF_CLIP_AREA && (b.style.overflow = "hidden"), 
    bflags & SYMF_SCROLLH && (b.style.overflowX = "scroll", 
    0 == (bflags & SYMF_SCROLLV) && (b.style.overflowY = "hidden"), 
    b.old_scrollh_pos = b.scrollLeft), 
    bflags & SYMF_SCROLLV && (b.style.overflowY = "scroll", 
    0 == (bflags & SYMF_SCROLLH) && (b.style.overflowX = "hidden"), 
    b.old_scrollv_pos = b.scrollTop), 
    k_del_all_children(b), 
    b.div = null, b.tbl = null, 
    TRACE_ENTER && log(str.conv("k_enter, " + b.blabel + ", bounds={rect}, scroll={rect}, seq={n}", b.bounds, b.scrollRect, b.bseq))) : TRACE_K && log("...ignoring nested block " + b.blabel);
};

export function k_leave(b, bflags = 0) {
    b.nesting -= 1, CHECKS && (b.nesting < 0 && internal_err("block was left more times than entered " + b.blabel), 
    null != b.div && b.div.open && internal_err("forgot to call div_end before leaving this block!")), 
    0 == b.nesting && (bflags & SYMF_SCROLLH && (b.scrollLeft = b.old_scrollh_pos), 
    bflags & SYMF_SCROLLV && (b.scrollTop = b.old_scrollv_pos), 
    g_curr_block = g_block_stack.pop(), 
    b.bseq = g_currseq, 
    TRACE_ENTER && log(str.conv("..k_leave, " + b.blabel + ", seq={n}", b.bseq)));
};
...
export function k_back(b, options) {
    ...
    if (null != shadows) {
        ...
        for (var shadow of shadows) {
            if (first ? first = !1 : ss += ", ", 
            ss += `${shadow.dx}px ${shadow.dy}px `, 
            shadow.blur > 0 ? ss += `${shadow.blur}px` : ss += "0", 
            shadow.spread > 0 && (ss += ` ${shadow.spread}px`), 
            shadow.color > 0) {
                var shad_opacity = 1;
                void 0 != shadow.opacity && (shad_opacity = shadow.opacity), 
                ss += ` ${js_color_to_str(shadow.color, shad_opacity)}`;
            } else ss += " black";
            void 0 != shadow.inset && (ss += " inset");
        }
        b.style.boxShadow = ss;
    }
    b.style.background = null == grad ? "" : js_grad_to_str(grad, opacity), 
    b.style.backgroundColor = -1 == fill ? "" : js_color_to_str(fill, opacity), 
    null != cornerlist && (b.style.borderRadius = js_corner_to_str(cornerlist)), 
    null != border && (b.style.border = js_border_to_str(border), 
    tweak_for_border(b, border, 2, 2)), 
    null != borderL && (b.style.borderLeft = js_border_to_str(borderL), 
    tweak_for_border(b, border, 1, 0)), 
    null != borderT && (b.style.borderTop = js_border_to_str(borderT), 
    tweak_for_border(b, border, 0, 1)), 
    null != borderR && (b.style.borderRight = js_border_to_str(borderR), 
    tweak_for_border(b, border, 1, 0)), 
    null != borderB && (b.style.borderBottom = js_border_to_str(borderB), 
    tweak_for_border(b, border, 0, 1));
};
...
function tweak_for_border(block, border, nx, ny) {
    ...
    console.log(`old size=${old_w},${old_h}`), 
    block.style.width = `${old_w - dx}px`, 
    block.style.height = `${old_h - dy}px`, 
    console.log(`new size=${block.clientWidth},${block.clientHeight}`), 
    block.bounds.width -= dx, 
...
function k_inset(b, arg_meas_n, arg_meas_s, arg_meas_e, arg_meas_w, arg_unit, arg_draw_func, arg_track_func = NOTRACK) {
    ...
    TRACE_K && log("k_inset on block " + b.blabel), 
    inset_n = 0, inset_s = 0, 
    inset_e = 0, inset_w = 0, 
    is_numeric(arg_meas_n) == Y && arg_meas_n >= 0 ? inset_n = unit_to_pixels(b, arg_meas_n, arg_unit) : CHECKS && argument_err("invalid inset"), 
    is_numeric(arg_meas_s) == Y && arg_meas_s >= 0 ? inset_s = unit_to_pixels(b, arg_meas_s, arg_unit) : CHECKS && argument_err("invalid inset"), 
    is_numeric(arg_meas_e) == Y && arg_meas_e >= 0 ? inset_e = unit_to_pixels(b, arg_meas_e, arg_unit) : CHECKS && argument_err("invalid inset"), 
    is_numeric(arg_meas_w) == Y && arg_meas_w >= 0 ? inset_w = unit_to_pixels(b, arg_meas_w, arg_unit) : CHECKS && argument_err("invalid inset"), 
    new_bounds = new Rectangle(b.bounds.left + inset_w, b.bounds.top + inset_n, b.bounds.width - inset_w - inset_e, b.bounds.height - inset_n - inset_s), 
    TRACE_K && log(str.conv("after inset of {n1},{n1},{n1},{n1}, bounds {rect}", inset_n, inset_s, inset_e, inset_w, new_bounds)), 
...
function k_indent_to_net(block, targh, targv, justh, justv, arg_draw_func, arg_track_func = NOTRACK) {
    ...
    !CHECKS || is_numeric(justh) == Y && is_numeric(justv) == Y && is_numeric(targh) == Y && is_numeric(targv) == Y || argument_err("non-numeric inputs"), 
    justh = Math.max(0, Math.min(1, justh)), 
    justv = Math.max(0, Math.min(1, justv)), 
    pixels_h = targh, excess_h = Math.max(0, block.bounds.width - pixels_h), 
    pixels_v = targv, excess_v = Math.max(0, block.bounds.height - pixels_v), 
    ...
    TRACE_K && log(str.conv("indent_to_net, targ {n},{n}, pixels {n},{n}, excess {n},{n}", targh, targv, pixels_h, pixels_v, excess_h, excess_v)), 
...
export function div_begin(b, gridf = null, grid_tblr = !1, arg_scrollh = !1, arg_scrollv = !1) {
    TRACE_SCROLL && log(str.conv("div_begin {str}, scrh={}, scrv={}, bounds={rect}, scroll={rect}", b.blabel, arg_scrollh, arg_scrollv, b.bounds, b.scrollRect)), 
    b.div = new a_div_spec(gridf, grid_tblr, arg_scrollh, arg_scrollv);
};
...
function scroll_block_id(targid, posx, posy) {
    let b;
    TRACE_SCROLL && log(">> scroll_block_id"), 
    null != (b = k_find_block_by_id(g_root_block, targid)) ? (TRACE_SCROLL && log(str.conv("scroll_block_id, blabel={str}, id={n}, x={n}, y={n}, scroll={rect}", b.blabel, b.bid, posx, posy, b.scrollRect)), 
    null != b.div ? (TRACE_SCROLL && log("  scrolling a div block"), 
    b.scrollRect = new Rectangle(b.bounds.left + posx, b.bounds.top + posy, b.bounds.width, b.bounds.height)) : null != b.tbl ? TRACE_SCROLL && log("  refreshing a tbl") : internal_err("block kind not allowed to have scrollbar")) : internal_err("content block is missing!");
}
...
export function slices_compute(parent, is_grid, arg_flags, scrolling, slices, avail, avail2) {
    ...
    if (content = 0 == (arg_flags & DIVF_VERT) ? parent.bounds.width : parent.bounds.height, 
    0 == (nslices = slices.length)) return content;
    for (used = 0, tot_aliquots = 0, 
    tentative = new Array(nslices), 
    i = 0; i < nslices; i++) (myslice = slices[i]).unit == al ? (tot_aliquots += myslice.mag, 
    pixels = 0) : pixels = unit_to_pixels(parent, myslice.mag, myslice.unit), 
    tentative[i] = pixels, 
    used += pixels;
    if (!scrolling) {
        if (leftover = avail - used, 
        TRACE_K && log(`start pass 2, nslices=${nslices}, avail=${avail}, tot_al=${tot_aliquots}, tot_tent=${used}, left=${leftover}`), 
        tot_aliquots > 0) for (pixels_per_al = leftover / tot_aliquots, 
        i = 0; i < nslices; i++) myslice = slices[i], 
        TRACE_K && myslice.unit == al && log(`  slice ${i} has min ${myslice.min_pt}`), 
        myslice.unit == al && myslice.min_pt != U && (pixels = myslice.mag * pixels_per_al) < (limit = pt_to_dots(parent, myslice.min_pt)) && (myslice.unit = px, 
        tentative[i] = limit, 
        TRACE_K && log(`    adjusting slice ${i} for min now ${limit}`));
        if (tot_aliquots > 0 && leftover > 0) for (;;) {
            for (done = !0, used = 0, 
            tot_aliquots = 0, i = 0; i < nslices; i++) (myslice = slices[i]).unit == al ? tot_aliquots += myslice.mag : used += tentative[i];
            for (leftover = avail - used, 
            TRACE_K && log(`top pass 3, used=${used}, tot_al=${tot_aliquots}, left=${leftover}`), 
            pixels_per_al = leftover / tot_aliquots, 
            i = 0; i < nslices; i++) (myslice = slices[i]).unit == al ? (pixels = myslice.mag * pixels_per_al, 
            myslice.max_pt != U && (limit = pt_to_dots(parent, myslice.max_pt), 
            TRACE_K && log(`    slice ${i} has max of ${limit}`), 
            pixels > limit && (pixels = limit, 
            myslice.unit = px, TRACE_K && log(`      limit slice ${i} to max ${limit}`), 
            done = !1)), TRACE_K && log(`    AL slice ${i} of ${myslice.mag} to ${pixels} px`), 
            tentative[i] = pixels) : (TRACE_K && log(`    keep slice ${i} at ${pixels}`), 
            pixels = tentative[i]);
            if (TRACE_K && log(`end pass 3, done=${done}`), 
            done) break;
        }
        if (leftover < -.1) {
            var factor = avail / used;
            for (TRACE_K && log(`pass 4, rescale by ${factor}, tot_tent=${used}, left=${leftover}`), 
            TRACE_K && log(`div slice overflow, avail=${avail}, spillover=${-leftover}, factor=${factor}`), 
            i = 0; i < nslices; i++) tentative[i] *= factor;
        }
    }
    if (0 == (arg_flags & DIVF_VERT)) for (pos = parent.bounds.left, 
    i = 0; i < nslices; i++) slices[i].box = new Rectangle(pos, parent.bounds.top, tentative[i], parent.bounds.height), 
    pos += tentative[i]; else for (pos = parent.bounds.top, 
    i = 0; i < nslices; i++) slices[i].box = new Rectangle(parent.bounds.left, pos, parent.bounds.width, tentative[i]), 
    pos += tentative[i];
    if (content = pos, TRACE_K) for (log("..end slices_compute:"), 
    i = 0; i < nslices; i++) dname = slices[i].id != SKIP_SLICE ? slices[i].drawer.name : "-space-", 
    log(str.conv(" slices[{}] " + dname + ", box={rect}, id={}", i, slices[i].box, slices[i].id));
    return slices.sort(order_slices), 
    content;
};
...
export function draw_grid(b) {
    ...
    for (logcol = 1, colx = 0; colx < div.horz_slices.length; colx++) if (div.horz_slices[colx].id != SKIP_SLICE) {
        for (logrow = 1, rowx = 0; rowx < div.vert_slices.length; rowx++) if (div.vert_slices[rowx].id != SKIP_SLICE) {
            box = new Rectangle(div.horz_slices[colx].box.left, div.vert_slices[rowx].box.top, div.horz_slices[colx].box.width, div.vert_slices[rowx].box.height), 
            TRACE_GRID && log(str.conv("horz_box={rect}, vert_box={rect}, net={rect}", div.horz_slices[colx].box, div.vert_slices[colx].box, box)), 
            ...
            setv(_M, 0, myblock.extra, F_ncells, F_x, div.ncols), 
            setv(_M, 0, myblock.extra, F_ncells, F_y, div.nrows), 
            setv(_M, 0, myblock.extra, F_cell, F_x, logcol), 
            setv(_M, 0, myblock.extra, F_cell, F_y, logrow), 
            cumx = div.grid_tblr ? (logcol - 1) * div.nrows + logrow : (logrow - 1) * div.ncols + logcol, 
            setv(_M, 0, myblock.extra, F_cell_seq, cumx), 
            setv(_M, 0, myblock.extra, F_box, F_left, 0), 
            setv(_M, 0, myblock.extra, F_box, F_top, 0), 
            setv(_M, 0, myblock.extra, F_box, F_width, box.width), 
            setv(_M, 0, myblock.extra, F_box, F_height, box.height), 
            unfreeze_draw(myblock, div.grid_func), 
            logrow++;
        }
        logcol++;
    }
...
export function draw_slices(b) {
    ...
    if (is_horz) {
        for (i = 0; i < nslices; i++) (myslice = slices[i]).id != SKIP_SLICE ? myslice.box.width <= 0 || (child = new_block(b, myslice.box, BKIND_SLICE_H, myslice.id, myslice.drawer), 
        setv(_M, 0, child.extra, F_cell, F_x, i + 1), 
        setv(_M, 0, child.extra, F_cell, F_y, 1), 
        setv(_M, 0, child.extra, F_cell_seq, i + 1), 
        setv(_M, 0, child.extra, F_ncells, F_x, div.ncols), 
        setv(_M, 0, child.extra, F_ncells, F_y, div.nrows), 
        unfreeze_draw(child, child.drawer), 
        offset = 0) : offset += myslice.box.width;
        offset > 0 && null != child && (child.style.marginRight = to_px(offset));
    } else {
        for (i = 0; i < nslices; i++) (myslice = slices[i]).id != SKIP_SLICE ? myslice.box.height <= 0 || (child = new_block(b, myslice.box, BKIND_SLICE_V, myslice.id, myslice.drawer), 
        setv(_M, 0, child.extra, F_cell, F_x, 1), 
        setv(_M, 0, child.extra, F_cell, F_y, i + 1), 
        setv(_M, 0, child.extra, F_cell_seq, i + 1), 
        setv(_M, 0, child.extra, F_ncells, F_x, div.ncols), 
        setv(_M, 0, child.extra, F_ncells, F_y, div.nrows), 
        unfreeze_draw(child, child.drawer), 
...
export function div_end(b) {
    ...
    TRACE_K && log(str.conv("div_end on block {str}, scrh={}, scrv={}, bounds={rect}", b.blabel, div.scrollh, div.scrollv, b.bounds)), 
    div.open || argument_err("this block already been closed up"), 
    div.open = !1, null != div.grid_func && (b.bkind = BKIND_GRID_BASE), 
    avail = b.bounds.width, 
    avail2 = b.bounds.height, 
    div.content_h = slices_compute(b, null != div.grid_func, 0, div.scrollh, div.horz_slices, avail, avail2), 
    ...
    div.ncols = Math.max(1, ncols), 
    div.nrows = Math.max(1, nrows), 
    setv(_M, 0, b.extra, F_ncells, F_x, div.ncols), 
    setv(_M, 0, b.extra, F_ncells, F_y, div.nrows), 
    setv(_M, 0, b.extra, F_boxtot, F_width, div.content_h), 
...
export function div_add(parent, arg_id, arg_flags, arg_meas, arg_unit, arg_min, arg_max, arg_func, arg_order = 0, arg_condition = Y) {
    if (arg_condition != Y) return void div_spa(parent, arg_flags, arg_meas, arg_unit, arg_min, arg_max);
    if (TRACE_K && log(str.conv("  div add, meas={n}, unit={n}, min={n}, max={n}", arg_meas, arg_unit, arg_min, arg_max)), 
    arg_meas <= 0) return;
    ...
    new_slice.min_pt = arg_min, 
    new_slice.max_pt = arg_max, 
    new_slice.mag = arg_meas, 
    0 == (arg_flags & DIVF_VERT) ? (parent.div.horz_slices.push(new_slice), 
    parent.div.horz_slices.length > 500 && internal_err("More than 500 columns; probable infinite loop in " + parent.blabel)) : (parent.div.vert_slices.push(new_slice), 
    parent.div.vert_slices.length > 2e3 && internal_err("More than 2000 rows; probable infinite loop in " + parent.blabel));
};

export function k_overlay(parentb, arg_func) {
    let overlay, label = arg_func.name + ":overlay";
    (overlay = new_block(parentb, parentb.bounds, BKIND_OVERLAY, label, arg_func)).dest = parentb.dest, 
...
function is_block_obsolete(block) {
    ...
    for (dependency of (TRACE_OBSOLETE && log(str.conv("is_block_obsolete block={str}, ndep={}", block.blabel, block.usesz.length)), 
    block.usesz)) {
        TRACE_OBSOLETE && log(str.conv("  testing {path}", dependency.path));
        var pathx = path_to_pathx(dependency.path);
        if (pathx.exists) {
            if ((node_seq = pathx.lev[pathx.lev.length - 1].seq) > drawnseq) return TRACE_OBSOLETE && log(str.conv("    -- OBSOLETE, node_seq={n}, drawn_seq={n}", node_seq, drawnseq)), 
            !0;
        } else {
            if (0 != dependency.seq) return TRACE_OBSOLETE && log("   -- OBSOLETE: used to exist, now doesn't exist"), 
            !0;
...
function find_x_in_section(x, sect) {
    ...
    for (i in sect.fields) (dist = int_abs(x - sect.fields[i].startdot)) < closest_dist && (closest_dist = dist, 
    closest_ix = i), (dist = int_abs(x - (sect.fields[i].stopdot - 1))) < closest_dist && (closest_dist = dist, 
    closest_ix = i);
    return closest_ix;
}
...
function find_in_rows(y, b, tbl) {
    ...
    for (rx in dy = 0, tbl.rows) if (row_top = tbl.rows[rx].rowbox.top + 0, 
    row_bot = tbl.rows[rx].rowbox.bottom + 0, 
    TRACE_XY && log(str.conv("  find_in_rows, i={}, y={}, top={}, bot={}", rx, y, row_top, row_bot)), 
...
function find_xy_in_table(b, event, localx, localy) {
    ...
    if (rowx = find_in_rows(localy, b, mytbl = b.tbl), 
    TRACE_XY && log(str.conv("  find_xy_in_table, local=[{n},{n}], rowx={}", localx, localy, rowx)), 
    rowx >= 0 && (rowID = mytbl.rows[rowx].rowid) != U) {
        let mykind = mytbl.rows[rowx].rowkind_ptr;
        if (colx = find_x_in_section(localx, mykind), 
        colID = U, colx >= 0 && (colID = mykind.fields[colx].colid), 
        TRACE_XY && log(str.conv("  sending to table tracker, colx={}, colID={n}", colx, colID)), 
        colID != U) {
            setv(_M, 0, b.extra, F_cell, F_x, colx + 1), 
            setv(_M, 0, b.extra, F_cell, F_y, rowx + 1), 
            setv(_M, 0, b.extra, F_row_kind, mykind.kindID);
            let r = mytbl.rows[rowx].rowbox;
            return setv(_M, 0, b.extra, F_box, F_left, r.left), 
            setv(_M, 0, b.extra, F_box, F_top, r.top), 
            setv(_M, 0, b.extra, F_box, F_width, r.width), 
            setv(_M, 0, b.extra, F_box, F_height, r.height), 
            b.trakfunc(b, event);
        }
    }
    return !1;
...
function callback_find_by_label(b, data) {
    TRACE_XY && log("callback_find_by_label, testing " + b.blabel + ", against " + data.targ_blabel), 
    b.blabel == data.targ_blabel && (data.go = !1, 
    data.found_block = b, 
    TRACE_XY && log("...FOUND"));
}
...
function callback_find_any(b, data) {
    let absorbval;
    if (TRACE_XY && log("callback_find_any, block " + b.blabel), 
    null != b.tbl) ; else if (null != b.drawer) {
        let tracker = b.drawer.tracker;
        null != tracker ? (absorbval = null != b.drawer.parms ? tracker.apply(null, [ b, data.event ].concat(b.drawer.parms)) : tracker(b, data.event)) == Y ? (data.go = !1, 
        data.absorber = b) : TRACE_XY && log("  event was sent to block, not absorbed") : TRACE_XY && log("  inside block, no tracking func");
    }
...
function find_xy_in_grid(b, event, localx, localy) {
    ...
    if (TRACE_XY && log(str.conv("  find_xy_in_grid, local=[{num}, {num}]", localx, localy)), 
    null == b.drawer.tracker) return !1;
    for (logcol = 1, colx = 0; colx < div.horz_slices.length; colx++) {
        if (idx = div.horz_slices[colx].id, 
        boxl = div.horz_slices[colx].box.left, 
        boxw = div.horz_slices[colx].box.width, 
        localx >= boxl && localx < boxl + boxw) {
    ...
    for (logrow = 1, rowx = 0; rowx < div.vert_slices.length; rowx++) {
        if (idy = div.vert_slices[rowx].id, 
        boxt = div.vert_slices[rowx].box.top, 
        boxh = div.vert_slices[rowx].box.height, 
        localy >= boxt && localy < boxt + boxh) {
            in_dead_space = in_dead_space || idy == SKIP_SLICE;
            break;
        }
        idy != SKIP_SLICE && logrow++;
    }
    if ((logcol > div.ncols || logrow > div.nrows) && (in_dead_space = !0), 
    in_dead_space) {
        if (0 == g_capture_blabel) return !1;
        logcol = U, logrow = U, 
        cumx = U, idx = U, idy = U;
    } else cumx = div.grid_tblr ? (logcol - 1) * div.nrows + logrow : (logrow - 1) * div.ncols + logcol;
    return setv(_M, 0, b.extra, F_cell_seq, cumx), 
    setv(_M, 0, b.extra, F_cell, F_x, logcol), 
    setv(_M, 0, b.extra, F_cell, F_y, logrow), 
    (absorbval = unfreeze_track(b, event, b.drawer)) == Y && (g_last_majorx > 0 && (setv(_M, 0, runtime, F_major_steps, g_last_majorx, F_hist_cell_cum, cumx), 
    setv(_M, 0, runtime, F_major_steps, g_last_majorx, F_hist_cell, F_x, logcol), 
    setv(_M, 0, runtime, F_major_steps, g_last_majorx, F_hist_cell, F_y, logrow), 
    setv(_M, 0, runtime, F_major_steps, g_last_majorx, F_hist_absorber, b.blabel), 
    TRACE_MAJOR && log(str.conv(" -- updating absorber to {}", b.blabel))), 
    !0);
}
...
function do_pointer_event(evkind, evtime, wherex, wherey, touch) {
    ...
    if (setv(_M, WHERE_LOOM, event, F_evkind, evkind), 
    setv(_M, WHERE_LOOM, event, F_when, evtime), 
    setv(_M, WHERE_LOOM, event, F_global_x, wherex), 
    setv(_M, WHERE_LOOM, event, F_global_y, wherey), 
    touch.mod_shf && (TRACE_MOUSE_MOVE && evkind != EV_HOVER && log(" -- SHIFT"), 
    setv(_M, WHERE_LOOM, event, F_is_shift, Y)), 
    touch.mod_alt && setv(_M, WHERE_LOOM, event, F_is_alt, Y), 
    touch.mod_cmd && setv(_M, WHERE_LOOM, event, F_is_cmd, Y), 
    touch.mod_ctl && setv(_M, WHERE_LOOM, event, F_is_ctrl, Y), 
    g_last_majorx = -1, 
    evkind != EV_HOVER) {
        if (0 != g_capture_blabel) {
            ...
            return null != capture_block ? (TRACE_MOUSE_MOVE && log("captured event to try_in_block"), 
            try_in_block(capture_block, event)) : console.log(`captured ${g_capture_blabel} disappeared, ignored`), 
            void (getn(event, F_evkind) == EV_DRAG_END && release_mouse());
        }
        TRACE_MOUSE_MOVE && log(str.conv("about to call visit_all, mouse=({n},{n})", wherex, wherey)), 
        visit_all_containing(g_root_block, wherex, wherey, evkind, event);
    } else for (var deferred of (g_hover_deferred = [], 
    visit_hover(!0, g_root_block, wherex, wherey, event), 
    setv(_M, 0, event, F_evkind, EV_ENTER), 
    g_hover_deferred)) try_in_block(deferred, event);
}

function do_key_event(when, unistr, keycode, is_shift = !1, is_alt = !1, is_cmd = !1, is_ctrl = !1) {
    let b;
    if (g_interface_locked == Y) return TRACE_EVENTS && log(str.conv("interface locked / suppressing key event, keycode=", keycode)), 
    !1;
    let event = new a_tree(_M, "$do_key_event");
    setv(_M, WHERE_LOOM, event, F_evkind, EV_KEYBOARD), 
    setv(_M, WHERE_LOOM, event, F_when, when), 
    setv(_M, WHERE_LOOM, event, F_keycode, keycode), 
    "" != unistr && setv(_M, WHERE_LOOM, event, F_unicode, unistr), 
    is_shift && setv(_M, WHERE_LOOM, event, F_is_shift, Y), 
    is_alt && setv(_M, WHERE_LOOM, event, F_is_alt, Y), 
    is_cmd && setv(_M, WHERE_LOOM, event, F_is_cmd, Y), 
    is_ctrl && setv(_M, WHERE_LOOM, event, F_is_ctrl, Y);
    ...
    null != (b = data.absorber) && (is_not_ide(b) ? (TRACE_MAJOR && log(str.conv("  keystroke was absorbed by {}", b.blabel)), 
    setv(_M, 0, runtime, F_major_steps, sx, F_hist_absorber, b.blabel)) : (TRACE_MAJOR && log(str.conv("  debugger keystroke, absorbed by {}, deleting {}", b.blabel, sx)), 
    clear_tree(0, 0, adr(runtime, F_major_steps, sx))), 
    !0);
}
...
function try_in_block(b, event) {
    ...
    let tracker = null;
    if (null != b.drawer && (tracker = b.drawer.tracker), 
    null != tracker) {
        let local = global_to_local_p(b, globalx = getn(event, F_global_x), globaly = getn(event, F_global_y));
        if (setv(_M, 0, event, F_x, local.x), 
        setv(_M, 0, event, F_y, local.y), 
        TRACE_XY && log(str.conv("try_in_block, {}, evkind={n}, global=[{n},{n}], local=[{n},{n}], bounds={rect}", b.blabel, getn(event, F_evkind), globalx, globaly, local.x, local.y, b.bounds)), 
        null != b.tbl) {
            if (find_xy_in_table(b, event, local.x, local.y)) return TRACE_XY && log("  CONSUMED by table"), 
            !0;
            TRACE_XY && log("  NOT CONSUMED by table");
        } else if (null != b.div && null != b.div.grid_func) {
            if (TRACE_XY && log("  grid event to " + b.blabel), 
            find_xy_in_grid(b, event, local.x, local.y)) return TRACE_XY && log("  CONSUMED by grid"), 
            !0;
            TRACE_XY && log("  NOT CONSUMED by grid");
        } else {
            let absorbed;
            if ((absorbed = unfreeze_track(b, event, b.drawer)) == Y) return TRACE_XY && log("  CONSUMED by block"), 
            g_last_majorx > 0 && (TRACE_MAJOR && log(str.conv(" -- updating absorber to {}", b.blabel)), 
            setv(_M, 0, runtime, F_major_steps, g_last_majorx, F_hist_absorber, b.blabel)), 
            !0;
            TRACE_XY && log("  NOT CONSUMED by block");
        }
    }
    return !!(b.bflags & BFLAG_MODAL) && (TRACE_XY && log("  MODAL: silent absorption of pointer event"), 
    !0);
}

function k_visit_blocks_bottom_up(b, callback, data) {
    let child, i, bx, children = b.childNodes, n = children.length;
    for (i = 1; i <= n && data.go; i++) (child = children[bx = n - i]).hasOwnProperty("bid") && (TRACE_VISIT && log(str.conv("  deeper to {str}", child.blabel)), 
    k_visit_blocks_bottom_up(child, callback, data));
    data.go && (TRACE_VISIT && log(str.conv("k_visit_blocks_bottom_up passed to {str}, seq={n}", b.blabel, b.bseq)), 
    callback(b, data));
}

function k_visit_blocks_top_down(b, callback, data) {
    data.go && (TRACE_OBSOLETE && log(str.conv("k_visit_blocks_top_down {str}, seq={n}", b.blabel, b.bseq)), 
    callback(b, data));
    let child, i, children = b.childNodes, n = children.length;
    for (i = 0; i < n && data.go; i++) (child = children[i]).hasOwnProperty("bid") && (gg_depth += 1, 
...
function k_find_block_by_label(b, label) {
    ...
    return TRACE_FIND && log(str.conv("k_find_block_by_label, curr={}, targ={}", b.blabel, label)), 
    b.blabel == label ? b : null;
}
...
function k_find_block_by_locator(b, targ_label, targ_locater) {
    ...
    return TRACE_FINDBX && log(str.conv("testing {str} {n}, targ {str} {n}", b.blabel, b.locator, targ_label, targ_locater)), 
    b.blabel == targ_label && b.locator == targ_locater ? (TRACE_FINDBX && log("  FOUND!"), 
    b) : null;
}
...
export function global_to_local(block, options) {
    ...
    for (var property in options) switch (property) {
      ...
      case "xy":
        gx = getn(options.xy, F_x), 
        gy = getn(options.xy, F_y);
        break;
    ...
    return setv(_M, 0, result, F_x, p.x), 
    setv(_M, 0, result, F_y, p.y), 
    result;
};

function global_to_local_p(block, globalx, globaly) {
    let cornerx = globalx, cornery = globaly, element = block;
    cornerx -= window.pageXOffset, 
    cornery -= window.pageYOffset;
    do {
        cornerx -= element.offsetLeft, 
        cornery -= element.offsetTop, 
        TRACE_GTOL && log(`global_to_local_p, b=${element.blabel}, sTop=${element.scrollTop}`), 
        cornerx += element.scrollLeft, 
        cornery += element.scrollTop;
        let ss = element.style.transformOrigin;
        if ("" != ss) {
            let list = ss.split(" ");
            list[0].length > 2 && (cornerx -= Number(list[0].substring(0, list[0].length - 2))), 
            list[1].length > 2 && (cornery -= Number(list[1].substring(0, list[1].length - 2)));
        }
        element = element.offsetParent;
    } while (null != element);
    return TRACE_GTOL && log(`.. mapped (${globalx},${globaly}) to (${cornerx},${cornery})`), 
...
export function local_to_parent(block, targname, options) {
    ...
    for (let property in options) switch (property) {
      ...
      case "xy":
        lx = getn(options.xy, F_x), 
        ly = getn(options.xy, F_y);
        break;

      default:
        argument_err("bad option: " + property);
    }
    for (var cumx = lx, cumy = ly, p = block; cumx += p.offsetLeft, 
    cumy += p.offsetTop, 
    p.id != ROOT_BLABEL && !p.id.startsWith(targname) && null != (p = p.parentElement); ) ;
    let result = new a_tree(_M, "$local_to_parent");
    return setv(_M, 0, result, F_x, cumx), 
    setv(_M, 0, result, F_y, cumy), 
    result;
};
...
export function local_to_global(block, options) {
    ...
    return setv(_M, 0, result, F_x, gxy.x), 
    setv(_M, 0, result, F_y, gxy.y), 
    result;
};

function local_to_global_p(block, local = null) {
    let cornerx = 0, cornery = 0, element = block;
    null != local && (cornerx = local.x, 
    cornery = local.y);
    let ss = element.style.transformOrigin;
    if ("" != ss) {
        let list = ss.split(" ");
...
function visit_hover(parent_inside, b, globalx, globaly, event) {
    ...
    is_inside != b.is_inside && (b.is_inside = is_inside, 
    is_inside ? g_hover_deferred.push(b) : (setv(_M, 0, event, F_evkind, EV_LEAVE), 
    try_in_block(b, event)));
}

function visit_all_containing(b, globalx, globaly, evkind, event) {
    let absorbed, gbounds, is_inside_block, not_ide;
    if (gbounds = local_to_global_r(b), 
    is_inside_block = b.blabel == ROOT_BLABEL || xy_in_rect(globalx, globaly, gbounds), 
    TRACE_XY && log(str.conv(" testing #{}:{}, xy=[{},{}], bounds={rect}, gbounds={rect}, inside={}", b.bid, b.blabel, globalx, globaly, b.bounds, gbounds, is_inside_block)), 
    is_inside_block && "$log" != b.blabel) {
        if (g_last_majorx < 0) if (not_ide = is_not_ide(b)) {
            if (0 == g_time_scale) return TRACE_MAJOR && log("--ignoring click while frozen"), 
            !0;
            TRACE_MAJOR && log(str.conv(" -- adding major event for ptr event, first={}", b.blabel)), 
            g_last_majorx = add_major_event(event, null);
        } else TRACE_MAJOR && log(str.conv(" -- is IDE event {}", b.blabel));
        let child, i, bx, children = b.childNodes, n = children.length;
        for (TRACE_XY && log(`  number of children is ${n}`), 
        i = 1; i <= n; i++) if ((child = children[bx = n - i]).hasOwnProperty("bid")) {
            if (TRACE_XY && log(str.conv("  recursive to child {}: {}", i, child.blabel)), 
            visit_all_containing(child, globalx, globaly, evkind, event)) return !0;
        } else TRACE_XY && log("child " + String(i) + " is not a block: " + String(child));
        if (absorbed = try_in_block(b, event), 
        TRACE_EVENTS && evkind == EV_DRAG_BEGIN && log(`>> DRAG_BEGIN, absorbed=${absorbed}`), 
        absorbed) {
            TRACE_EVENTS && log(str.conv("    {} absorbed event", b.blabel));
            var curr_focus = document.activeElement;
            b != curr_focus && curr_focus.blur(), 
            evkind == EV_DRAG_BEGIN && 0 == g_capture_blabel && capture_mouse(b);
        }
        return absorbed;
    }
    return !1;
...
function callback_compare_label(b, data) {
    TRACE_K && log("callback_compare_label, " + b.blabel + " against " + data.label), 
    b.blabel == data.label && (data.result = b, 
    data.go = !1);
}
...
function dump_display_list() {
    let data;
    log("=== ALL BLOCKS ==="), 
    gg_depth = 0, k_visit_blocks_top_down(g_root_block, callback_dump, data = {
        go: !0
    }), log("=== ..end blocks.. ===");
}
...
function k_del_all_children(b) {
    if (null != b) {
        if (TRACE_OBSOLETE && log("  k_del_all_children, starting at " + b.blabel), 
        b.has_transform) b = b.firstChild.firstChild;
...
function rebuild_affected_blocks(b) {
    ...
    if (TRACE_OBSOLETE && b.blabel == OBBLOCK && log(str.conv("rebuild_affected block={str}, id={n}, seq={n}, ndep={}", b.blabel, b.bid, b.bseq, b.usesz.length)), 
    do_children = !0, is_block_obsolete(b) && (TRACE_OBSOLETE && log("  OBSOLETE " + b.blabel), 
    entry_id = document.activeElement.id, 
    null != b.drawer.parms ? b.drawer.code.apply(null, [ b ].concat(b.drawer.parms)) : b.drawer.code(b), 
    TRACE_OBSOLETE && log(str.conv("  ..end rebuild of {str}, seq={n}", b.blabel, b.bseq)), 
...
export function rebuild_all() {
    ...
    if (firstx == U && (firstx = getn(runtime, F_major_stepx), 
    setv(_M, 0, runtime, F_major_firstx, firstx)), 
    null != (maind = null != aaaa.mon_drawer ? aaaa.mon_drawer : aaaa.main_drawer)) {
        for (var node = document.body; node.firstChild; ) node.removeChild(node.lastChild);
        let horz = USABLE_H, vert = USABLE_V;
        setv(_M, 0, runtime, F_window_horz, horz), 
        setv(_M, 0, runtime, F_window_vert, vert), 
        TRACE_RESIZE && log(`---\x3e rebuild_all: total ${horz} x ${vert}`);
        let interior = new Rectangle(0, 0, horz, vert);
        g_root_block = new_block(null, interior, BKIND_ROOT, ROOT_BLABEL, maind), 
        document.body.appendChild(g_root_block), 
        post_resize_event(g_root_block, maind.tracker), 
        LOG_DRAW && add_major_event(DRAW_EVENT, g_root_block), 
        maind.code(g_root_block), 
        TRACE_K && dump_display_list();
    }
};
...
function post_resize_event(b, trackerf) {
    if (null != trackerf) {
        ...
        setv(_M, 0, resize_event, F_evkind, EV_RESIZE), 
        setv(_M, 0, resize_event, F_when, 0), 
        LOG_DRAW && add_major_event(resize_event, b), 
        trackerf(b, resize_event);
    }
}

function check_tree_for_dirty(node) {
    let child, some_dirty;
    for (var ix in some_dirty = !1, 
    node.children) 0 != ((child = node.children[ix]).eflags & EF_IS_DERIVED) && (TRACE_OBSOLETE && log(str.conv("  checking child.sub={n}", child.sub)), 
    g_fields_to_mark.indexOf(child.sub) >= 0 && (TRACE_OBSOLETE && log(str.conv("  marking field as dirty {n}", child.sub)), 
    child.eflags |= EF_IS_DIRTY, 
    some_dirty = !0)), some_dirty && (node.seq = g_currseq), 
    null != child.children && check_tree_for_dirty(child);
}
...
function trickle(seconds) {
    ...
    if (null != g_root_block) {
        if (0 != g_fields_to_mark.length) {
            if (TRACE_TRICKLE) for (ff of (log("trickle, checking for marked fields, nmarked=" + String(g_fields_to_mark.length) + ", marked_fields="), 
            g_fields_to_mark)) log(str.conv(" {n}", ff));
            for (topnode of TREES) TRACE_TRICKLE && log("trickle, check top node " + topnode.val), 
            check_tree_for_dirty(topnode);
            g_fields_to_mark.length = 0;
        }
        if (TRACE_TRICKLE && g_last_drawn_seq != g_currseq && log(str.conv("trickle, last_drawn={}, currseq={}", g_last_drawn_seq, g_currseq)), 
        g_last_drawn_seq < g_currseq) {
            if (rebuild_affected_blocks(g_root_block), 
            getn(runtime, F_update_blocklist) == Y && debug_get_blocks(adr(debug_blocklist)), 
            null != aaaa.menubar_func) {
                var save_root = g_mon_root_block;
                g_mon_root_block = null, 
                rebuild_affected_blocks(g_menu_block), 
                g_mon_root_block = save_root;
            }
            g_last_drawn_seq = g_currseq, 
            TRACE_TRICKLE && log(str.conv("..end trickle, last_drawn={n}", g_last_drawn_seq));
        }
    }
}
...
export function cookie_write(key, val, options) {
    ...
    lifespan != U && (cookie += `;max-age=${lifespan};SameSite=Lax`), 
    cookie.length < 4096 ? document.cookie = cookie : console.error(`Warning, cookie too big (${cookie.length})`);
};
...
function cookie_encode(val) {
    ...
    return val instanceof a_tree ? bytes_put_vtree(buf, adr(val)) : val instanceof a_path ? bytes_put_vtree(buf, val) : bytes_put_val(buf, val, 0), 
    arraybuf_to_base64(buf.arraybuf, buf.position);
}
...
function cookie_decode(base64_str) {
    ...
    return opcode == SOP_TREE_BIN ? bytes_get_tree(bytes, adr(val = new a_tree(_M, "$cookie_decode"))) : val = bytes_get_val2(bytes, opcode), 
    val;
}
...
function arraybuf_to_base64(arraybuffer, len) {
    for (var bytes = new Uint8Array(arraybuffer), ss = "", i = 0; i < len; i += 3) ss += BASE64C[bytes[i] >> 2], 
    ss += BASE64C[(3 & bytes[i]) << 4 | bytes[i + 1] >> 4], 
    ss += BASE64C[(15 & bytes[i + 1]) << 2 | bytes[i + 2] >> 6], 
    ss += BASE64C[63 & bytes[i + 2]];
    return len % 3 == 2 ? ss = ss.substring(0, ss.length - 1) + "=" : len % 3 == 1 && (ss = ss.substring(0, ss.length - 2) + "=="), 
    ss;
}
...
function k_init() {
    DRAW_EVENT = new a_tree(_M, "$DRAW_EVENT"), 
    setv(_M, 0, DRAW_EVENT, F_evkind, EV_DRAW), 
    g_interface_locked = !1, 
    g_capture_blabel = 0, 
    g_field_mark_func = {}, 
    g_fields_to_mark = [], 
