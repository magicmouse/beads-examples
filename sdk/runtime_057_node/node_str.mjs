import { a_meas, a_path, a_image, a_tree, a_function, a_sound, a_gradient, ERR, Y, N, U } from "./node_std.mjs";

import * as std from "./node_std.mjs";
      ...
      case 1:
        if ("]" == cc) {
            switch (token += "]") {
              ...
              case "[hour12]":
                token = (hh = std.getn(datepath, std.F_dt_hour)) >= 0 && hh <= 11 ? to_str(hh, {
                    min: 2,
                    zero_pad: Y
                }) : hh >= 12 && hh <= 23 ? to_str(hh - 12, {
                    min: 2,
                    zero_pad: Y
                }) : std.ERR_STR;
                break;
...
export function tree_to_json(base, options = null) {
    ...
    function numval(val, forcess) {
        return Number.isNaN(val) ? '"@ERR"' : val === U ? "null" : val === Y ? "true" : val === N ? "false" : val === std.INFINITY ? '"@INFINITY"' : val === std.NEG_INFINITY ? '"@NEG_INFINITY"' : std.is_enum_b(val) ? '"@' + enum_to_str(val) + '"' : forcess ? '"' + String(val) + '"' : String(val);
    }
...
export function enum_to_str(val, use_stringtable = !1) {
    return std.CHECKS && !is_enumb(val) && std.halt(), void 0 === std.ENUMS[val] ? "ENUM:" + to_str(val -= std.FIRST_ENUM, {
        min: 10,
        zero_pad: Y
    }) : std.ENUMS[val];
};
...
export function to_char(n) {
    return Number.isNaN(n) || n === U || n <= 0 ? "" : String.fromCharCode(n);
};
...
export function to_str(val, options = null) {
    ...
    for (let property in options) switch (property) {
      case "increase":
        is_autoinc = options.increase == Y;
        break;

      case "decrease":
        is_autodec = options.decrease == Y;
        break;
      ...
      case "currency":
        is_currency = options.currency == Y;
        break;
      ...
      case "human":
        is_human = options.human == Y;
        break;

      case "k":
        is_kflag = options.k == Y;
        break;
      ...
      case "neg_paren":
        is_negparen = options.neg_paren == Y;
        break;

      case "pos_plus":
        is_posplus = options.pos_plus == Y;
        break;

      case "percent":
        is_percent = options.percent == Y;
        break;
      ...
      case "zero_pad":
        is_zeropad = options.zero_pad == Y;
        break;

      case "zero_drop":
        is_zerodrop = options.zero_drop == Y;
        break;
    ...
    } else if (val instanceof RegExp) ss = val.toString(); else if (val instanceof std.a_sound) ss = to_str(val.id, options); else if (val instanceof std.a_sys_gradient) ss = "gradient:" + val.grad_kind; else if (val === std.INFINITY) ss = "INFINITY", 
    is_zeropad = !1; else if (val === std.NEG_INFINITY) ss = "-INFINITY", is_zeropad = !1; else if (val === Y) ss = "Y", is_zeropad = !1; else if (val === N) ss = "N", is_zeropad = !1; else if (is_enumb(val)) ss = enum_to_str(val), 
    is_zeropad = !1; else if (Number.isNaN(val)) ss = "ERR", is_zeropad = !1; else {
...
export function node_to_str(elem, show_u) {
    ...
    if (null !== elem) if (elem.eflags & std.EF_VAL_IS_U) {
        if (!show_u) return "";
        nodetype = ":U", nodeval = "";
    } else elem.eflags & std.EF_VAL_IS_ERR ? (nodetype = ":ERR", nodeval = "") : elem.eflags & std.EF_VAL_IS_NUM ? (nodetype = ":num", nodeval = to_str(elem.val)) : elem.eflags & std.EF_VAL_IS_STR ? (nodetype = ":str", 
    nodeval = '"' + elem.val + '"') : elem.eflags & std.EF_VAL_IS_IMAGE ? (nodetype = ":image", nodeval = "IMAGE") : elem.eflags & std.EF_VAL_IS_FUNC ? (nodetype = ":func", nodeval = elem.val.name) : elem.eflags & std.EF_VAL_IS_PATH ? (nodetype = ":path", 
    nodeval = conv_path(elem.val)) : elem.eflags & std.EF_VAL_IS_MEAS ? (nodetype = ":meas", nodeval = "MEAS") : elem.eflags & std.EF_VAL_IS_SOUND ? (nodetype = ":sound", nodeval = "SOUND") : elem.eflags & std.EF_VAL_IS_GRAD ? (nodetype = ":grad", 
    nodeval = "GRAD") : elem.eflags & std.EF_VAL_IS_BYTES ? (nodetype = ":bytes", nodeval = "BYTES") : elem.eflags & std.EF_VAL_IS_COLOR ? (nodetype = ":color", nodeval = color_to_rgb_str(elem.val)) : elem.eflags & std.EF_VAL_IS_DATE ? (nodetype = ":date", 
    elem.val, nodeval = "DATE") : elem.eflags & std.EF_VAL_IS_VIDEO ? (nodetype = ":video", nodeval = "VIDEO") : elem.eflags & std.EF_VAL_IS_OBJECT ? (nodetype = ":object", nodeval = "OBJECT") : std.argument_err("bad node"); else nodetype = "<NULL>", 
    nodeval = "";
...
export function str_repeat(ss, n) {
    let nn;
    return ss === std.U_STR || ss === std.ERR_STR ? ss : std.is_numeric(n) !== Y ? std.ERR_STR : (nn = Math.round(n)) <= 0 ? "" : 1 === nn ? ss : ss.repeat(nn);
};
...
export function str_begins(ss, pattern) {
    ...
    return pat_len > str_len ? N : ss.substr(0, pat_len) === pattern ? Y : N;
};

export function str_ends(ss, pattern) {
    if (null === ss || null === pattern) return U;
    let str_len = ss.length, pat_len = pattern.length;
    return pat_len > str_len ? N : ss.substr(str_len - pat_len, pat_len) === pattern ? Y : N;
};
...
export function subset(ss, options) {
    ...
    if (std.is_err_enum(start) === Y || std.is_err(len) === Y || std.is_err(end) === Y) return ERROR_STRING;
...
export function str_digit(ss, options) {
    let cc, digit, rev_flag = N, from = 1;
...
export function str_ins(ss, ins, options) {
    ...
    for (let property in options) switch (property) {
      case "del":
        if (del = options.del, std.is_numeric(del) != Y) return std.ERR_STR;
        del < 0 && (del = 0);
        break;

      case "to":
        if (to = options.to, std.is_numeric(to) != Y) return std.ERR_STR;
        to < 1 && (to = 1);
        break;

      case "rev":
        reverse = options.rev == Y;
        break;
...
export function str_del(ss, options) {
    ...
    for (let property in options) switch (property) {
      case "from":
        if (from = options.from, std.is_numeric(from) != Y) return std.ERR_STR;
        from < 1 && (from = 1);
        break;
      ...
      case "len":
        if (len = options.len, std.is_numeric(len) != Y) return std.ERR_STR;
        if (len <= 0) return "";
        break;
...
export function str_to_regexp(input, options) {
    ...
    for (var property in options) switch (property) {
      case "ignore_case":
        ignore = options.ignore_case == std.Y;
        break;

      case "global":
        global = options.global == std.Y;
        break;

      case "multiline":
        multiline = options.multiline == std.Y;
        break;
...
export function str_pos(haystack, needle, options) {
    var start = 1, ignore = std.Y, list = new a_tree(_M, "str_pos");
...
export function split_lines_words(module, loc, buf, dest, _options) {
    ...
    if (std.trunc_tree(module, loc, dest), null != buf && buf != std.ERR_STR && 0 !== (len = buf.length)) {
        for (let property in _options) switch (property) {
          case "quotes":
            handle_quotes = _options.quotes === Y;
            break;
...
export function str_range(base, start, stop = 99999999999999) {
    return 0 === base.length || std.is_numeric(start) !== Y || std.is_numeric(stop) !== Y ? "" : (start < 0 && (start = 0), stop > base.length && (stop = base.length), base.slice(start, stop));
};
            ...
            switch (token += "}") {
              case "{}":
                if (null === (parm = parms[parmx]) || void 0 == parm) token = "U"; else if ("number" == typeof parm || "number" == typeof parm) token = to_str(parm, {
                    dig: 3,
                    decrease: Y
                }); else if ("string" == typeof parm) token = parm; else if ("boolean" == typeof parm) token = conv_bool(parm); else if (parm instanceof std.Rectangle) token = conv_rect(parm); else if (parm instanceof std.a_tree) token = parm.sub; else if (parm instanceof std.a_path) token = conv_path(parm); else if (parm instanceof std.a_pathx) token = conv_pathx(parm); else if (parm instanceof std.a_meas) token = to_str(parm.mag, {
                    dig: 3,
                    decrease: Y
